...
std::string format_arena_used(const ginfer::ArenaMemorySummary& arena) {
    return format_bytes(arena.used_bytes) + " / " + format_bytes(arena.capacity_bytes);
}

std::string format_arena_peak(const ginfer::ArenaMemorySummary& arena) {
    return format_bytes(arena.peak_used_bytes) + " / " + format_bytes(arena.capacity_bytes);
}

std::string format_sampling(const ginfer::ResolvedSamplingParameters& sampling) {
...
std::string format_finish(ginfer::FinishReason reason) {
    switch (reason) {
...
std::string format_kv_cache(ginfer::KvCacheStorage storage) {
    return storage == ginfer::KvCacheStorage::BFloat16 ? "bf16" : "int8-group64";
}

std::string format_kv_capacity_mode(ginfer::KvCapacityMode mode) {
    return mode == ginfer::KvCapacityMode::Automatic ? "auto" : "explicit";
}
...
class StreamingSink final : public ginfer::OutputSink {
public:
    void publish(ginfer::OutputDelta delta) override {
        std::ostream& output =
            delta.channel == ginfer::OutputChannel::Reasoning ? std::cerr : std::cout;
        output << delta.text;
        output.flush();
...
void print_load_summary(const ginfer::LoadSummary& load, double wall_seconds) {
...
void print_generation_summary(const ginfer::GenerationResult& result,
                              const ginfer::ResolvedSamplingParameters& sampling,
                              const ginfer::MemorySummary& memory) {
    ...
    const ginfer::SpeculativeStats& speculative = result.speculative;
    if (speculative.enabled) {
        const std::string backend =
            speculative.backend == ginfer::SpeculativeBackend::DFlash ? "dflash" : "mtp";
        print_metric(backend + " draft window", std::to_string(speculative.draft_window));
        if (speculative.verification_window != speculative.draft_window) {
            print_metric(backend + " maximum verify window",
                         std::to_string(speculative.verification_window));
            print_metric(backend + " expanded rounds",
                         std::to_string(speculative.expanded_rounds));
        }
...
int main(int argc, char** argv) {
    try {
        const ginfer::cli::Options cli = ginfer::cli::parse_options(argc, argv);
        if (cli.help_requested) {
            std::cout << ginfer::cli::usage_text(argv[0]);
            return 0;
        }

        ginfer::PromptInput input =
            cli.messages_path.empty()
                ? ginfer::product::prompt_from_text(cli.prompt, cli.enable_thinking)
                : ginfer::product::prompt_from_messages(cli.messages_path, cli.enable_thinking,
                                                        cli.enable_vision);
        input.options.reasoning_effort = cli.reasoning_effort;

        ginfer::RequestOptions request;
        ...
        ginfer::product::LoadProgressRenderer load_progress(
            std::cerr, ginfer::product::stderr_load_progress_options());
        ...
        ginfer::PreparedPrompt prompt = engine.prepare(std::move(input));

        StreamingSink sink;
        ginfer::GenerationHandle generation = engine.submit(std::move(prompt), std::move(request));
        const ginfer::ResolvedSamplingParameters sampling = generation.resolved_sampling();
        const ginfer::GenerationResult result             = generation.wait(&sink);
        sink.finish_streams();
        ...
        std::cerr << ginfer::cli::usage_text(argv[0]);
        return 1;
    }
}
