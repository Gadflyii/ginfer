...
std::string usage_text(const char* argv0) {
    return std::string("usage: ") + argv0 +
           " <model.ginfer> (--prompt <text>|--messages <messages.json>)\n"
           "       [--max-context N] [--kv-capacity N|auto] [--prefill-chunk N] [--max-new N]\n"
           "       [--device N]\n"
           "       [--kv-dtype bf16|int8] [--spec auto|none|mtp|dflash --draft-tokens N]\n"
           "       [--lm-head-draft]\n"
...
Options parse_options(int argc, char** argv) {
    ...
    if (argc < 2) { throw std::invalid_argument(".ginfer model path is required"); }
