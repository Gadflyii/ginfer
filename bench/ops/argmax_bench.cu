...
//
//   ./ginfer_argmax_bench
//   ./ginfer_argmax_bench --shape full --cols 1
//   ./ginfer_argmax_bench --shape shortlist --cols 120
...
Options parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--shape") {
            ...
            usage(argc > 0 ? argv[0] : "ginfer_argmax_bench");
            std::exit(0);
        } else {
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        std::fprintf(stderr, "ginfer_argmax_bench: %s\n", e.what());
        return 2;
    }
    return 0;
}
