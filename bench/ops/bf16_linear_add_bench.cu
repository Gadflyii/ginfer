...
Result make_result(std::string route, std::int32_t tokens, const bench::ColdTiming& timing,
    ...
    const double memory_floor_us   = bench::dram_memory_floor_us(logical_bytes, roof);
    const double compute_floor_us  = useful_flops / (kRtx5090DenseBf16Tflops * 1.0e12) * 1.0e6;
    const double roofline_floor_us = std::isfinite(memory_floor_us)
                                         ? std::max(memory_floor_us, compute_floor_us)
                                         : std::numeric_limits<double>::quiet_NaN();
    const double roofline_pct      = std::isfinite(roofline_floor_us)
...
void print_result(const Result& result) {
    const char* bound = !std::isfinite(result.memory_floor_us)
                            ? "unknown"
                            : (result.memory_floor_us >= result.compute_floor_us ? "memory"
                                                                                : "compute");
    char roof[32];
    if (std::isfinite(result.roofline_pct)) {
        std::snprintf(roof, sizeof(roof), "%6.2f%%", result.roofline_pct);
    } else {
        std::snprintf(roof, sizeof(roof), "unknown");
    }
    std::printf("T=%-4d %-36s median=%8.3f us  %7.1f GB/s READ=%6.2f%%  "
                "%7.2f TFLOP/s TC=%6.2f%%  %-7s roofline=%s\n",
                result.tokens, result.route.c_str(), result.timing.median_us, result.effective_gbs,
                result.read_pct, result.useful_tflops, result.tc_pct, bound, roof);
}
...
int main(int argc, char** argv) {
    try {
        ...
        std::fprintf(stderr, "ginfer_bf16_linear_add_bench: %s\n", error.what());
        return 1;
    }
}
