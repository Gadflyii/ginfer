...
[[noreturn]] void usage(const char* message) {
    std::fprintf(stderr,
                 "error: %s\n"
                 "usage: ginfer_causal_softmax_attention_bench "
    ...
    void launch(Entry entry, cudaStream_t stream) {
        if (entry == Entry::Append) {
            ...
            ops::gqa_attention(q_tensor_, k_tensor_, v_tensor_, positions_tensor_, validity,
                               ...
                               output_tensor_, bench_occupancy(), stream);
        } else {
            ops::gqa_attention_cached(q_tensor_, positions_tensor_, kScale, cache_view_, envelope_,
                                      workspace_, output_tensor_, bench_occupancy(), stream);
        }
    }
...
void report(const Result& result) {
    ...
    char dram[32];
    bench::format_dram_roof(dram, sizeof(dram), gbps, bench::current_device_dram_roofline());
    std::printf("entry=%-6s geometry=%-14s kv=%-4s mapping=%-10s execution=%-5s cache=%-4s "
                ...
                "logical=%8.1f GB/s (%s) math=%7.2f TFLOP/s (%5.1f%% of %.1f)\n",
                ...
                result.timing.median_us, result.timing.min_us, result.timing.p95_us, gbps, dram,
                tflops, tflops / kDenseBf16TcTflops * 100.0, kDenseBf16TcTflops);
}
...
int main(int argc, char** argv) {
    try {
        ...
        std::fprintf(stderr, "ginfer_causal_softmax_attention_bench: %s\n", error.what());
        return 1;
    }
}
