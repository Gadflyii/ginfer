...
[[noreturn]] void usage(const char* message) {
    std::fprintf(stderr,
                 "error: %s\n"
                 "usage: ginfer_context_softmax_attention_bench "
...
std::size_t workspace_capacity(std::int32_t tokens, std::int32_t context) {
    ...
    return ops::bidirectional_gqa_attention_workspace_capacity_bytes(envelope, tokens, tokens, 1, bench_occupancy());
}
...
void report(const Result& result) {
    ...
    std::printf("entry=context execution=%-5s cache=%-4s T=%2d L=%6d workspace=%8zu "
                ...
                "(%s) math=%7.2f TFLOP/s (%5.1f%% of %.1f)\n",
                ...
                result.timing.min_us, result.timing.p95_us, gbps, dram, tflops,
...
int main(int argc, char** argv) {
    try {
        ...
        std::fprintf(stderr, "ginfer_context_softmax_attention_bench: %s\n", error.what());
        return 1;
    }
}
