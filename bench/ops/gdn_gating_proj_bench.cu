...
//
// Examples:
//   ./build/bench/ginfer_gdn_gating_proj_bench --35b --candidate auto
//   ./build/bench/ginfer_gdn_gating_proj_bench --35b \
...
void run(const Options& opt, std::int32_t tokens, std::size_t interval_capacity,
    ...
    const auto plan                   = opt.auto_route || opt.composed_norm_control
                                            ? ops::detail::bf16_gdn_gating_resolve_plan(problem, bench_occupancy())
                                            : ops::detail::bf16_gdn_gating_resolve_candidate(opt.candidate, problem,
                                                                                             bench_occupancy());
    const auto norm_plan              = ops::detail::bf16_gdn_norm_gating_resolve_plan(problem, bench_occupancy());
    ...
    const auto launch = [&](cudaStream_t stream) {
        if (opt.norm_control && opt.composed_norm_control) {
            ...
            if (opt.geometry35) {
                ops::gdn_gating_proj(th, parent, tA_log, tdt_bias, ws, tg, tbeta, bench_occupancy(),
                                     stream);
            } else {
                ops::gdn_gating_proj(th, wa, wb, tA_log, tdt_bias, ws, tg, tbeta, bench_occupancy(),
                                     stream);
            }
        } else if (opt.norm_control) {
            if (opt.geometry35) {
                ops::gdn_norm_gating_proj(tx, tnorm_weight, 1.0e-6F, parent, tA_log, tdt_bias, ws,
                                          th, tg, tbeta, bench_occupancy(), stream);
            } else {
            ...
            if (opt.geometry35) {
                ops::gdn_gating_proj(tx, parent, tA_log, tdt_bias, ws, tg, tbeta, bench_occupancy(),
                                     stream);
            } else {
                ops::gdn_gating_proj(tx, wa, wb, tA_log, tdt_bias, ws, tg, tbeta, bench_occupancy(),
                                     stream);
            }
        } else {
            ops::detail::bf16_gdn_gating_execute_candidate(opt.candidate, tx, wa, wb, tA_log,
                                                           tdt_bias, ws, tg, tbeta,
                                                           bench_occupancy(), stream);
        }
    };
