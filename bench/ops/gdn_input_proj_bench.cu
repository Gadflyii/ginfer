...
[[noreturn]] void usage(const char* message) {
    std::fprintf(stderr,
                 "error: %s\n"
                 "usage: ginfer_gdn_input_proj_bench "
...
void report(const Result& result) {
    ...
    std::printf("entry=gdn_input_proj format=%-6s policy=%-3s cache=%-4s T=%4d "
                ...
                "logical=%8.1f GB/s (%s) math=%8.2f TFLOP/s\n",
                ...
                result.timing.p95_us, gbps, dram, tflops);
}
...
int main(int argc, char** argv) {
    try {
        ...
        std::fprintf(stderr, "ginfer_gdn_input_proj_bench: %s\n", error.what());
        return 1;
    }
}
