#pragma once
//
// ginfer_bench_common.h — shared bench harness for L1 op performance binaries.
//
...
//
// DRAM spec GB/s is a closed SKU map, not a formula and not a 5090 default.
// GINFER_BENCH_DRAM_ROOFLINE_MAP_ONLY compiles just that map for the host test.
...
// More specific names first: PRO 6000 Server vs Workstation before a collision.
inline DramRoofline dram_roofline_for_gpu_name(std::string_view name) {
    const auto contains = [name](std::string_view needle) {
        return name.find(needle) != std::string_view::npos;
    };
    if (contains("RTX PRO 6000")) {
        return {true, contains("Server") ? 1597.0 : 1792.0};
    }
    if (contains("RTX 5090")) { return {true, 1792.0}; }
    if (contains("RTX 4090")) { return {true, 1008.0}; }
    if (contains("RTX 3090")) { return {true, 936.0}; }
    return {};
}

inline const char* format_dram_spec_gbs(char* out, std::size_t n, const DramRoofline& roof) {
    if (roof.known) {
        std::snprintf(out, n, "%.1f", roof.spec_gbs);
    } else {
...
inline const char* format_dram_roof(char* out, std::size_t n, double gbs,
                                   const DramRoofline& roof) {
    if (roof.known) {
        std::snprintf(out, n, "%.1f%% of %.0f", gbs / roof.spec_gbs * 100.0, roof.spec_gbs);
    } else {
        std::snprintf(out, n, "unknown");
    }
    return out;
...
inline double dram_spec_pct(double gbs, const DramRoofline& roof) {
    if (!roof.known) { return std::numeric_limits<double>::quiet_NaN(); }
    return gbs / roof.spec_gbs * 100.0;
}

inline double dram_memory_floor_us(std::uint64_t bytes, const DramRoofline& roof) {
    if (!roof.known) { return std::numeric_limits<double>::quiet_NaN(); }
    return static_cast<double>(bytes) / (roof.spec_gbs * 1.0e9) * 1.0e6;
}

} // namespace ginfer::bench

#if !defined(GINFER_BENCH_DRAM_ROOFLINE_MAP_ONLY)
...
inline const OccupancyPolicy& bench_occupancy() {
    static DeviceContext device;
    return device.occupancy();
}

inline DramRoofline device_dram_roofline(int dev = 0) {
    cudaDeviceProp properties{};
    CUDA_CHECK(cudaGetDeviceProperties(&properties, dev));
    return dram_roofline_for_gpu_name(properties.name);
}

inline DramRoofline current_device_dram_roofline() {
    int device = 0;
    CUDA_CHECK(cudaGetDevice(&device));
    return device_dram_roofline(device);
}

// cudaDeviceProp memory-clock fields were removed in CUDA 13; GB/s is the
// closed SKU map. Unlisted names return NaN so callers cannot score a fake 1792%.
inline double device_peak_bw_gbs(int dev = 0) {
    const DramRoofline roof = device_dram_roofline(dev);
    return roof.known ? roof.spec_gbs : std::numeric_limits<double>::quiet_NaN();
}
...
inline void print_result(const char* tag, const Result& r) {
    const DramRoofline roof = current_device_dram_roofline();
    if (roof.known) {
        std::printf(
            "%-32s median=%8.2f us  min=%8.2f us  p95=%8.2f us  %8.1f GB/s  (%.1f%% of %.0f GB/s "
            "roofline)\n",
            tag, r.median_us, r.min_us, r.p95_us, r.gbs, r.gbs / roof.spec_gbs * 100.0,
            roof.spec_gbs);
    } else {
        std::printf("%-32s median=%8.2f us  min=%8.2f us  p95=%8.2f us  %8.1f GB/s  (unknown)\n",
...
#endif // !GINFER_BENCH_DRAM_ROOFLINE_MAP_ONLY
