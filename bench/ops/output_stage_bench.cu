...
int run(const Options& options) {
    ...
    ginfer::DeviceBuffer norm(norm_host.size() * sizeof(std::uint16_t));
    CUDA_CHECK(cudaMemcpy(norm.p, norm_host.data(), norm.bytes, cudaMemcpyHostToDevice));
    ginfer::DeviceBuffer hidden(static_cast<std::size_t>(kHidden) * kMaxTokens *
                                sizeof(std::uint16_t));
    ginfer::DeviceBuffer logits(static_cast<std::size_t>(kVocab) * kMaxTokens *
                                sizeof(std::uint16_t));
    ginfer::DeviceBuffer tokens(static_cast<std::size_t>(kMaxTokens) * sizeof(std::int32_t));
    ginfer::DeviceBuffer flush(options.flush_size);
    ...
    std::printf("# gpu=%s cuda=13.1 sm=%d%d flush_mib=%zu warmup=%d repeat=%d\n", properties.name,
                properties.major, properties.minor, options.flush_size >> 20, options.warmup,
                options.repeat);
...
int main(int argc, char** argv) {
    try {
        ...
        std::fprintf(stderr, "ginfer_output_stage_bench: %s\n", error.what());
        return 2;
    }
}
