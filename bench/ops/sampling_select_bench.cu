...
//
//   ./ginfer_sampling_select_bench --sample --batch 8 --mode stochastic
//   ./ginfer_sampling_select_bench --muse --batch 1 --top-k 64
//   ./ginfer_sampling_select_bench --mtp --mode stochastic --mtp-k 5
//   ./ginfer_sampling_select_bench --matrix
...
constexpr std::int32_t kMusePhysicalRows = 202048;
constexpr std::int32_t kMuseTokenDomain  = 202048;
...
void usage(const char* argv0) {
    std::printf("usage: %s [--sample|--mtp|--matrix] [--mode greedy|stochastic] "
                "[--muse] [--batch 1..8] [--mtp-k 1..5] [--top-k 1..64] [--no-counts]\n",
                argv0);
}
...
Options parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--sample") {
            ...
            usage(argc > 0 ? argv[0] : "ginfer_sampling_select_bench");
            std::exit(0);
        } else {
    ...
    if (options.muse && !options.top_k_set) { options.top_k = 64; }
    if (options.matrix && (options.sample || options.mtp)) {
        throw std::invalid_argument("--matrix cannot be combined with --sample or --mtp");
    }
    if (options.muse && (options.mtp || options.matrix)) {
        throw std::invalid_argument("--muse is an ordinary-sampling profile");
    }
    ...
    const int maximum_top_k = options.muse ? 64 : 20;
    if (options.top_k < 1 || options.top_k > maximum_top_k) {
        throw std::invalid_argument("--top-k exceeds the selected profile cap");
    }
    return options;
}
...
DeviceBuffer make_muse_logits(int cols) {
    std::vector<std::uint16_t> host(static_cast<std::size_t>(kMusePhysicalRows) * cols);
    for (int col = 0; col < cols; ++col) {
        const int hot = (17 + col * 7919) % kMuseTokenDomain;
        for (int row = 0; row < kMusePhysicalRows; ++row) {
            float value = -8.0f + static_cast<float>((row * 17 + col * 31) % 4096) / 4096.0f;
            if (row == hot) { value = 8.0f; }
            host[static_cast<std::size_t>(col) * kMusePhysicalRows + row] = f32_to_bf16(value);
        }
    }
    DeviceBuffer device(host.size() * sizeof(std::uint16_t));
    CUDA_CHECK(cudaMemcpy(device.p, host.data(), device.bytes, cudaMemcpyHostToDevice));
...
void run_muse_sample(DeviceBuffer& logits, int batch, int top_k, Mode mode) {
    std::vector<ops::SamplingConfig> host_configs(static_cast<std::size_t>(batch));
    for (int row = 0; row < batch; ++row) {
        ops::SamplingConfig& config = host_configs[static_cast<std::size_t>(row)];
        config.temperature          = mode == Mode::Greedy ? 0.0f : 1.0f;
        config.top_k                = top_k;
        config.top_p                = 0.95f;
        config.seed                 = 20260820ull + static_cast<unsigned long long>(row);
    }
    DeviceBuffer configs(host_configs.size() * sizeof(ops::SamplingConfig));
    configs.copy_from_host(host_configs.data(), configs.bytes);
    DeviceBuffer out(static_cast<std::size_t>(batch) * sizeof(std::int32_t));
    std::vector<std::int32_t> positions(static_cast<std::size_t>(batch));
    for (int row = 0; row < batch; ++row) { positions[static_cast<std::size_t>(row)] = 4096 + row; }
    DeviceBuffer device_positions = make_i32(positions);
    Tensor tlogits(logits.p, DType::BF16, {kMusePhysicalRows, batch});
    Tensor tout(out.p, DType::I32, {batch});
    Tensor tpositions(device_positions.p, DType::I32, {batch});
    const std::size_t workspace_bytes =
        ops::sampling_workspace_capacity_bytes(kMuseTokenDomain, batch, batch);
    WorkspaceArena workspace(std::max<std::size_t>(workspace_bytes, 256));
    const auto* config_ptr = static_cast<const ops::SamplingConfig*>(configs.p);

    const double bytes = static_cast<double>(kMuseTokenDomain) * 2.0 * batch;
    const Result result = bench_loop(
        [&](cudaStream_t stream) {
            ops::sample(tlogits, tout, kMuseTokenDomain, config_ptr, tpositions,
                        ops::kSamplePurposeDecode, workspace, stream);
        },
        bytes);
    const std::string mode_label =
        mode == Mode::Greedy ? "greedy"
                             : "stochastic T=1 top_p=.95 top_k=" + std::to_string(top_k);
    const std::string label = "Muse B=" + std::to_string(batch) + " " + mode_label;
    print_result(label.c_str(), result);
}
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (options.muse) {
            DeviceBuffer logits = make_muse_logits(options.batch);
            std::printf("payload: profile=Muse physical_rows=%d token_domain=%d "
                        "logits=%.3f MiB/row\n",
                        kMusePhysicalRows, kMuseTokenDomain,
                        static_cast<double>(kMuseTokenDomain * 2) / 1048576.0);
            std::printf("workspace: lanes=%d %.1f KiB\n", options.batch,
                        static_cast<double>(ops::sampling_workspace_capacity_bytes(
                            kMuseTokenDomain, options.batch, options.batch)) /
                            1024.0);
            run_muse_sample(logits, options.batch, options.top_k, options.mode);
            return 0;
        }
        ...
        std::fprintf(stderr, "ginfer_sampling_select_bench: %s\n", e.what());
        return 2;
    }
    return 0;
}
