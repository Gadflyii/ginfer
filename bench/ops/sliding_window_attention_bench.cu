...
[[noreturn]] void usage(const char* message) {
    std::fprintf(stderr,
                 "error: %s\n"
                 "usage: ginfer_sliding_window_attention_bench "
...
std::size_t workspace_capacity(std::int32_t tokens, std::int32_t context) {
    ...
    return ops::swa_workspace_capacity_bytes(envelope, tokens, tokens, 1, 4096);
}
...
void report(const Result& result) {
    ...
    std::printf("entry=sliding_window execution=%-5s cache=%-4s T=%2d L=%6d W=%d "
                ...
                "useful=%8.1f GB/s (%s) math=%7.2f TFLOP/s (%5.1f%% of %.1f)\n",
...
int main(int argc, char** argv) {
    try {
        ...
        std::fprintf(stderr, "ginfer_sliding_window_attention_bench: %s\n", error.what());
        return 1;
    }
}
