...
int main(int argc, char** argv) {
    try {
        ...
        ginfer::DeviceBuffer input =
            bench::make_bf16(static_cast<std::size_t>(options.hidden) * max_t);
        ginfer::DeviceBuffer residual = bench::make_bf16(static_cast<std::size_t>(kRows) * max_t);
...
#undef RUN_MMA
        ...
        std::fprintf(stderr, "ginfer_w8_linear_add_bench: %s\n", error.what());
        return 1;
    }
}
