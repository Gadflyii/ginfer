...
int main(int argc, char** argv) {
    try {
        ...
        ginfer::DeviceBuffer flush(kFlushBytes);
        ginfer::DeviceBuffer input = bench::make_bf16(static_cast<std::size_t>(kHidden) * max_t);
        ginfer::DeviceBuffer output(static_cast<std::size_t>(kOutputRows) * max_t * 2);
        ginfer::DeviceBuffer gate_up(static_cast<std::size_t>(kGateUpRows) * max_t * 2);
        ...
        std::fprintf(stderr, "ginfer_w8_linear_swiglu_bench: %s\n", error.what());
        return 1;
    }
}
