#include "ginfer_bench_support.h"

#include "ginfer/engine.h"

#if __has_include(<cuda_profiler_api.h>)
#include <cuda_profiler_api.h>
#define GINFER_HAS_CUDA_PROFILER_CONTROL 1
#else
#define GINFER_HAS_CUDA_PROFILER_CONTROL 0
#endif
...
void fill_cuda_environment(ginfer::bench::BenchEnvironment& env, int device) {
...
bool has_decode_tests(const std::vector<ginfer::bench::BenchTest>& tests) {
...
ginfer::RequestOptions benchmark_request(const ginfer::bench::BenchTest& test) {
...
ginfer::bench::RepTiming run_repetition(ginfer::Engine& engine,
                                        const ginfer::bench::BenchTest& test,
                                        const std::vector<ginfer::TokenId>& corpus) {
    const int prompt_tokens = test.kind == ginfer::bench::TestKind::Decode
                                  ? ginfer::bench::kDecodeSeedTokens
                                  : test.n_prompt;
    auto prompt = engine.prepare_tokens(ginfer::bench::prompt_slice(corpus, prompt_tokens), false);
    ginfer::GenerationResult generated =
    ...
    if (generated.finish_reason != ginfer::FinishReason::OutputLimit) {
        throw std::runtime_error(test.label + " did not finish at the requested output limit");
    }

    ginfer::bench::RepTiming timing;
...
void prime_decode_graph(ginfer::Engine& engine, ginfer::bench::BenchEnvironment& env,
                        const std::vector<ginfer::TokenId>& corpus) {
    ...
    const ginfer::bench::BenchTest prime{ginfer::bench::TestKind::Decode, 0, decode_tokens,
                                         "decode-graph-prime"};
    (void)run_repetition(engine, prime, corpus);
    env.decode_graph_primed = true;
}

void write_output(const ginfer::bench::BenchOptions& options, const std::string& text) {
...
int main(int argc, char** argv) {
    ginfer::bench::BenchOptions options;
    try {
        options = ginfer::bench::parse_args(argc, argv);
    } catch (const std::exception& error) {
        std::cerr << "ginfer_bench: " << error.what() << '\n';
        return 2;
    }
    if (options.help_requested) {
        std::cout << ginfer::bench::usage_text(argc > 0 ? argv[0] : "ginfer_bench");
        return 0;
    }

    try {
        const std::vector<ginfer::TokenId> corpus =
            ginfer::bench::load_corpus_ids(options.corpus_path);
        const std::vector<ginfer::bench::BenchTest> tests = ginfer::bench::expand_tests(options);
#if !GINFER_HAS_CUDA_PROFILER_CONTROL
        if (options.profile_measured) {
            throw std::invalid_argument(
                "this CUDA toolkit does not provide cuda_profiler_api.h; "
                "run without --profile-measured");
        }
#endif
        ...
        ginfer::bench::validate_prompt_lengths(tests, corpus.size());
        const std::uint32_t max_context = ginfer::bench::resolve_max_context(
        ...
        engine_options.kv_capacity   = ginfer::KvCapacityPolicy::explicit_capacity(max_context);
        engine_options.prefill_chunk = options.prefill_chunk;
        engine_options.kv_cache      = options.kv_cache;
        engine_options.speculative.backend       = options.mtp_draft_tokens == 0
                                                       ? ginfer::SpeculativeBackend::None
                                                       : ginfer::SpeculativeBackend::Mtp;
        ...
        ginfer::bench::BenchEnvironment env;
        env.artifact_path            = options.artifact_path;
        env.artifact_file_size_bytes = ginfer::bench::file_size_or_zero(options.artifact_path);
        ...
        if (options.use_cuda_graph && has_decode_tests(tests)) {
            env.decode_graph_prime_output_tokens =
                ginfer::bench::decode_graph_prime_output_tokens(options.mtp_draft_tokens);
        }

        std::cerr << "[ginfer_bench] loading " << options.artifact_path
                  << " (max_context=" << max_context
                  << ", kv_cache=" << ginfer::bench::kv_cache_name(options.kv_cache) << ")\n";
        ginfer::Engine engine(std::move(engine_options));
        ...
        std::vector<ginfer::bench::TestResult> results;
        results.reserve(tests.size());
        for (std::size_t i = 0; i < tests.size(); ++i) {
            const auto& test = tests[i];
            std::cerr << "[ginfer_bench] test " << (i + 1) << '/' << tests.size() << ' '
                      << test.label << ": warmup=" << options.warmup
                      << " reps=" << options.repetitions << '\n';

            ginfer::bench::TestResult result;
            result.test = test;
        ...
        case ginfer::bench::OutputFormat::Table:
            report = ginfer::bench::format_table(env, results);
            break;
        case ginfer::bench::OutputFormat::Json:
            report = ginfer::bench::format_json(env, command_line(argc, argv), results);
            break;
        case ginfer::bench::OutputFormat::Csv:
            report = ginfer::bench::format_csv(env, results);
            break;
        }
