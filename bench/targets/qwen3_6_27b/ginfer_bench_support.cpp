...
#ifdef GINFER_SOURCE_DIR
    const auto source_relative =
        (std::filesystem::path(GINFER_SOURCE_DIR) / path).lexically_normal();
...
SpeculativeStats aggregate_speculative(const TestResult& result) {
    ...
    for (const RepTiming& rep : result.reps) {
        ...
        out.verification_window =
            std::max(out.verification_window, in.verification_window);
        out.rounds += in.rounds;
        out.expanded_rounds += in.expanded_rounds;
...
void append_speculative_json(std::ostringstream& out, const SpeculativeStats& stats,
    ...
    out << indent << "\"speculative\": {\n"
        ...
        << indent << "  \"verification_window\": " << stats.verification_window << ",\n"
        << indent << "  \"rounds\": " << stats.rounds << ",\n"
        << indent << "  \"expanded_rounds\": " << stats.expanded_rounds << ",\n"
...
std::string usage_text(std::string_view program) {
    if (program.empty()) { program = "ginfer_bench"; }
    std::ostringstream out;
    out << "Usage: " << program << " --weights <artifact.ginfer> [options]\n\n"
        << "Product-route throughput benchmark over ginfer::Engine. pp measures Engine prefill;\n"
        << "tg measures G generated tokens after an untimed one-token seed prefill.\n\n"
        << "Options:\n"
        << "  --weights <path>            required .ginfer artifact\n"
...
std::string format_table(const BenchEnvironment& env, const std::vector<TestResult>& results) {
    std::ostringstream out;
    out << "ginfer_bench product throughput report\n"
...
std::string format_csv(const BenchEnvironment& env, const std::vector<TestResult>& results) {
    ...
    out << "label,kind,n_prompt,n_gen,target,weights_id,max_context,prefill_chunk,mtp_draft_tokens,"
           ...
           "spec_draft_window,spec_verification_window,spec_rounds,spec_expanded_rounds,"
           "spec_fallback_steps,spec_acceptance_rate,"
    ...
    for (const TestResult& result : results) {
        ...
        out << result.test.label << ',' << kind_string(result.test.kind) << ','
            ...
            << result.workspace_allocator_peak_bytes << ',' << spec.draft_window << ','
            << spec.verification_window << ',' << spec.rounds << ',' << spec.expanded_rounds << ','
