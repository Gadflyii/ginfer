...
`build/apps/ginfer-serve` loads one registered artifact and exposes OpenAI- and
Anthropic-compatible HTTP endpoints over one resident GInfer Engine.
...
```bash
./build/apps/ginfer-serve models/qwen3_6_35b_a3b.ginfer \
  --max-context 16384 \
  --spec mtp --draft-tokens 3 \
  --lm-head-draft
```

Qwen3.8 DFlash2 companion artifacts default to DFlash2 with four draft tokens:

```bash
./build/apps/ginfer-serve models/qwen3_8_27b_dflash2_q4.ginfer \
  --max-context 16384
```

The same command accepts `qwen3_8_27b_nvfp4_dflash2_q4.ginfer` for the registered mixed
NVFP4/FP8 backbone with the Q4 companion.

An explicit Qwen3.8 DFlash2 window must be in `1..7`. Use `--spec none` for autoregressive
Qwen3.8 serving. To serve image/video requests, select `--spec none --vision` or
`--spec mtp --draft-tokens N --vision`; DFlash2 and Vision are mutually exclusive.

Muse Glimmer is text-only and also defaults to DFlash2 with four draft tokens. Select another
explicit DFlash2 window at startup with:

```bash
./build/apps/ginfer-serve models/muse_glimmer_30b.ginfer \
  --max-context 16384 \
...
Qwen Vision is disabled by default: its weights, Vision scratch phase, and frozen
request-transient buffer are not allocated, and media requests and token-count requests fail with
HTTP 400 `vision_disabled`. Add `--vision` when a Qwen server must accept image or video input.
Muse rejects `--vision` and `--spec mtp`. Speculative residency is frozen at startup. Omitting
`--spec` means `--spec auto`: the four Qwen3.8 companion profiles and Muse select DFlash with four
draft tokens, while every other profile selects no speculative backend. `--spec none` explicitly
leaves MTP/DFlash weights and state nonresident. `--lm-head-draft` is the Qwen optimized proposal
head and requires an explicit MTP/DFlash selection. DFlash selects Qwen3.8 or Muse DFlash2, or
35B-A3B's text-only backend, and cannot be combined with Vision. A later request cannot enable a
capability omitted at startup.

Qwen3.8 and Muse native-Q4 DFlash2 are qualified through the public Engine route on `sm_120a` and
physical `sm_86`. Their other Ampere storage profiles and all `sm_89` DFlash2 routes remain
admitted for real-device qualification and tuning but are not measured support claims.
Qwen3.6-35B-A3B DFlash remains `sm_120a`-only.
...
- string content and ordered text parts; Qwen Vision also accepts `image_url` and `video_url`;
...
artifact template's existing tool/reasoning-instruction composition. Muse maps `developer` to its
system-class ATEM framing and accepts text parts only.

At startup, the loaded target frontend reports its prompt capabilities; they are never inferred
from the request's `model` field or public model label. Qwen derives them from the exact embedded
`frontend/chat_template.jinja` resource. The Qwen effort template exposes `low`, `medium`, and
`xhigh`; Muse's registered ATEM frontend exposes `low`, `medium`, `high`, and `xhigh`, with `high`
as its default, plus the native direct-response strength `instruct`. Protocol effort vocabularies
are normalized to those model-owned capabilities instead of rejecting a valid protocol label:

| Requested level | Qwen effort template | Qwen binary toggle | Muse Glimmer |
|---|---|---|---|
| `none` | direct response | direct response | `instruct` |
| `minimal` | `low` | thinking on, template default | `low` |
| `low` | `low` | thinking on, template default | `low` |
| `medium` | `medium` | thinking on, template default | `medium` |
| `high` | `xhigh` | thinking on, template default | `high` |
| `xhigh` | `xhigh` | thinking on, template default | `xhigh` |
| `max` | `xhigh` | thinking on, template default | `xhigh` |

For Chat Completions, `reasoning_effort` accepts `none`, `minimal`, `low`, `medium`, `high`, `xhigh`,
and `max` and uses the table above.
`enable_thinking` controls the same new-turn thinking switch; a contradictory combination with
`reasoning_effort` returns `conflicting_template_option`. On Muse, disabling thinking renders
`Reasoning strength: instruct.` in its ATEM system turn.

`preserve_thinking` controls whether reasoning from closed assistant turns remains in later
Qwen prompts. It defaults to the server setting, which is off unless `--preserve-thinking` is
used. Muse's ATEM renderer always emits supplied assistant reasoning as an assistant-to-self turn;
the preserve toggle does not alter its prompt. If both OpenAI spellings are present they must carry
the same boolean value. Unknown non-null `chat_template_kwargs` are rejected.
...
### Qwen multimodal request

Start a Qwen server with `--vision` before sending media. Muse has no media route:
...
GInfer implements the typed-Item and semantic-event core of the OpenAI
...
| Field | GInfer Responses Core contract |
|---|---|
...
| `reasoning.effort` | accepts `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, or `max` and maps it through the loaded frontend table above |
| `chat_template_kwargs.preserve_thinking` | optional boolean controlling closed-turn Qwen reasoning; it does not alter Muse ATEM prompts |
...
| `input_video` | GInfer extension with HTTP(S) or data-URI `video_url`; requires server `--vision` |
...
`instructions` is represented as a leading developer turn for the current request; Qwen and Muse
then apply their target-specific role lowering.
...
GInfer renders these definitions in the loaded target's Qwen or ATEM prompt and parses model output
into separate `function_call` output Items. Each output has a protocol Item `id` (`fc_...`) and a distinct
`call_id` (`call_...`). The client executes the function and sends a `function_call_output` Item in
a later request. GInfer does not execute functions or enforce JSON Schema through constrained
...
GInfer has no native RAG pipeline, document ingestion/index, vector store, reranker, hosted
`file_search`/`web_search`/`tool_search`, or server-side MCP execution. This is also true of the
Anthropic surface. Retrieval can be client-executed through an ordinary function tool: the client
runs retrieval and returns its result in the next inference turn. Unsupported hosted or retrieval
tool declarations fail explicitly; they are never accepted as no-op capabilities.

### Response object and usage

A terminal wire response has `object: "response"`, one of `completed`, `incomplete`, or
`cancelled` in `status`, and a typed `output` array. GInfer may emit:
...
`input_tokens` includes the chat template and, for Qwen media requests, expanded media tokens.
`cached_tokens` is the exact resident prompt prefix reused by Engine. `output_tokens` is the count
of accepted generated token IDs, including a withheld stop token when applicable.
`reasoning_tokens` is counted by the loaded target's output decoder while accepted tokens are still
in the reasoning channel; it is not estimated by re-tokenizing decoded text.
...
streams immediately; only an ambiguous `<tool_call>` / `<atem:function_calls>` suffix or the
structured tool region is held. Malformed tool markup is flushed back as ordinary text without
losing bytes.
...
`thinking.type: "disabled"` maps to the loaded frontend's direct-response mode: Qwen's native
thinking-off prompt or Muse's `instruct` strength. `enabled` and `adaptive` select thinking-on;
other values are rejected. The independent top-level `preserve_thinking` boolean controls
closed-turn Qwen history and otherwise uses the server default; Muse always emits supplied
assistant reasoning in its ATEM history.

Anthropic `output_config.effort` accepts the protocol values `low`, `medium`, `high`, `xhigh`, and
`max`, then uses the same positive-level mapping table above. Anthropic effort controls the whole
response independently of thinking, while GInfer's native strength controls reasoning only. When
both are supplied with `thinking.type: "disabled"`, disabled wins and the model runs its native
...
| `--vision` | enable Qwen media input and load Vision GPU allocations | off |
| `--no-cuda-graph` | disable CUDA Graph decode | graphs on |
| `--no-prefix-reuse` | disable compatible-prefix caching | prefix reuse on |
| `--no-thinking` | disable thinking when the loaded frontend supports the toggle | thinking on |
| `--preserve-thinking` | preserve closed-turn Qwen assistant reasoning by default | off |
...
Muse Glimmer uses `1.0/0.95/64/0/0` for its ATEM reasoning-strength modes. Frequency penalty is
`0` for all registered presets. Process flags override registered values, request fields override
process flags, and `--greedy` finally forces temperature `0`.

Run `./build/apps/ginfer-serve --help` for the exact option contract.
...
Every line is one `ginfer_serve_request_log` schema-v11 JSON object. All events carry
...
`verification_window`, `rounds`, `expanded_rounds`, `drafted_tokens`, `accepted_tokens`,
`fallback_steps`, and `accepted_per_position`. The two windows differ when Qwen3.8's confirmed
context-copy path expands target verification beyond its learned DFlash2 block. Rates can be
...
Muse autoregressive decode follows that CUDA Graph route. Its current DFlash2 round is uncaptured
and has no graph allowance; `--no-cuda-graph` therefore changes Muse AR only.
...
and is not divided evenly among request lanes. Qwen's registered native ceiling is 262,144 tokens;
Muse's is 131,072.
...
Compatible resident prefixes are reused unless the server is started with `--no-prefix-reuse`.
For Qwen multimodal histories, a hit requires matching token types, three-axis MRoPE positions,
encoded-media digest, grid, and consumer spans; changing an earlier image or video therefore resets
the prefix instead of reusing placeholder-token KV. Media wholly inside a matched prefix skips
Vision execution, while new suffix media is encoded normally. The completion log reports the
reused token count as `cache=`.

The Qwen family runtime distinguishes `full_reset`, `append_frontier`,
...
Muse uses only `full_reset` and `append_frontier`. Its lane planner compares the incoming prompt
token IDs with the completed lane's committed token ledger and retains the longest common prefix
when Main Text KV is still resident. A fully identical prompt backs the match up by one token so
Muse can recompute the final hidden activation needed to sample the first output token. A positive
retained count reports `append_frontier`; a zero count, disabled reuse, or incompatible lane reports
`full_reset`. Muse never reports the Qwen turn/response checkpoint paths. DFlash2 also requires its
2,048-token cyclic draft KV to contain the selected frontier: after that cache wraps, only an
appended suffix at the exact retained frontier can reuse it, while a rewind resets the lane. AR has
no such cyclic restriction. A terminal Muse DFlash2 round that publishes only a speculative prefix
remains retained for ownership and eviction accounting, but is marked semantically non-reusable;
the next request resets rather than treating that partial round as an exact append frontier.
...
or usage accounting. For Qwen MTP and DFlash, a stop-truncated multi-token round commits the exact
accepted target prefix and a following compatible turn can still reuse it. Muse DFlash2 has the
partial-terminal exception described above: its retained ownership is not semantic prefix
identity, so the following request takes `full_reset`. Output-limit and context-capacity finishes
...
responses. GInfer does not execute tools and does not enforce client JSON Schema through constrained
decoding.
