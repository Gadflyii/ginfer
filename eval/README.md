...
Qwen model or create a `.ginfer` artifact.
...
never reserve more slots than the target capacity. Set the local `ginfer-serve` target to one; set a
...
```bash
eval/.venv/bin/python -m ginfer_eval validate \
...
```bash
eval/.venv/bin/python -m ginfer_eval plan \
...
```bash
eval/.venv/bin/python -m ginfer_eval run \
...
SERPAPI_API_KEY=... eval/.venv/bin/python -m ginfer_eval run \
...
```bash
eval/.venv/bin/python -m ginfer_eval status --run eval/runs/<run-id>
eval/.venv/bin/python -m ginfer_eval resume --run eval/runs/<run-id>
eval/.venv/bin/python -m ginfer_eval summarize --run eval/runs/<run-id>
```
...
`ginfer_eval/backends/base.py`, registers one stable name in `backends/registry.py`, retains its raw
...
```bash
PYTHONPATH=eval eval/.venv/bin/python -m py_compile $(rg --files eval/ginfer_eval -g '*.py')
