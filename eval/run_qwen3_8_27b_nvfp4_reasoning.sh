...
server_bin="${repo_dir}/build/apps/ginfer-serve"
artifact="${repo_dir}/out/qwen3_8_27b_nvfp4.ginfer"
...
step_config() {
    case "$1" in
        text)
            # 262144 does not fit the RTX 5090: weights leave 11,245,380,608 B,
            # the minimum reservation grows as 1,811,051,776 + 32,816/token, and
            # 1 GiB headroom caps the context at ~254,769 tokens. 252,928 still
            # covers GPQA's 245,760 output budget plus the largest measured
            # 2,847-token prompt with 4,321 tokens of slack.
            printf 'text_full\t252928\t0\t%s\n' \
...
if [[ "${plan_only}" -eq 1 ]]; then
    for step in "${steps[@]}"; do
        ...
        PYTHONPATH="${repo_dir}/eval" "${eval_python}" -m ginfer_eval plan \
...
if [[ ! -x "${server_bin}" || ! -x "${eval_python}" ]]; then
    echo "ginfer-serve and the evaluation Python must be executable" >&2
    exit 1
fi
...
run_step() {
    ...
    server_log="${log_dir}/qwen3_8_27b_nvfp4-${step}-c2-${run_stamp}.server.log"
    request_log="${log_dir}/qwen3_8_27b_nvfp4-${step}-c2-${run_stamp}.requests.jsonl"
    ...
    for ((attempt = 1; attempt <= 120; ++attempt)); do
        ...
        if ! kill -0 "${server_pid}" 2>/dev/null; then
            wait "${server_pid}" || true
            echo "ginfer-serve exited before becoming ready; see ${server_log}" >&2
            exit 1
        fi
        sleep 1
    done
    if [[ "${ready}" -ne 1 ]]; then
        echo "ginfer-serve did not become ready within 120 seconds; see ${server_log}" >&2
        exit 1
    fi
    ...
    PYTHONPATH="${repo_dir}/eval" "${eval_python}" -m ginfer_eval run \
