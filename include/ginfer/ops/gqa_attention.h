 ...
 *
 *   score[x]      = scale * dot(q[:,h,j,b], K_cache[b][:,x,kvh]) for admitted x
 *   probability   = softmax_x(score)
 *   ideal[:,h,j,b] = sum_x probability[x] * V_cache[b][:,x,kvh].
 *
 * Admitted keys are x in [0,p] when window_size==0, otherwise
 * [max(0, p-window_size+1), p].
 *
 * The registered geometries are `[256,24|4,W,B]` group 6, `[256,16|2,W,B]` group 8, and
 * `[128,32|2,W,B]` group 16. D=256 uses scale 1/sqrt(256). D=128 takes `scale` as a runtime
 * float so the caller can fold a model qk_scale_factor (Muse Glimmer uses 3.87/sqrt(128)).
 ...
 * window_size==0 is full causal history [0,p]. window_size>0 admits keys x in
 * [max(0, p-window_size+1), p] for the query at absolute position p (Muse sliding layers use
 * 2048). D=256 callers must pass window_size 0.
 *
...
void gqa_attention(const Tensor& q, const Tensor& k, const Tensor& v, const Tensor& positions,
                   ...
                   cudaStream_t stream, std::int32_t window_size = 0);

/**
 * A2: perform only the cache-write part of A1. k/v are contiguous BF16 `[256,4|2,T]` or
 * `[128,2,T]`, positions is contiguous sequential I32 [T], and every addressed code and INT8
 * scale is overwritten. It reads no unrelated cache row, receives no execution envelope, and
 * owns no persistent frontier.
 */
 ...
 * mutating any cache plane. q/out are contiguous BF16 `[256,24|16,T]` or `[128,32,T]`, positions
 * is contiguous sequential I32 [T], and the mathematical formula, optional sliding window, and
 * execution-envelope contract are identical to A1. Caller workspace is reported by
