 ...
 * the destination cache lane. The registered ring capacity W is 2048 or 4096, and absolute
 * position p maps to physical slot p mod W. The caller guarantees that each row's live interval
 * ends immediately before positions[0,b] and that advancing it by counts[b] makes every
 * overwritten old slot dead. One row may commit at most W tokens, so no two live writes race for
 * one physical slot.
 */
void kv_cache_append_prefix(const Tensor& k, const Tensor& v, const Tensor& positions,
