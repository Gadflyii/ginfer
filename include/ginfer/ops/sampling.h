...
struct SamplingConfig {
    ...
    std::int32_t top_k         = 0;    // see sample() top_k contract (route-dependent cap)
...
// Invalid profiles or intervals throw; a legal single-block route returns zero, while the
// registered Muse top-64 hierarchy returns its caller-owned transient capacity.
 ...
 *
 * Candidates are sorted by adjusted_v descending with lower token id breaking ties. Route
 * selection is by token_domain and column geometry, not by reading device configs. The Muse route
 * (token_domain 202048) and single-block fallback honor top_k in [1,63] as that many candidates;
 * top_k<=0 or top_k>=64 keeps min(64, token_domain). The historical 20-cap remains for the Qwen
 * multiblock route: top_k in [1,19] keeps that many; top_k<=0 or top_k>=20 keeps min(20,
 * token_domain).
 *
