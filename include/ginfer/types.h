...
struct SpeculativeOptions {
    SpeculativeBackend backend = SpeculativeBackend::Automatic;
...
struct SpeculativeStats {
    ...
    bool enabled                        = false;
    // Backend-learned/configured proposal width and maximum target verification width.
    std::uint32_t draft_window          = 0;
    std::uint32_t verification_window   = 0;
    std::uint64_t rounds                = 0;
    std::uint64_t expanded_rounds       = 0;
