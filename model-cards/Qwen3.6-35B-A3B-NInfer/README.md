...
only for GInfer; it is not a Transformers checkpoint, Safetensors distribution, or GGUF file.
...
| Field | Value |
|---|---|
| Filename | `qwen3_6_35b_a3b.ginfer` |
| Size | 22,783,246,080 bytes (21.22 GiB) |
| SHA-256 | `1fb9ea0b5b8561e49d9604115ec89e5d9f2b6f6434e32c37c57fffd480a325d2` |
| Container version | 2 |
| GInfer model ID | `qwen3.6-35b-a3b` |
| GInfer weights ID | `groupwise-int` |
| GInfer target key | `qwen3_6_35b_a3b` |

The file contains the registered Text, Vision, MTP, proposal-head, DFlash, tokenizer,
chat-template, generation, and media-processor objects required by GInfer.
...
printf '%s  %s\n' \
  ...
  'qwen3_6_35b_a3b.ginfer' | sha256sum --check
```
...
- NVIDIA GeForce RTX 5090 or NVIDIA RTX PRO 6000 Blackwell (`sm_120a`); NVIDIA GeForce RTX 4090
  (`sm_89`) or RTX 3090 (`sm_86`) is groupwise-int only (24 GB envelope; no FP8, no NVFP4) and
  this 21.22 GiB artifact often will not start at product defaults;
...
submitted serially to a persistent `ginfer-serve` process with CUDA Graph enabled, a 1,024-token
...
These are single-sample results under the stated GInfer evaluation profile, not pass@k scores.

## Limits

- The artifact is accepted only by GInfer revision `bd265a3` or later and the matching registered
  target.
- GInfer executes on one RTX 5090, RTX PRO 6000 Blackwell, RTX 4090, or RTX 3090 (groupwise-int,
  24 GB envelope) and one CUDA device, with a startup-fixed capacity of 1–8 active
  requests per Engine. On 24 GB the 35B-A3B artifact often will not start.
...
This GInfer artifact is distributed under the Apache License 2.0. The source
