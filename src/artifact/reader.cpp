...
struct Reader::Impl {
    explicit Impl(const std::filesystem::path& path) : file(path) {
        ...
        if (std::equal(kV1Magic.begin(), kV1Magic.end(), file.data())) {
            throw ArtifactError("GInfer artifact v1 is no longer supported; replace it with the "
                                "published version-2 artifact");
        }
        if (!std::equal(kMagic.begin(), kMagic.end(), file.data())) {
            throw ArtifactError("artifact magic is not GInfer v2");
        }
