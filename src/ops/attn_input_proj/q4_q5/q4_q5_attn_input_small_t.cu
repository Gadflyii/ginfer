...
using Q4AttnSimtR4C8Schedule = Q4RowSplitSimtGemmSchedule<4, 8, 16, 2, Cache::ca, 1>;

template <int Cols>
using Q4AttnExactSchedule =
    Q4RowSplitSimtGemmSchedule<(Cols <= 4 ? 8 : 4), Cols, 16, 2, Cache::ca, 1>;
...
template <int Cols>
void launch_q4_exact(const Tensor& x, const Weight& weight, Tensor& q, Tensor& key,
                     cudaStream_t stream) {
    using Schedule = Q4AttnExactSchedule<Cols>;
    launch_q4_simt<Schedule, true>(x, weight, q, key, stream);
}
...
void launch_q4(const Tensor& x, const Weight& weight, Tensor& q, Tensor& key, cudaStream_t stream) {
    ...
    case 1:
        ...
        return;
    case 2:
        launch_q4_exact<2>(x, weight, q, key, stream);
        return;
    case 3:
        launch_q4_exact<3>(x, weight, q, key, stream);
        return;
    case 4:
        launch_q4_exact<4>(x, weight, q, key, stream);
        return;
    case 5:
        launch_q4_exact<5>(x, weight, q, key, stream);
        return;
    case 6:
        launch_q4_exact<6>(x, weight, q, key, stream);
        return;
    case 7:
        launch_q4_exact<7>(x, weight, q, key, stream);
        return;
    case 8:
        launch_q4_exact<8>(x, weight, q, key, stream);
        return;
    case 9:
    case 10:
    case 11:
    ...
    case 15:
        ...
        return;
    case 16:
        launch_q4_simt_route<Q4AttnSimtR4C8Schedule>(x, weight, q, key, stream);
        return;
    default:
...
void launch_q5_split4_exact(const Tensor& x, const Weight& weight, Tensor& gate, Tensor& value,
    ...
    case 8:
        ...
        return;
    default:
        throw std::invalid_argument("attention Q5 split4 requires T in [2,8]");
    }
}
