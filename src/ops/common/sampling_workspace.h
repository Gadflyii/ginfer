...
inline constexpr int kSamplerWideCandidates       = 64;
inline constexpr int kSamplerWideTokenDomain      = 202048;
inline constexpr int kSamplerWideItemsPerThread   = 4;
inline constexpr int kSamplerWidePartialTileItems = kSamplerBlock * kSamplerWideItemsPerThread;
inline constexpr int kSamplerWidePartialsPerGroup = 16;
inline constexpr int kSamplerWideMergeTileItems   =
    kSamplerGroupBlock * kSamplerWideItemsPerThread;
...
__host__ __device__ inline int sampler_wide_partial_count(int vocab) {
    return div_up(vocab, kSamplerWidePartialTileItems);
}

__host__ __device__ inline int sampler_wide_group_count(int partial_blocks) {
    return div_up(partial_blocks, kSamplerWidePartialsPerGroup);
}

// Ordinary Muse sampling has a dedicated top-64 hierarchy. Configs are device-resident, so the
// fixed token-domain route must be able to represent the largest supported top_k for every row.
__host__ __device__ inline bool sampler_wide_multiblock_ok(int vocab, int columns) {
    if (vocab != kSamplerWideTokenDomain || columns <= 0 || columns > kSamplerMaxColumns) {
        return false;
    }
    const int partial_blocks = sampler_wide_partial_count(vocab);
    const int groups         = sampler_wide_group_count(partial_blocks);
    return partial_blocks > 0 && groups > 0 &&
...
// The Qwen/speculative multi-block route is deliberately finite. A single final merge tile must
// hold every group candidate and the registered routes use at most sixteen columns. Muse's token
// domain is excluded because it requires the top-64 hierarchy above.
__host__ __device__ inline bool sampler_multiblock_ok(int vocab, int cols, int partial_blocks,
                                                      int group_count) {
    return vocab > kSamplerTileItems && vocab != kSamplerWideTokenDomain && cols > 0 &&
           cols <= kSamplerMaxColumns && partial_blocks > 0 && group_count > 0 &&
...
struct WideSamplingWorkspace {
    unsigned long long* partial_keys = nullptr;
    std::int32_t* group_done         = nullptr;
    std::int32_t partial_stride      = 0;
};

struct WideSamplingWorkspaceLayout {
    TensorRegion partial_keys;
    TensorRegion group_done;
    std::size_t bytes           = 0;
    std::int32_t partial_stride = 0;
    ...
    [[nodiscard]] WideSamplingWorkspace bind(DeviceSpan backing) const {
        if (!multiblock) { return {}; }
        return {
            static_cast<unsigned long long*>(partial_keys.bind(backing).data),
            static_cast<std::int32_t*>(group_done.bind(backing).data),
...
inline WideSamplingWorkspaceLayout
make_wide_sampling_workspace_layout(std::int32_t token_domain, std::int32_t columns) {
    WideSamplingWorkspaceLayout out;
    if (!sampler_wide_multiblock_ok(token_domain, columns)) { return out; }
    ...
    out.partial_keys =
        layout.add_tensor(DType::I64, {kSamplerWideCandidates, out.partial_stride, columns}, 256,
                          "Muse sampling partial keys");
    out.group_done =
        layout.add_tensor(DType::I32, {columns}, 256, "Muse sampling group counters");
    out.bytes = layout.finish(256, "Muse sampling workspace");
    return out;
}
