...
using Q4GdnSimtR4C8Schedule = Q4RowSplitSimtGemmSchedule<4, 8, 16, 2, Cache::ca, 1>;

template <int Cols>
using Q4GdnExactSchedule =
...
void launch_q4_exact(const Tensor& x, const Weight& weight, Tensor& out, cudaStream_t stream) {
    using Schedule = Q4GdnExactSchedule<Cols>;
    launch_q4_simt<Schedule, true>(x, weight, out, stream);
}
...
void launch_q4(const Tensor& x, const Weight& weight, Tensor& out, cudaStream_t stream) {
    ...
    if (x.ne[1] <= 16) {
        launch_q4_simt_route<Q4GdnSimtR4C8Schedule>(x, weight, out, stream);
        return;
    }
