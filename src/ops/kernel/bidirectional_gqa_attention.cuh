...
inline constexpr int kSwaMaximumWindow        = 4096;
...
bidirectional_gqa_stage_tile(__nv_bfloat16* dst, const __nv_bfloat16* context,
                             ...
                             int kv_head, int context_stride, int physical_page, int cyclic_window,
                             int tid) {
    ...
    for (int chunk = tid; chunk < KeyBlock * VecsPerRow; chunk += Threads) {
        ...
        if constexpr (CyclicSwa) {
            const int context_position = (live ? key0 + row : 0) & (cyclic_window - 1);
...
__device__ __forceinline__ void noncausal_gqa_split_partial_body(
    ...
    int max_context, int cyclic_window, int split_capacity, float scale,
    ...
    if constexpr (CyclicSwa) {
        if (cyclic_window != 2048 && cyclic_window != kSwaMaximumWindow) { return; }
    }
...
#pragma unroll
    ...
    bidirectional_gqa_stage_tile<CyclicSwa, KeyBlock, Threads>(
        ...
        context_stride, current_page, cyclic_window, tid);
    cp_commit();
...
#pragma unroll
        ...
        if (iteration + 1 < iterations) {
            ...
            bidirectional_gqa_stage_tile<CyclicSwa, KeyBlock, Threads>(
                ...
                context_stride, next_page, cyclic_window, tid);
            cp_commit();
        }
        ...
        for (int nt = 0; nt < QKNt; ++nt) {
            ...
            const bool allow00 =
                ...
                (!CyclicSwa || current_is_query ||
                 current_key0 + col0 >= q_position0 - (cyclic_window - 1));
            const bool allow01 =
                row0_live && col1 < current_valid &&
                (!CyclicSwa || current_is_query ||
                 current_key0 + col1 >= q_position0 - (cyclic_window - 1));
            const bool allow10 =
                row1_live && col0 < current_valid &&
                (!CyclicSwa || current_is_query ||
                 current_key0 + col0 >= q_position1 - (cyclic_window - 1));
            const bool allow11 =
                row1_live && col1 < current_valid &&
                (!CyclicSwa || current_is_query ||
                 current_key0 + col1 >= q_position1 - (cyclic_window - 1));
...
__launch_bounds__(WarpsPerCta * 32, 2) __global__ void swa_split_partial_kernel(
    ...
    const __nv_bfloat16* __restrict__ context_k, const __nv_bfloat16* __restrict__ context_v,
    int padded_context, int max_context, int cyclic_window, int split_capacity, float scale,
    __nv_bfloat16* __restrict__ partial_acc, float* __restrict__ partial_m,
    float* __restrict__ partial_l, __nv_bfloat16* __restrict__ out) {
    noncausal_gqa_split_partial_body<true, Tokens, WarpsPerCta, KeyBlock, DirectOutput>(
        ...
        padded_context, 0, max_context, cyclic_window, split_capacity, scale, partial_acc, partial_m,
        partial_l, out);
}
...
noncausal_gqa_reduce_body(const __nv_bfloat16* __restrict__ partial_acc,
                          ...
                          int cyclic_window, int split_capacity, __nv_bfloat16* __restrict__ out) {
