...
// ginfer::ops - split-KV GQA small-T attention, int8 KV-cache partial kernel.
...
__launch_bounds__(WarpsPerCta * 32, MinBlocksPerSm) __global__
    void gqa_attention_decode_i8_tiled_kernel(
        ...
        std::int32_t eight_k_split_max, __nv_bfloat16* partial_acc, float* partial_m,
        float* partial_l) {
    ...
    const int active_split_count = gqa_small_t_active_splits<Geometry, true>(
        window, split_count, TokenTile, eight_k_split_max);
    ...
    auto issue_kv_tile = [&](int tile_k0, int physical_page) {
        for (int key_l = tid; key_l < Bc; key_l += Threads) {
            ...
            if (key >= split_start && key < split_end) {
                ...
                ginfer::ops::cp_async<8>(&k_scale_s[key_l * Groups], &cache_k_scale[off]);
                ginfer::ops::cp_async<8>(&v_scale_s[key_l * Groups], &cache_v_scale[off]);
            } else {
...
#pragma unroll 1
        for (int chunk = tid; chunk < Bc * (D / 16); chunk += Threads) {
            ...
            if (key >= split_start && key < split_end) {
                ...
                ginfer::ops::cp_async<16>(dst, &cache_k_i8[off]);
                ginfer::ops::cp_async<16>(&v_i8[key_l * D + d], &cache_v_i8[off]);
            } else {
                std::int8_t* dst = &k_i8[key_l * D + gqa_small_t_tc_swz(key_l, dc * 8) * 2];
...
#pragma unroll
        ...
        if (has_next) { ginfer::ops::cp_wait<0>(); }
        __syncthreads();
    }
