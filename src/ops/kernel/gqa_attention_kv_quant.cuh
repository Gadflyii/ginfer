...
// ginfer::ops - signed int8, per-token group-wise KV cache codec (shared device
...
__device__ __forceinline__ std::int64_t gqa_kv_quant_scale_index(int physical_page, int kv_head,
    ...
    return paged_kv_element_offset<Geometry::QuantGroups, Geometry::KVHeads>(physical_page, kv_head,
                                                                            page_offset, group);
}
