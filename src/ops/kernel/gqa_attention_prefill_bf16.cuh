...
__global__ void gqa_attention_prefill_fill_bf16_kernel(
    ...
    const std::int64_t n   = static_cast<std::int64_t>(tokens) * Geometry::KVHeads * (D / VecElems);
    if (idx >= n) { return; }
...
#pragma unroll
    ...
    for (int kb = 0; kb < n_block_max; ++kb) {
        ...
        ginfer::ops::cp_wait<0>(); // K(kb) landed (also publishes q_s / prev PV done)
        __syncthreads();
...
#pragma unroll
        ...
        ginfer::ops::cp_wait<0>(); // V(kb) landed; QK done reading k_s
        __syncthreads();
