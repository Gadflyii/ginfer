...
__device__ __forceinline__ int gqa_sliding_key_lo(int qabs, int window_size) {
    if (window_size <= 0) { return 0; }
    const int lo = qabs - window_size + 1;
    return lo > 0 ? lo : 0;
}

__device__ __forceinline__ bool gqa_key_admitted(int key, int qabs, int window_size) {
    return key >= gqa_sliding_key_lo(qabs, window_size) && key <= qabs;
}
