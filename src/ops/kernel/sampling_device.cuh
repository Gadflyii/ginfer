...
// Shared implementation primitives for include/ginfer/ops/sampling.h and
// include/ginfer/ops/speculative_round.h. The ordering key is exact for finite BF16
// logits (including numeric-zero ties). The Qwen multiblock route stores a top-20
// set; the Muse/single-block route stores a top-64 set. Global staging is supplied
// by the caller.
...
using SamplingWideSort =
    cub::BlockRadixSort<unsigned long long, kSamplerBlock, kSamplerWideItemsPerThread>;
...
__device__ __forceinline__ int sampling_candidate_cap(const SamplingConfig& cfg,
                                                      std::int32_t vocab, int pipeline_cap) {
    // top_k is clamped to the route's pipeline cap: top_k <= 0 (no explicit
    // limit) or a top_k larger than the cap both select the full pipeline set.
...
__device__ __forceinline__ int sampling_candidate_cap(const SamplingConfig& cfg,
                                                      std::int32_t vocab) {
    return sampling_candidate_cap(cfg, vocab, kSamplerCandidateCap);
}
...
sampling_wide_partial_offset(const WideSamplingWorkspace& workspace, int col, int partial, int j) {
    return ((col * workspace.partial_stride + partial) * kSamplerWideCandidates) + j;
}
...
sampling_build_truncated_small(const __nv_bfloat16* logits, std::int64_t base, std::int32_t vocab,
                               ...
                               int pipeline_cap = kSamplerCandidateCap) {
    const int tid = threadIdx.x;
    const int cap = sampling_candidate_cap(cfg, vocab, pipeline_cap);
...
// Single-block large-column fallback with a 64-candidate pipeline cap. Each thread keeps a local
// top-kWide set; warps stage 32 lists into shared and thread 0 merges. Ordinary Muse sampling uses
// its hierarchical route; DFlash support construction and unsupported column geometries retain
// this exact fallback.
template <int kWideCandidates>
__device__ inline void sampling_build_truncated_block_wide(
    const __nv_bfloat16* logits, std::int64_t base, std::int32_t vocab, const SamplingConfig& cfg,
    float* stage_val, int* stage_idx, float* cand_val, int* cand_idx, float* prob, int* n_support,
    const std::int32_t* overlay = nullptr, int overlay_len = 0) {
    static_assert(kWideCandidates > 0, "wide sampler cap must be positive");
    const int tid = threadIdx.x;
    const int cap = sampling_candidate_cap(cfg, vocab, kWideCandidates);

    float local_val[kWideCandidates];
    int local_idx[kWideCandidates];
#pragma unroll
    for (int j = 0; j < kWideCandidates; ++j) {
        local_val[j] = -CUDART_INF_F;
        local_idx[j] = INT_MAX;
    }
    ...
    for (int w = 0; w < nwarps; ++w) {
        if (warp == w) {
            const int off = lane * kWideCandidates;
            for (int j = 0; j < cap; ++j) {
                stage_val[off + j] = local_val[j];
                stage_idx[off + j] = local_idx[j];
            }
        }
        __syncthreads();
        if (tid == 0) {
            for (int p = 0; p < 32; ++p) {
                const int off = p * kWideCandidates;
                for (int j = 0; j < cap; ++j) {
                    const int idx = stage_idx[off + j];
                    if (idx == INT_MAX) { continue; }
                    sampling_insert_candidate(cand_val, cand_idx, cap, stage_val[off + j], idx);
                }
            }
        }
        __syncthreads();
