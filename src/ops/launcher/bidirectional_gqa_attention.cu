...
void dispatch_tokens(std::int32_t tokens, Launch&& launch) {
    switch (tokens) {
#define GINFER_BIDIRECTIONAL_GQA_TOKEN_CASE(TOKENS)                                                \
    case TOKENS:                                                                                   \
        dispatch_token_case<TOKENS>(launch);                                                       \
        return
        GINFER_BIDIRECTIONAL_GQA_TOKEN_CASE(1);
        GINFER_BIDIRECTIONAL_GQA_TOKEN_CASE(2);
        GINFER_BIDIRECTIONAL_GQA_TOKEN_CASE(3);
        GINFER_BIDIRECTIONAL_GQA_TOKEN_CASE(4);
        GINFER_BIDIRECTIONAL_GQA_TOKEN_CASE(5);
        GINFER_BIDIRECTIONAL_GQA_TOKEN_CASE(6);
        GINFER_BIDIRECTIONAL_GQA_TOKEN_CASE(7);
        GINFER_BIDIRECTIONAL_GQA_TOKEN_CASE(8);
        GINFER_BIDIRECTIONAL_GQA_TOKEN_CASE(9);
        GINFER_BIDIRECTIONAL_GQA_TOKEN_CASE(10);
        GINFER_BIDIRECTIONAL_GQA_TOKEN_CASE(11);
        GINFER_BIDIRECTIONAL_GQA_TOKEN_CASE(12);
        GINFER_BIDIRECTIONAL_GQA_TOKEN_CASE(13);
        GINFER_BIDIRECTIONAL_GQA_TOKEN_CASE(14);
        GINFER_BIDIRECTIONAL_GQA_TOKEN_CASE(15);
        GINFER_BIDIRECTIONAL_GQA_TOKEN_CASE(16);
#undef GINFER_BIDIRECTIONAL_GQA_TOKEN_CASE
    default:
...
BidirectionalGqaPlan bidirectional_gqa_resolve_plan(std::int32_t tokens,
    ...
    const std::int32_t occupancy_max = bidirectional_gqa_runtime_max_split(occupancy);
    std::int32_t splits              = 1;
    if (!direct) {
        const std::int32_t tiles = std::max(1, static_cast<std::int32_t>(envelope_tiles));
        splits                   = std::min(occupancy_max, std::min(split_limit, tiles));
    }
    return {
...
void bidirectional_gqa_attention_launch(const Tensor& q, const Tensor& query_k,
    ...
    dispatch_tokens(q.ne[2], [&]<int Tokens, int Warps>() {
        ...
        if (plan.warps != Warps || plan.split_capacity < 1 ||
            plan.split_capacity > bidirectional_gqa_runtime_max_split(occupancy)) {
