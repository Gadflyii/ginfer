// ginfer::ops - split-KV GQA small-T launcher and unified route dispatcher.
#include "ops/launcher/gqa_attention.h"

#include "ops/common/math.h"
#include "ops/kernel/gqa_attention_decode.cuh"
#include "ops/kernel/gqa_attention_decode_bf16.cuh"
#include "ops/kernel/gqa_attention_decode_d128.cuh"
...
std::int32_t gqa_small_t_split_upper_bound(std::int32_t window, std::int32_t decode_splits) {
    if (window <= 0) { return decode_splits; }
    ...
    return (splits < decode_splits) ? splits : decode_splits;
}

template <typename Geometry>
std::int32_t gqa_small_t_split_count(std::int32_t window, std::int32_t tokens, DType kv_dtype,
                                     const OccupancyPolicy& occupancy) {
    const std::int32_t decode_splits =
        gqa_runtime_decode_splits(occupancy, Geometry::DecodeSplitScale);
    ...
    // Bc=64 is one CTA/SM. Keep the 8K grid at or below one persistent wave after
    // accounting for the geometry's KV-head count: floor(sm_count/4)*scale.
...
void launch_tc_partial_i8(const Tensor& q, CacheInput input, const Tensor& pos, float scale,
                          ...
                          Tensor& partial_l, const OccupancyPolicy& occupancy,
                          cudaStream_t stream) {
    Tensor& cache_k       = cache.k_pages;
    Tensor& cache_v       = cache.v_pages;
    Tensor& cache_k_scale = cache.k_scale_pages;
    Tensor& cache_v_scale = cache.v_scale_pages;
    const std::int32_t eight_k_split_max =
        gqa_eight_k_split_max(occupancy.sm_count, Geometry::DecodeSplitScale);
    auto launch = [&]<int WarpsPerCta, int MinBlocksPerSm, int KeyBlock, bool DynamicArena>() {
        ...
        gqa_attention_decode_i8_tiled_kernel<Geometry, TokenTile, WarpsPerCta, MinBlocksPerSm,
            ...
            <<<grid, WarpsPerCta * 32, kDynamicBytes, stream>>>(
                ...
                logical_capacity, scale, eight_k_split_max,
...
std::int32_t gqa_attention_split_capacity(std::int32_t q_heads, std::int32_t tokens,
    ...
    if (q_heads == Gqa27Geometry::QHeads) {
        return gqa_small_t_launch_capacity<Gqa27Geometry>(envelope, tokens, cache_dtype, occupancy);
    }
    if (q_heads == Gqa35Geometry::QHeads) {
        return gqa_small_t_launch_capacity<Gqa35Geometry>(envelope, tokens, cache_dtype, occupancy);
    }
...
void launch_d128_partial(const Tensor& q, CacheInput input, const Tensor& pos, float scale,
                         std::int32_t window_size, PagedKVBatchLayerView cache,
                         const GqaSmallTInvocation& invocation, std::int32_t logical_capacity,
                         std::int32_t splits, Tensor& partial_acc, Tensor& partial_m,
                         Tensor& partial_l, cudaStream_t stream) {
    constexpr int kBlock = 32 * WarpsPerCta;
    const dim3 grid(Geometry::KVHeads, splits, invocation.batch_size);
    gqa_attention_d128_partial_kernel<Geometry, TokenTile, WarpsPerCta, MultiBatch, Masked, Int8,
                                      CacheInput>
        <<<grid, kBlock, 0, stream>>>(
            static_cast<const __nv_bfloat16*>(q.data), input,
            static_cast<const std::int32_t*>(pos.data),
            cache.dtype == DType::BF16 ? static_cast<__nv_bfloat16*>(cache.k_pages.data) : nullptr,
            cache.dtype == DType::BF16 ? static_cast<__nv_bfloat16*>(cache.v_pages.data) : nullptr,
            cache.dtype == DType::I8 ? static_cast<std::int8_t*>(cache.k_pages.data) : nullptr,
            cache.dtype == DType::I8 ? static_cast<std::int8_t*>(cache.v_pages.data) : nullptr,
            cache.dtype == DType::I8 ? static_cast<__half*>(cache.k_scale_pages.data) : nullptr,
            cache.dtype == DType::I8 ? static_cast<__half*>(cache.v_scale_pages.data) : nullptr,
            static_cast<const std::int32_t*>(cache.block_tables.data),
            invocation.valid_columns == nullptr
                ? nullptr
                : static_cast<const std::int32_t*>(invocation.valid_columns->data),
            ...
            invocation.column_begin, logical_capacity, scale, window_size,
...
void gqa_attention_small_t_launch_for(const Tensor& q, CacheInput input, const Tensor& pos,
    ...
    const auto splits = gqa_small_t_launch_capacity<Geometry>(envelope, invocation.width,
                                                              cache.dtype, occupancy);

    const auto dispatch_profile = [&](auto&& launch_body) {
        const bool masked = invocation.valid_columns != nullptr;
        if (invocation.batch_size == 1) {
            if (masked) {
                launch_body.template operator()<false, true>();
            } else {
                launch_body.template operator()<false, false>();
            }
        } else if (masked) {
            launch_body.template operator()<true, true>();
        } else {
            launch_body.template operator()<true, false>();
        }
    };

    if constexpr (Geometry::HeadDim == 128) {
#define GINFER_GQA_D128_DISPATCH(TOKENS, WARPS)                                                    \
    do {                                                                                           \
        const auto launch_profile = [&]<bool MultiBatch, bool Masked>() {                          \
            if (cache.dtype == DType::I8) {                                                        \
                launch_d128_partial<Geometry, (TOKENS), (WARPS), MultiBatch, Masked, true>(        \
                    q, input, pos, scale, window_size, cache, invocation, logical_capacity,        \
                    splits, partial_acc, partial_m, partial_l, stream);                            \
            } else {                                                                               \
                launch_d128_partial<Geometry, (TOKENS), (WARPS), MultiBatch, Masked, false>(       \
                    q, input, pos, scale, window_size, cache, invocation, logical_capacity,        \
                    splits, partial_acc, partial_m, partial_l, stream);                            \
            }                                                                                      \
        };                                                                                         \
        dispatch_profile(launch_profile);                                                          \
    } while (0)
        switch (invocation.width) {
        case 1:
            GINFER_GQA_D128_DISPATCH(1, 2);
            break;
        case 2:
            GINFER_GQA_D128_DISPATCH(2, 2);
            break;
        case 3:
            GINFER_GQA_D128_DISPATCH(3, 3);
            break;
        case 4:
            GINFER_GQA_D128_DISPATCH(4, 4);
            break;
        case 5:
            GINFER_GQA_D128_DISPATCH(5, 5);
            break;
        case 6:
            GINFER_GQA_D128_DISPATCH(6, 6);
            break;
        default:
            throw std::invalid_argument("gqa_attention_small_t_launch: unsupported T");
        }
#undef GINFER_GQA_D128_DISPATCH
    } else {
        (void)window_size;
        // BF16 keeps its row-tile warp count; INT8 selects its producer/consumer
        // geometry inside launch_tc_partial_i8.
#define GINFER_GQA_SMALL_T_DISPATCH(TOKENS, WARPS)                                                 \
    do {                                                                                           \
        const auto launch_profile = [&]<bool MultiBatch, bool Masked>() {                          \
            if (cache.dtype == DType::I8) {                                                        \
                launch_tc_partial_i8<Geometry, (TOKENS), MultiBatch, Masked>(                      \
                    q, input, pos, scale, cache, invocation, logical_capacity,                     \
                    implementation_window, splits, partial_acc, partial_m, partial_l, occupancy,   \
                    stream);                                                                       \
            } else {                                                                               \
        ...
        switch (invocation.width) {
        case 1:
            GINFER_GQA_SMALL_T_DISPATCH(1, 2);
            break;
        case 2:
            GINFER_GQA_SMALL_T_DISPATCH(2, 4);
            break;
        case 3:
            GINFER_GQA_SMALL_T_DISPATCH(3, 4);
            break;
        case 4:
            GINFER_GQA_SMALL_T_DISPATCH(4, 4);
            break;
        case 5:
            GINFER_GQA_SMALL_T_DISPATCH(5, 4);
            break;
        case 6:
            GINFER_GQA_SMALL_T_DISPATCH(6, 4);
            break;
        default:
            throw std::invalid_argument("gqa_attention_small_t_launch: unsupported T");
        }
#undef GINFER_GQA_SMALL_T_DISPATCH
    }
    ...
    const auto launch_reduce = [&]<bool Int8, bool MultiBatch, bool Masked, bool Offset>() {
        gqa_attention_small_t_reduce_output_kernel<Geometry, kDChunk, Int8, MultiBatch, Masked,
            ...
            <<<reduce_grid, kReduceBlock, 0, stream>>>(
                ...
                invocation.batch_size, splits, eight_k_split_max,
...
void gqa_attention_small_t_launch(const Tensor& q, const Tensor& k, const Tensor& v,
                                  ...
                                  Tensor& out, const OccupancyPolicy& occupancy, cudaStream_t stream,
                                  std::int32_t window_size) {
    ...
    if (q.ne[1] == GqaMuseGeometry::QHeads) {
        gqa_attention_small_t_launch_for<GqaMuseGeometry>(
            q, input, pos, scale, cache, invocation, envelope, partial_acc, partial_m, partial_l,
            out, occupancy, stream, window_size);
        return;
    }
...
void gqa_attention_cached_small_t_launch(const Tensor& q, const Tensor& pos, float scale,
    ...
    if (q.ne[1] == GqaMuseGeometry::QHeads) {
        gqa_attention_small_t_launch_for<GqaMuseGeometry>(
            ...
            partial_l, out, occupancy, stream, window_size);
        return;
    }
    if (q.ne[1] == Gqa27Geometry::QHeads) {
