// ginfer::ops - rope launcher: private token-count tuning and generic fallback.
...
template <RopeKernelMode Mode, int QHeads, int KHeads>
void launch_fixed(const Tensor& positions, Tensor* q, Tensor* k, const OccupancyPolicy& occupancy,
                  cudaStream_t stream) {
    const int tokens = positions.ne[0];
    ...
    if constexpr (kTextMode<Mode>) {
        ...
        } else if (tokens <= occupancy.rope_large_block_wave) {
            block = kLargeBlock;
...
bool launch_fixed_pair(const Tensor& positions, int rotary_dim, float theta, Tensor& q, Tensor& k,
    ...
    if (rotary_dim == 64 && theta == 1.0e7F) {
        if (q.ne[1] == 24 && k.ne[1] == 4) {
            if (axes == 1) {
                launch_fixed<RopeKernelMode::Text1D, 24, 4>(positions, &q, &k, occupancy, stream);
                return true;
            }
            if (axes == 3) {
                launch_fixed<RopeKernelMode::TextMrope, 24, 4>(positions, &q, &k, occupancy,
                                                               stream);
                return true;
            }
        }
        if (q.ne[1] == 16 && k.ne[1] == 2) {
            if (axes == 1) {
                launch_fixed<RopeKernelMode::Text1D, 16, 2>(positions, &q, &k, occupancy, stream);
                return true;
            }
            if (axes == 3) {
                launch_fixed<RopeKernelMode::TextMrope, 16, 2>(positions, &q, &k, occupancy,
                                                               stream);
                return true;
            }
        }
    ...
    if (axes == 2 && rotary_dim == 72 && theta == 10'000.0F && q.ne[1] == 16 && k.ne[1] == 16) {
        launch_fixed<RopeKernelMode::Vision2D, 16, 16>(positions, &q, &k, occupancy, stream);
        return true;
    }
    return false;
}
...
void launch_fixed_single(const Tensor& positions, Tensor& x, const OccupancyPolicy& occupancy,
                         cudaStream_t stream) {
    launch_fixed<Mode, Heads, 0>(positions, &x, nullptr, occupancy, stream);
}
...
bool launch_text_single(const Tensor& positions, int axes, Tensor& x,
    ...
    if (axes == 1) {
        launch_fixed_single<RopeKernelMode::Text1D, Heads>(positions, x, occupancy, stream);
        return true;
    }
    if (axes == 3) {
        launch_fixed_single<RopeKernelMode::TextMrope, Heads>(positions, x, occupancy, stream);
        return true;
    }
    return false;
}
...
bool launch_fixed_single_dispatch(const Tensor& positions, int rotary_dim, float theta, Tensor& x,
    ...
    if (axes == 1 && x.ne[0] == 128 && rotary_dim == 128 && theta == 1.0e7F) {
        if (x.ne[1] == 32) {
            launch_fixed_single<RopeKernelMode::DflashText1D, 32>(positions, x, occupancy, stream);
            return true;
        }
        if (x.ne[1] == 8) {
            launch_fixed_single<RopeKernelMode::DflashText1D, 8>(positions, x, occupancy, stream);
            return true;
        }
    }
    if (rotary_dim == 64 && theta == 1.0e7F) {
        if (launch_text_single<24>(positions, axes, x, occupancy, stream) ||
            launch_text_single<4>(positions, axes, x, occupancy, stream) ||
            launch_text_single<16>(positions, axes, x, occupancy, stream) ||
            launch_text_single<2>(positions, axes, x, occupancy, stream)) {
            return true;
        }
    }
    if (axes == 2 && rotary_dim == 72 && theta == 10'000.0F && x.ne[1] == 16) {
        launch_fixed_single<RopeKernelMode::Vision2D, 16>(positions, x, occupancy, stream);
        return true;
    }
    return false;
}
...
void rope_launch(const Tensor& positions, int rotary_dim, float theta, Tensor& q, Tensor& k,
                 const OccupancyPolicy& occupancy, cudaStream_t stream) {
    if (!launch_fixed_pair(positions, rotary_dim, theta, q, k, occupancy, stream)) {
...
void rope_single_launch(const Tensor& positions, int rotary_dim, float theta, Tensor& x,
                        const OccupancyPolicy& occupancy, cudaStream_t stream) {
    if (!launch_fixed_single_dispatch(positions, rotary_dim, theta, x, occupancy, stream)) {
