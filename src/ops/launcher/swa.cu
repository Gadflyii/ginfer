...
void dispatch_tokens(std::int32_t tokens, Launch&& launch) {
    switch (tokens) {
#define GINFER_SWA_TOKEN_CASE(TOKENS)                                                              \
    case TOKENS:                                                                                   \
        dispatch_token_case<TOKENS>(launch);                                                       \
        return
        GINFER_SWA_TOKEN_CASE(1);
        GINFER_SWA_TOKEN_CASE(2);
        GINFER_SWA_TOKEN_CASE(3);
        GINFER_SWA_TOKEN_CASE(4);
        GINFER_SWA_TOKEN_CASE(5);
        GINFER_SWA_TOKEN_CASE(6);
        GINFER_SWA_TOKEN_CASE(7);
        GINFER_SWA_TOKEN_CASE(8);
        GINFER_SWA_TOKEN_CASE(9);
        GINFER_SWA_TOKEN_CASE(10);
        GINFER_SWA_TOKEN_CASE(11);
        GINFER_SWA_TOKEN_CASE(12);
        GINFER_SWA_TOKEN_CASE(13);
        GINFER_SWA_TOKEN_CASE(14);
        GINFER_SWA_TOKEN_CASE(15);
        GINFER_SWA_TOKEN_CASE(16);
#undef GINFER_SWA_TOKEN_CASE
    default:
...
void swa_launch(const Tensor& q, const Tensor& query_k, const Tensor& query_v,
    ...
    dispatch_tokens(q.ne[2], [&]<int Tokens, int Warps>() {
        ...
        if (context.capacity != static_cast<std::uint32_t>(plan.window)) {
            throw std::invalid_argument("swa: plan window does not match cyclic context");
        }
