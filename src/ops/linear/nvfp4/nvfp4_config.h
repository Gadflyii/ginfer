...
// Muse Glimmer 30B transformer-block projections.  These remain separate physical
// weights because the source checkpoint assigns independent weight-global divisors
// even when projections consume the same activation.
using Nvfp4MuseQueryGateGeometry = Nvfp4GemvGeometry<4096, 6656>;
using Nvfp4MuseKeyValueGeometry  = Nvfp4GemvGeometry<256, 6656>;
using Nvfp4MuseAttnOutGeometry   = Nvfp4GemvGeometry<6656, 4096>;
using Nvfp4MuseMlpInGeometry     = Nvfp4GemvGeometry<19968, 6656>;
using Nvfp4MuseMlpOutGeometry    = Nvfp4GemvGeometry<6656, 19968>;
...
using Nvfp4Activation6656Geometry  = Nvfp4ActivationGeometry<6656>;
using Nvfp4Activation19968Geometry = Nvfp4ActivationGeometry<19968>;
...
inline constexpr bool is_nvfp4_linear_problem(std::int32_t output_rows, std::int32_t input_rows) {
    return (output_rows == Nvfp4AttnInputGeometry::kOutputRows &&
           ...
           (output_rows == Nvfp4MuseQueryGateGeometry::kOutputRows &&
            input_rows == Nvfp4MuseQueryGateGeometry::kInputRows) ||
           (output_rows == Nvfp4MuseKeyValueGeometry::kOutputRows &&
            input_rows == Nvfp4MuseKeyValueGeometry::kInputRows) ||
           (output_rows == Nvfp4MuseAttnOutGeometry::kOutputRows &&
            input_rows == Nvfp4MuseAttnOutGeometry::kInputRows) ||
           (output_rows == Nvfp4MuseMlpInGeometry::kOutputRows &&
            input_rows == Nvfp4MuseMlpInGeometry::kInputRows) ||
           (output_rows == Nvfp4MuseMlpOutGeometry::kOutputRows &&
            input_rows == Nvfp4MuseMlpOutGeometry::kInputRows);
}
...
inline Nvfp4Problem resolve_nvfp4_problem(std::int32_t output_rows, std::int32_t input_rows) {
    ...
    if (output_rows == Nvfp4MuseQueryGateGeometry::kOutputRows &&
        input_rows == Nvfp4MuseQueryGateGeometry::kInputRows) {
        return Nvfp4Problem::MuseQueryGate;
    }
    if (output_rows == Nvfp4MuseKeyValueGeometry::kOutputRows &&
        input_rows == Nvfp4MuseKeyValueGeometry::kInputRows) {
        return Nvfp4Problem::MuseKeyValue;
    }
    if (output_rows == Nvfp4MuseAttnOutGeometry::kOutputRows &&
        input_rows == Nvfp4MuseAttnOutGeometry::kInputRows) {
        return Nvfp4Problem::MuseAttnOut;
    }
    if (output_rows == Nvfp4MuseMlpInGeometry::kOutputRows &&
        input_rows == Nvfp4MuseMlpInGeometry::kInputRows) {
        return Nvfp4Problem::MuseMlpIn;
    }
    if (output_rows == Nvfp4MuseMlpOutGeometry::kOutputRows &&
        input_rows == Nvfp4MuseMlpOutGeometry::kInputRows) {
        return Nvfp4Problem::MuseMlpOut;
    }
