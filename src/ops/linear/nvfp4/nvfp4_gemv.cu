...
void launch_nvfp4_decode(const Tensor& x, const Weight& weight, Tensor& out, cudaStream_t stream) {
    ...
    case Nvfp4Problem::MuseQueryGate:
        launch_exact<Nvfp4MuseQueryGateGeometry>(x, weight, out, stream);
        return;
    case Nvfp4Problem::MuseKeyValue:
        launch_exact<Nvfp4MuseKeyValueGeometry>(x, weight, out, stream);
        return;
    case Nvfp4Problem::MuseAttnOut:
        launch_exact<Nvfp4MuseAttnOutGeometry>(x, weight, out, stream);
        return;
    case Nvfp4Problem::MuseMlpIn:
        launch_exact<Nvfp4MuseMlpInGeometry>(x, weight, out, stream);
        return;
    case Nvfp4Problem::MuseMlpOut:
        launch_exact<Nvfp4MuseMlpOutGeometry>(x, weight, out, stream);
        return;
    }
}
