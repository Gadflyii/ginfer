...
void launch_nvfp4_small_t(const Tensor& x, const Weight& weight, Tensor& out, cudaStream_t stream) {
    ...
    case Nvfp4Problem::MuseQueryGate:
        launchers<Nvfp4MuseQueryGateGeometry>()[index](x, weight, out, stream);
        return;
    case Nvfp4Problem::MuseKeyValue:
        launchers<Nvfp4MuseKeyValueGeometry>()[index](x, weight, out, stream);
        return;
    case Nvfp4Problem::MuseAttnOut:
        launchers<Nvfp4MuseAttnOutGeometry>()[index](x, weight, out, stream);
        return;
    case Nvfp4Problem::MuseMlpIn:
        launchers<Nvfp4MuseMlpInGeometry>()[index](x, weight, out, stream);
        return;
    case Nvfp4Problem::MuseMlpOut:
        launchers<Nvfp4MuseMlpOutGeometry>()[index](x, weight, out, stream);
        return;
    }
}
