...
using M32N32Pipelined             = Nvfp4W4a4MmaSchedule<32, 32, 256, 2, 2, 3, 2>;
using M32N64                      = Nvfp4W4a4MmaSchedule<32, 64, 256, 2, 4, 2, 2>;
using M32N64Pipelined             = Nvfp4W4a4MmaSchedule<32, 64, 256, 2, 4, 3, 1>;
...
void launch_problem(const Weight& weight, Tensor& out, Nvfp4W4a4Workspace workspace,
    ...
    if (tokens >= 1024 && (tokens % kTmaBlockM) == 0) {
        ...
        if constexpr (std::is_same_v<Geometry, Nvfp4MuseMlpOutGeometry>) {
            launch_gemm<Geometry, M32N32Pipelined>(weight, out, workspace, tokens, stream);
        } else if constexpr (std::is_same_v<Geometry, Nvfp4MuseMlpInGeometry>) {
            if (tokens > 32) {
                launch_gemm<Geometry, M32N128>(weight, out, workspace, tokens, stream);
            } else {
                launch_gemm<Geometry, M32N64Pipelined>(weight, out, workspace, tokens, stream);
            }
        } else {
...
void launch_nvfp4_w4a4_quantize(const Tensor& x, const Weight& weight, Nvfp4W4a4Workspace workspace,
    ...
    case Nvfp4Activation4096Geometry::kInputRows:
        launch_quantize_exact<Nvfp4Activation4096Geometry>(x, weight, workspace, stream);
        return;
    case Nvfp4Activation6656Geometry::kInputRows:
        launch_quantize_exact<Nvfp4Activation6656Geometry>(x, weight, workspace, stream);
        return;
    case Nvfp4Activation19968Geometry::kInputRows:
        launch_quantize_exact<Nvfp4Activation19968Geometry>(x, weight, workspace, stream);
        return;
    default:
...
void launch_nvfp4_w4a4(const Tensor& x, const Weight& weight, Tensor& out,
    ...
    case Nvfp4Problem::Residual17408:
        ...
        return;
    case Nvfp4Problem::MuseQueryGate:
        launch_problem<Nvfp4MuseQueryGateGeometry>(weight, out, workspace, tokens, stream);
        return;
    case Nvfp4Problem::MuseKeyValue:
        launch_problem<Nvfp4MuseKeyValueGeometry>(weight, out, workspace, tokens, stream);
        return;
    case Nvfp4Problem::MuseAttnOut:
        launch_problem<Nvfp4MuseAttnOutGeometry>(weight, out, workspace, tokens, stream);
        return;
    case Nvfp4Problem::MuseMlpIn:
        launch_problem<Nvfp4MuseMlpInGeometry>(weight, out, workspace, tokens, stream);
        return;
    case Nvfp4Problem::MuseMlpOut:
        launch_problem<Nvfp4MuseMlpOutGeometry>(weight, out, workspace, tokens, stream);
        return;
    }
}
