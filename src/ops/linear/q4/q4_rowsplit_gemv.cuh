...
using Q4GemvR1W8DirectSchedule =
    Q4RowSplitGemvSchedule<1, 8, 16, 1, Q4GemvActivationAccess::Direct,
                           ...
                           Cache::ca, 80, 1>;
// Muse Glimmer K=6656/19968 cannot use the K=5120 static R1W8 byte path (104 groups, 52
// scale-pairs, 52 % 8 != 0). Fallback T=1 path: 1x8 CTA, cg streaming tiles, 4-stage
// async, integer dequant. Prefer q4_gemv_warp_split_kernel when K divides evenly.
using Q4GemvR1W8AsyncDirectSchedule =
    Q4RowSplitGemvSchedule<1, 8, 16, 4, Q4GemvActivationAccess::Direct,
                           Q4GemvLaneMapping::PackedWord8, Q4GemvDecodeMode::Fp16Mantissa,
                           Q4GemvCodeTransfer::AsyncVector16, Q4GemvScaleAccess::SharedPair32,
                           Cache::cg, 0, 6>;
...
#pragma unroll
    for (int group_base = 0; group_base < Schedule::kGroupsPerWarpTile; group_base += 4) {
        ...
        if (local_group < active_groups) {
            ...
            const uint4 activation_bits = load_ldg<uint4>(activation + k_begin);
            accumulator =
                Q4SimtDecodeAtom::fma_eight(packed, scale_bits, activation_bits, accumulator);
        }
    }
    return accumulator;
}
...
template <int WarpsPerRow, int Stages, int VecsPerStage>
__device__ __forceinline__ void q4_issue_code_tile(
    uint4 (&shared_codes)[WarpsPerRow][Stages][VecsPerStage], const std::uint8_t* code_row,
    int warp_in_row, int stage, int group_begin, int remaining, int lane) {
    constexpr int kCodeBytes   = Q4RowSplitStorage::kCodeBytesPerGroup;
    constexpr int kStageGroups = VecsPerStage / 2;
    const int active           = min(kStageGroups, remaining);
    const int active_vecs      = active * 2;
    const auto* global_codes   = reinterpret_cast<const uint4*>(
        code_row + static_cast<std::int64_t>(group_begin) * kCodeBytes);
    if (lane < VecsPerStage) {
        if (lane < active_vecs) {
            cp_async<16, Cache::cg>(&shared_codes[warp_in_row][stage][lane], &global_codes[lane]);
        } else {
            shared_codes[warp_in_row][stage][lane] = make_uint4(0u, 0u, 0u, 0u);
        }
    }
    cp_commit();
}
...
template <int WarpsPerRow, int Stages, int VecsPerStage>
__device__ __forceinline__ float q4_consume_word_tile(
    const uint4 (&shared_codes)[WarpsPerRow][Stages][VecsPerStage], const __nv_bfloat16* x,
    const std::uint8_t* scale_row, int warp_in_row, int stage, int tile_group_begin, int active,
    int lane, float acc) {
    constexpr int kGroupK      = Q4RowSplitStorage::kGroupK;
    constexpr int kStageGroups = VecsPerStage / 2;
    const auto* packed_words   = reinterpret_cast<const std::uint32_t*>(shared_codes[warp_in_row][stage]);
    const int lane_group       = lane >> 3;
    const int lane_in_group    = lane & 7;
#pragma unroll
    for (int group_base = 0; group_base < kStageGroups; group_base += 4) {
        const int local_group = group_base + lane_group;
        if (local_group < active) {
            const std::uint32_t packed     = packed_words[group_base * 8 + lane];
            const std::uint16_t scale_bits = load_ldg<std::uint16_t>(
                scale_row + static_cast<std::int64_t>(tile_group_begin + local_group) * 2);
            const int k_begin = (tile_group_begin + local_group) * kGroupK + lane_in_group * 8;
            const uint4 activation_bits = load_ldg<uint4>(x + k_begin);
            acc = Q4SimtDecodeAtom::fma_eight(packed, scale_bits, activation_bits, acc);
        }
    }
    return acc;
}

// T=1 FFN GEMV: one row per CTA, K split evenly across warps.
template <int GroupsPerRow, int WarpsPerRow>
__global__ __launch_bounds__(WarpsPerRow * 32, 6) void q4_gemv_warp_split_kernel(
    const __nv_bfloat16* __restrict__ x, const std::uint8_t* __restrict__ codes,
    const std::uint8_t* __restrict__ scales, __nv_bfloat16* __restrict__ out) {
    static_assert(WarpsPerRow > 0 && WarpsPerRow <= 32);
    static_assert(GroupsPerRow > 0 && (GroupsPerRow % WarpsPerRow) == 0);
    constexpr int kGroupsPerWarp = GroupsPerRow / WarpsPerRow;
    constexpr int kCodeBytes     = Q4RowSplitStorage::kCodeBytesPerGroup;
    constexpr int kStageGroups   = 16;
    constexpr int kTileCount     = (kGroupsPerWarp + kStageGroups - 1) / kStageGroups;
    constexpr int kStages        = kTileCount >= 3 ? 4 : (kTileCount >= 2 ? 2 : 1);
    constexpr int kPrefetch      = kStages - 1;
    constexpr int kVecsPerStage  = kStageGroups * kCodeBytes / static_cast<int>(sizeof(uint4));
    static_assert(kVecsPerStage == 32);

    __align__(16) __shared__ uint4 shared_codes[WarpsPerRow][kStages][kVecsPerStage];
    __shared__ float row_partials[WarpsPerRow];

    const int lane        = static_cast<int>(threadIdx.x) & 31;
    const int warp_in_row = static_cast<int>(threadIdx.x) >> 5;
    const int row         = static_cast<int>(blockIdx.x);
    const int group0      = warp_in_row * kGroupsPerWarp;

    const std::uint8_t* code_row =
        codes + static_cast<std::int64_t>(row) * GroupsPerRow * kCodeBytes;
    const std::uint8_t* scale_row =
        scales +
        static_cast<std::int64_t>(row) * GroupsPerRow * Q4RowSplitStorage::kScaleBytesPerGroup;

#pragma unroll
    for (int prefetch = 0; prefetch < kPrefetch; ++prefetch) {
        if (prefetch < kTileCount) {
            q4_issue_code_tile<WarpsPerRow, kStages, kVecsPerStage>(
                shared_codes, code_row, warp_in_row, prefetch, group0 + prefetch * kStageGroups,
                kGroupsPerWarp - prefetch * kStageGroups, lane);
        } else {
            cp_commit();
        }
    }
    if constexpr (kPrefetch == 0) {
        q4_issue_code_tile<WarpsPerRow, kStages, kVecsPerStage>(shared_codes, code_row, warp_in_row,
                                                               0, group0, kGroupsPerWarp, lane);
    }
...
#pragma unroll 4
    for (int tile = 0; tile < kTileCount; ++tile) {
        if constexpr (kPrefetch > 0) {
            ...
            if (fetch < kTileCount) {
                q4_issue_code_tile<WarpsPerRow, kStages, kVecsPerStage>(
                    shared_codes, code_row, warp_in_row, fetch % kStages,
                    group0 + fetch * kStageGroups, kGroupsPerWarp - fetch * kStageGroups, lane);
            } else {
                cp_commit();
            }
        }
        if constexpr (kPrefetch == 0) {
            cp_wait<0>();
        } else if constexpr (kPrefetch == 1) {
            cp_wait<1>();
        } else if constexpr (kPrefetch == 3) {
            cp_wait<3>();
        }
        __syncwarp();

        const int tile_group_begin = group0 + tile * kStageGroups;
        const int active           = min(kStageGroups, kGroupsPerWarp - tile * kStageGroups);
        acc = q4_consume_word_tile<WarpsPerRow, kStages, kVecsPerStage>(
            shared_codes, x, scale_row, warp_in_row, tile % kStages, tile_group_begin, active, lane,
            acc);
        __syncwarp();
    }

    acc = warp_reduce_sum(acc);
    if (lane == 0) { row_partials[warp_in_row] = acc; }
    __syncthreads();
    if (threadIdx.x == 0) {
        float row_accumulator = 0.0f;
#pragma unroll
        for (int warp = 0; warp < WarpsPerRow; ++warp) {
            row_accumulator += row_partials[warp];
        }
