...
#define GINFER_GATED_DELTA_NET_PROPAGATE(expr)                                                     \
    do {                                                                                           \
        const cudaError_t ginfer_gated_delta_net_error = (expr);                                   \
        if (ginfer_gated_delta_net_error != cudaSuccess) { return ginfer_gated_delta_net_error; }  \
    } while (0)
