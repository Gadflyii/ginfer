...
namespace ginfer::ops::detail::gated_delta_net::chunked {
namespace {
...
cudaError_t launch_output(const chunk_output_config& cfg) {
    ...
    GINFER_GATED_DELTA_NET_PROPAGATE(v.check_shape());
    GINFER_GATED_DELTA_NET_PROPAGATE(v.check_full_chunks());
    ...
    // Keep at most one resident wave of 4 CTAs/SM and distribute chunks evenly
    // across it. Small grids retain one logical job per CTA.
    if (cfg.target_ctas <= 0) { return cudaErrorInvalidValue; }
    const std::int64_t target_ctas    = cfg.target_ctas;
    ...
    GINFER_GATED_DELTA_NET_PROPAGATE(v.check_grid(grid_chunks, cfg.H_v));
