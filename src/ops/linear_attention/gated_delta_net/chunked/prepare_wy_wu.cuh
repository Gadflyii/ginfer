...
namespace ginfer::ops::detail::gated_delta_net::chunked::prepare_wy_wu {

using ginfer::ops::mma_tf32;
using ginfer::ops::Cache;
using ginfer::ops::cp_async;
using ginfer::ops::cp_commit;
using ginfer::ops::cp_wait;
using ginfer::ops::ldmatrix_x2;
using ginfer::ops::ldmatrix_x4;
using ginfer::ops::mma_bf16;
using ginfer::ops::smem_addr;
...
#pragma unroll
    for (int v = lane; v < STORE_PER_WARP; v += ginfer::ops::kWarpSize) {
...
prepare_wy_wu_kernel(const __nv_bfloat16* __restrict__ k_in, const __nv_bfloat16* __restrict__ v_in,
    ...
    constexpr int BLOCK_THREADS = BLOCK_WARPS * ginfer::ops::kWarpSize;
    ...
    if (warp == 0) {
        ...
        float partial = a + bv;
#pragma unroll
        for (int o = 1; o < ginfer::ops::kWarpSize; o <<= 1) {
...
#pragma unroll
        ...
        if constexpr (BLOCK_WARPS == 8) {
            if (kp == 0 && warp >= WY_WARPS) {
                constexpr int HELPER_THREADS = (BLOCK_WARPS - WY_WARPS) * ginfer::ops::kWarpSize;
                constexpr int VECS_PER_ROW   = WU_PANEL_COLS / 8;
                constexpr int N_VECS         = BT * VECS_PER_ROW;
                const int helper_tid         = tid - WY_WARPS * ginfer::ops::kWarpSize;
