...
#    include <cuda_bf16.h>
#    define GINFER_KERNELS_HOST_DEVICE __host__ __device__
#else
#    define GINFER_KERNELS_HOST_DEVICE
#endif

namespace ginfer::ops::detail::gated_delta_net {

inline uint3 init_fastdiv_values(std::uint64_t d64) {
    if (d64 == 0 || d64 > static_cast<std::uint64_t>(0xffffffffu)) {
        std::fprintf(stderr,
                     "ginfer::ops::detail::gated_delta_net::init_fastdiv_values: "
                     "invalid divisor %llu\n",
...
struct head_map {
    ...
    GINFER_KERNELS_HOST_DEVICE int group_size() const { return H_v / H_qk; }

    GINFER_KERNELS_HOST_DEVICE int qk_head(int h_v) const {
...
#undef GINFER_KERNELS_HOST_DEVICE
