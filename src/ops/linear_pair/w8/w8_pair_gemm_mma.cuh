...
#pragma unroll 1
        for (int item = tid; item < BM * (BK / 16); item += THREADS) {
            ...
            if constexpr (Full) {
                ...
                ginfer::ops::cp_async_zfill<16>(dst, &codes[gi * 32 + chunk * 16],
                                                grow < m ? 16 : 0);
            }
        }
    };
            ...
            for (int row = tid; row < BM; row += THREADS) {
                ...
                if constexpr (Full) {
                    ...
                    if (g0 + 8 <= kg) {
                        ...
                        ginfer::ops::cp_async_zfill<16>(dst, &scales[gi * 2], max(0, kg - g0) * 2);
                    }
                } else {
                    ...
                    ginfer::ops::cp_async_zfill<16>(dst, &scales[gi * 2], valid_scales * 2);
                }
            }
        }
    };
