...
struct Q4SwiGluWideRows {
    static constexpr int kOutputRowsPerCta = RowsPerCta / 2;

    __device__ __forceinline__ int weight_row(int output_row0, int local_row) const {
        const int row_block = local_row >> 4;
        const int row       = row_block * 8 + (local_row & 7);
        return output_row0 + row + ((local_row & 8) != 0 ? kIntermediate : 0);
    }
...
void launch_small_t_active(const Tensor& x, const Weight& w, Tensor& out, cudaStream_t stream) {
...
#if defined(GINFER_COMPILED_CC) && GINFER_COMPILED_CC < 120
    using Q4SwiGluSchedule = Q4SmallTMmaSchedule<32, 4>;
    using Q4SwiGluRows = Q4SwiGluWideRows<Q4SwiGluSchedule::kRowsPerCta>;
#else
    using Q4SwiGluSchedule = Q4DraftSmallTSchedule;
    using Q4SwiGluRows     = Q4SwiGluSmallTRows;
#endif
    constexpr int kBlocks = kIntermediate / Q4SwiGluRows::kOutputRowsPerCta;
    ...
    q4_small_t_mma_kernel<Q4SwiGluSmallTGeometry, TileCols, ActiveCols, Q4SwiGluSmallTEpilogue,
                          Q4SwiGluRows, Q4SwiGluSchedule>
        <<<kBlocks, Q4SwiGluSchedule::kThreads, 0, stream>>>(
            static_cast<const __nv_bfloat16*>(x.data), static_cast<const std::uint8_t*>(w.qdata),
            static_cast<const std::uint8_t*>(w.scales), static_cast<__nv_bfloat16*>(out.data),
            epilogue, Q4SwiGluRows{});
