// ginfer::ops - GQA A1/A2/A3 validation and finite route dispatch.
...
constexpr float kExpectedScale256                    = 0.0625f;
...
struct GqaRegisteredShape {
    std::int32_t q_heads  = 0;
    std::int32_t kv_heads = 0;
    std::int32_t head_dim = 0;
};

GqaRegisteredShape shape_for_q_heads(std::int32_t q_heads, const char* op) {
    if (q_heads == 24) { return {24, 4, 256}; }
    if (q_heads == 16) { return {16, 2, 256}; }
    if (q_heads == 32) { return {32, 2, 128}; }
...
std::int32_t kv_heads_for_q_heads(std::int32_t q_heads, const char* op) {
    return shape_for_q_heads(q_heads, op).kv_heads;
}

std::int32_t head_dim_for_q_heads(std::int32_t q_heads, const char* op) {
    return shape_for_q_heads(q_heads, op).head_dim;
}

void require_kv_geometry(std::int32_t kv_heads, std::int32_t head_dim, const char* op) {
    const bool ok = (head_dim == 256 && (kv_heads == 4 || kv_heads == 2)) ||
                    (head_dim == 128 && kv_heads == 2);
    if (!ok) {
...
void require_scale(float scale, std::int32_t head_dim, const char* op) {
    if (!std::isfinite(scale) || scale <= 0.0f) {
        throw std::invalid_argument(std::string(op) + ": scale must be a finite positive value");
    }
    if (head_dim == 256 && std::abs(scale - kExpectedScale256) > 1.0e-6f) {
        throw std::invalid_argument(std::string(op) + ": scale must be 1/sqrt(256)");
    }
}

void require_window(std::int32_t window_size, std::int32_t head_dim, const char* op) {
    if (window_size < 0) {
        throw std::invalid_argument(std::string(op) + ": window_size must be non-negative");
    }
    if (window_size > 0 && head_dim != 128) {
        throw std::invalid_argument(std::string(op) +
                                    ": sliding window requires the D=128 Muse geometry");
    }
}
...
std::uint32_t validate_cache(const PagedKVLayerView& cache, std::int32_t kv_heads,
                             std::int32_t head_dim, const char* op) {
    require_kv_geometry(kv_heads, head_dim, op);
    if ((cache.dtype != DType::BF16 && cache.dtype != DType::I8) ||
        cache.num_kv_heads != kv_heads || cache.head_dim != head_dim) {
...
void validate_envelope(GqaExecutionEnvelope envelope, const PagedKVLayerView& cache,
    ...
    const std::uint32_t capacity =
        validate_cache(cache, cache.num_kv_heads, cache.head_dim, op);
...
void validate_attention_tensors(const Tensor& q, const Tensor& positions, const Tensor& out,
                                ...
                                float scale, std::int32_t window_size, const char* op) {
    ...
    const std::int32_t head_dim = head_dim_for_q_heads(q_heads, op);
    const std::int32_t tokens   = q.ne[2];
    require_scale(scale, head_dim, op);
    require_window(window_size, head_dim, op);
...
void validate_batched_attention_tensors(const Tensor& q, const Tensor& positions,
                                        ...
                                        std::int32_t window_size, const char* op) {
    if (q.dtype != DType::BF16 || out.dtype != DType::BF16) {
        throw std::invalid_argument(std::string(op) + ": q/out must be BF16");
    }
    ...
    const std::uint32_t capacity = validate_batch_cache(cache, kv_heads, head_dim, op);
...
void for_each_small_t_chunk(const Tensor& q, const Tensor& positions, WorkspaceArena& workspace,
                            ...
                            const OccupancyPolicy& occupancy, Tensor& out, Launch&& launch) {
    const std::int32_t head_dim = head_dim_for_q_heads(q.ne[1], "gqa_attention");
    for (std::int32_t begin = 0; begin < q.ne[2]; begin += kSmallTChunkTokens) {
        ...
        const std::int32_t splits = detail::gqa_attention_split_capacity(
            q.ne[1], count, cache_dtype, envelope, occupancy);
        SmallTWorkspace partial =
            allocate_small_t_workspace(workspace, head_dim, q.ne[1], count, splits);
...
void launch_chunked_small_t(const Tensor& q, const Tensor& k, const Tensor& v,
    ...
    for (std::int32_t begin = 0; begin < q.ne[2]; begin += kSmallTChunkTokens) {
        ...
        auto chunk_scope         = workspace.scope();
        const std::int32_t splits = detail::gqa_attention_split_capacity(
            q.ne[1], count, cache.dtype, envelope, occupancy);
        SmallTWorkspace partial =
            allocate_small_t_workspace(workspace, head_dim, q.ne[1], count, splits, q.ne[3]);
        detail::gqa_attention_small_t_launch(q, k, v, positions, valid_columns, table_rows, scale,
                                             ...
                                             partial.l, out, occupancy, stream, window_size);
    }
}
...
void launch_cached_chunked_small_t(const Tensor& q, const Tensor& positions, float scale,
    ...
    for_each_small_t_chunk(
        q, positions, workspace, cache.dtype, envelope, occupancy, out,
        [&](std::int32_t, std::int32_t, const Tensor& q_chunk, const Tensor& position_chunk,
            ...
            detail::gqa_attention_cached_small_t_launch(q_chunk, position_chunk, scale, cache,
                                                        ...
                                                        out_chunk, occupancy, stream, window_size);
        });
}
...
std::size_t gqa_attention_workspace_capacity_bytes(std::int32_t q_heads, DType cache_dtype,
    ...
    const std::int32_t head_dim = head_dim_for_q_heads(q_heads, "gqa_attention workspace");
    const auto chunk_capacity   = [&](std::int32_t width) {
        const std::int32_t splits = detail::gqa_attention_split_capacity(
            q_heads, width, cache_dtype, envelope, occupancy);
...
void gqa_attention(const Tensor& q, const Tensor& k, const Tensor& v, const Tensor& positions,
                   ...
                   WorkspaceArena& workspace, Tensor& out, const OccupancyPolicy& occupancy,
                   cudaStream_t stream, std::int32_t window_size) {
    ...
    const std::int32_t head_dim = head_dim_for_q_heads(q.ne[1], op);
...
void gqa_kv_append(const Tensor& k, const Tensor& v, const Tensor& positions,
    ...
    require_contiguous_nonnull(v, op, "v");
    require_contiguous_nonnull(positions, op, "positions");
    const std::uint32_t capacity = validate_cache(cache, kv_heads, head_dim, op);
...
void gqa_attention_cached(const Tensor& q, const Tensor& positions, float scale,
    ...
    validate_attention_tensors(q, positions, out, cache, envelope, scale, window_size, op);
    ...
    if (detail::gqa_attention_uses_small_t(q.ne[2])) {
        const std::int32_t splits = detail::gqa_attention_split_capacity(
            q.ne[1], q.ne[2], cache.dtype, envelope, occupancy);
        SmallTWorkspace partial =
            allocate_small_t_workspace(workspace, head_dim, q.ne[1], q.ne[2], splits);
