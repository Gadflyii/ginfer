...
constexpr bool registered_cyclic_capacity(std::uint32_t capacity) {
    return capacity == 2048 || capacity == 4096;
}
...
void validate_cyclic_cache(const CyclicKVCacheLayerView& cache,
    ...
    if (cache.num_kv_heads != kKVHeads || cache.head_dim != kHeadDim ||
        !registered_cyclic_capacity(cache.capacity) || cache.padded_capacity < cache.capacity ||
