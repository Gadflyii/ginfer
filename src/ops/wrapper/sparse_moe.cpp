...
void sparse_moe(const Tensor& x, const SparseMoeWeights& weights, SparseMoeEpilogue epilogue,
                Tensor& destination, WorkspaceArena& workspace, const OccupancyPolicy& occupancy,
                cudaStream_t stream) {
    ...
    if (use_prefill) {
        ...
        detail::sparse_moe_prefill_launch(x, weights, destination, plan, views, occupancy, stream);
        return;
    }
    if (use_small_t) {
