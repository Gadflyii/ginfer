...
[[nodiscard]] inline SpeculativeBackend parse_speculative_backend(std::string_view value) {
    if (value == "auto") { return SpeculativeBackend::Automatic; }
    if (value == "none") { return SpeculativeBackend::None; }
    ...
    case SpeculativeBackend::Automatic:
        if (options.draft_tokens != 0 || options.proposal_head != ProposalHead::Full) {
            throw std::invalid_argument(
                "automatic speculative selection does not accept explicit draft/head options");
        }
        return;
    case SpeculativeBackend::None:
        if (options.draft_tokens != 0 || options.proposal_head != ProposalHead::Full) {
