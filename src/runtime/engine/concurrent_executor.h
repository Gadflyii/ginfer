...
public:
    ...
    using PublishedOutput = std::remove_cvref_t<decltype(std::declval<OutputSession>().commit_preview())>;
    ...
    Submission submit(PreparedPrompt prompt, PromptSummary prompt_summary,
...
private:
    ...
    struct Request {
        Request(std::uint64_t request_identity, PreparedPrompt input, OutputSession output_session,
