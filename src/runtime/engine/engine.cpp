...
public:
    ...
    std::variant<targets::qwen3_6::PreparedPrompt,
                 targets::muse_glimmer_30b::Package::PreparedPrompt>
        value;
};
...
public:
    ...
    using ExecutorMuse = runtime::ConcurrentExecutor<targets::MuseGlimmer30BInstance>;
    using Executor     = std::variant<std::monostate, std::unique_ptr<Executor27>,
                                  std::unique_ptr<Executor35>, std::unique_ptr<ExecutorMuse>>;
        ...
        executor          = std::visit(
            [&](auto& target_ptr) -> Executor {
                ...
                if constexpr (std::is_same_v<Instance, targets::Qwen3_6_27BInstance>) {
                    ...
                    return std::make_unique<ExecutorMuse>(*target_ptr, options);
                }
            },
            active);
    }
    ...
    return std::visit(
        [&](auto& executor) -> GenerationHandle {
            ...
            if constexpr (std::is_same_v<Executor, std::monostate>) {
                ...
                using PromptT = typename std::remove_cvref_t<decltype(*executor)>::PreparedPrompt;
                if (!std::holds_alternative<PromptT>(prompt.impl_->value)) {
                    throw std::logic_error("prepared prompt does not match the active target");
                }
                auto submission = executor->submit(std::get<PromptT>(std::move(prompt.impl_->value)),
