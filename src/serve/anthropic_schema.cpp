...
ginfer::product::media_acquire::Source parse_image_source(const Json& block) {
    ...
    ginfer::product::media_acquire::Source out;
    if (type == "base64") {
        ...
        out.kind  = ginfer::product::media_acquire::SourceKind::Data;
        ...
        out.kind = ginfer::product::media_acquire::SourceKind::Url;
    } else {
...
void parse_thinking(const Json& body, GenerationRequest& out) {
    ...
    if (type == "disabled") {
        ...
        out.reasoning_effort       = RequestedReasoningEffort::None;
        out.reasoning_effort_param = "thinking.type";
    } else if (type == "enabled" || type == "adaptive") {
        out.enable_thinking = true;
    } else {
        bad_request("thinking.type must be one of disabled, enabled, or adaptive",
                    "thinking.type");
    }
...
GenerationRequest parse_messages_request(const Json& body, const RequestLimits& limits) {
    ...
    // Anthropic effort applies independently of thinking. GInfer has no separate whole-response
    // effort control, so an explicit disabled mode wins over output_config.effort and normalizes
    // to the target's native non-reasoning mode.
...
const char* messages_stop_reason(ginfer::FinishReason reason, bool has_tool_calls) {
