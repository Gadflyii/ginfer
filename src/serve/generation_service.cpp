...
ApiError request_error_to_api_error(const ginfer::RequestError& exception) {
    ApiError error;
    error.param   = "messages";
    error.message = exception.what();
    switch (exception.kind()) {
    case ginfer::RequestErrorKind::ContextLengthExceeded:
        error.status = 400;
        error.code   = "context_length_exceeded";
        break;
    case ginfer::RequestErrorKind::MediaBudgetExceeded:
        error.status = 400;
        error.code   = "media_budget_exceeded";
        break;
    case ginfer::RequestErrorKind::Overloaded:
        error.param.clear();
        error.status = 429;
        error.type   = "rate_limit_error";
        error.code   = "server_overloaded";
        break;
    case ginfer::RequestErrorKind::QueueTimeout:
        error.param.clear();
        error.status = 503;
        error.type   = "server_error";
        error.code   = "request_queue_timeout";
        break;
    case ginfer::RequestErrorKind::Cancelled:
        error.status = 499;
        error.type   = "request_cancelled";
        error.code   = "client_disconnected";
        break;
    case ginfer::RequestErrorKind::Unavailable:
        error.param.clear();
        error.status = 503;
        error.type   = "server_error";
...
[[noreturn]] void throw_media_error(const ginfer::product::media_acquire::Error& exception) {
    ApiError error;
    error.param   = "messages";
    error.message = exception.what();
    switch (exception.kind()) {
    case ginfer::product::media_acquire::ErrorKind::BudgetExceeded:
        error.status = 400;
        error.code   = "media_budget_exceeded";
        break;
    case ginfer::product::media_acquire::ErrorKind::RemoteUnavailable:
        error.status = 502;
        error.type   = "server_error";
        error.code   = "media_fetch_failed";
        break;
    case ginfer::product::media_acquire::ErrorKind::RemoteTimeout:
        error.status = 504;
        error.type   = "server_error";
        error.code   = "media_fetch_timeout";
        break;
    case ginfer::product::media_acquire::ErrorKind::DeadlineExceeded:
        error.status = 503;
        error.type   = "server_error";
        error.code   = "request_queue_timeout";
        break;
    case ginfer::product::media_acquire::ErrorKind::Cancelled:
...
ginfer::OwnedMedia acquire_media(const ContentPart& part, Clock::time_point deadline,
                                 const std::function<bool()>& is_cancelled,
                                 std::size_t& remaining_bytes) {
    if (remaining_bytes == 0) {
        throw_media_error(ginfer::product::media_acquire::Error(
            ginfer::product::media_acquire::ErrorKind::BudgetExceeded,
            "request media exceeds aggregate byte limit"));
    }
    ginfer::product::media_acquire::Policy policy;
    policy.max_bytes    = std::min(policy.max_bytes, remaining_bytes);
    policy.deadline     = deadline;
    policy.is_cancelled = is_cancelled;
    std::vector<std::uint8_t> source_bytes;
    try {
        source_bytes = ginfer::product::media_acquire::acquire_bytes(part.source, policy);
    } catch (const ginfer::product::media_acquire::Error& exception) {
    ...
    case ginfer::product::media_acquire::SourceKind::Path:
    case ginfer::product::media_acquire::SourceKind::Url:
        media.source_name = part.source.value;
        break;
    case ginfer::product::media_acquire::SourceKind::Data:
        media.source_name = "inline-data";
        break;
    case ginfer::product::media_acquire::SourceKind::Bytes:
...
[[noreturn]] void throw_request_error(const ginfer::RequestError& exception) {
...
void check_preparation_control(Clock::time_point deadline,
    ...
    if (Clock::now() >= deadline) {
        throw_request_error(ginfer::RequestError(RequestErrorKind::QueueTimeout,
                                                 "inference request expired during preparation"));
    }
}

class ServiceOutputSink final : public ginfer::OutputSink {
public:
    ServiceOutputSink(const StreamSink& sink, bool filter_tool_calls)
        : sink_(&sink), filter_tool_calls_(filter_tool_calls) {}

    void publish(ginfer::OutputDelta delta) override {
        if (delta.text.empty()) { return; }
        if (delta.channel == ginfer::OutputChannel::Reasoning) {
...
GenerationService::GenerationService(ServeOptions options, LoadProgress load_progress)
    ...
    ginfer::EngineOptions engine_options;
    ...
    engine_              = std::make_unique<ginfer::Engine>(std::move(engine_options));
    options_.speculative = engine_->options().speculative;
    prompt_capabilities_ = engine_->prompt_capabilities();
    if (!options_.enable_thinking && !prompt_capabilities_.enable_thinking) {
        throw std::invalid_argument(
            "--no-thinking is not supported by the loaded chat template");
    }
...
std::shared_ptr<RequestLifetime> GenerationService::acquire_request_lifetime() const {
    const auto started = Clock::now();
    {
        ...
        if (request_capacity_->active >= request_capacity_->maximum) {
            throw_request_error(ginfer::RequestError(RequestErrorKind::Overloaded,
...
PreparedRequest GenerationService::prepare(const GenerationRequest& request,
    ...
    ginfer::RequestOptions request_options = to_request_options(request, options_);
    ...
    try {
        ...
        std::size_t remaining_media_bytes =
            std::min(options_.max_request_bytes, ginfer::kMaximumPromptMediaBytes);
        ...
        ginfer::PreparedPrompt prompt = engine_->prepare(std::move(input), control);
        check_preparation_control(prepared.lifetime->deadline, is_cancelled);
    ...
    } catch (const ApiException&) { throw; } catch (const ginfer::RequestError& exception) {
...
GenerationOutcome GenerationService::run(PreparedRequest& prepared, const StreamSink* sink,
    ...
    ginfer::OutputSink* public_sink = output_sink.get();
    ginfer::CancellationView cancellation;
    if (is_cancelled || (sink != nullptr && sink->is_cancelled)) {
        cancellation = ginfer::CancellationView([external = std::move(is_cancelled), sink]() {
            return (external && external()) ||
                   (sink != nullptr && sink->is_cancelled && sink->is_cancelled());
        });
    }

    ginfer::GenerationResult result;
    try {
        result = prepared.generation.wait(public_sink, cancellation);
    } catch (const ginfer::RequestError& exception) { throw_request_error(exception); }
    ...
    outcome.metrics.speculative_expanded_rounds = result.speculative.expanded_rounds;
    ...
    if (prepared.tool_capable) {
        ParsedToolCallOutput parsed =
            parse_tool_call_output(outcome.text, prepared.tool_name_max_length);
