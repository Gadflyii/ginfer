...
// Product-side adapter between HTTP protocol requests and the public GInfer
...
struct GenerationMetrics {
    ...
    std::uint64_t speculative_expanded_rounds = 0;
    ...
    ginfer::PrefixReusePath prefix_reuse_path = ginfer::PrefixReusePath::FullReset;
};
...
struct GenerationOutcome {
    ...
    ginfer::FinishReason finish_reason = ginfer::FinishReason::OutputLimit;
...
struct PreparedRequest {
    ginfer::GenerationHandle generation;
    ginfer::ResolvedSamplingParameters sampling;
    double prepare_seconds     = 0.0;
...
public:
    ...
    [[nodiscard]] ginfer::LoadSummary load_summary() const { return engine_->load_summary(); }

    [[nodiscard]] ginfer::MemorySummary memory_summary() const { return engine_->memory_summary(); }

    [[nodiscard]] ginfer::RuntimeStats runtime_stats() const { return engine_->runtime_stats(); }

    [[nodiscard]] ginfer::MediaCacheSummary media_cache_summary() const {
        return engine_->media_cache_summary();
    }

    [[nodiscard]] ginfer::ModelSamplingDefaults sampling_defaults() const {
...
private:
    ...
    std::unique_ptr<ginfer::Engine> engine_;
    ginfer::PromptCapabilities prompt_capabilities_;
