...
ThroughputReport make_throughput_report(const ginfer::RuntimeStats& previous,
                                        const ginfer::RuntimeStats& current,
...
void HttpServer::run_stats_reporter() {
    ...
    ginfer::RuntimeStats previous   = service_->runtime_stats();
    ...
    for (;;) {
        ...
        const ginfer::RuntimeStats current = service_->runtime_stats();
...
void HttpServer::attach(GenerationService& service) {
    ...
    const ginfer::LoadSummary load = service.load_summary();
    public_model_id_               = resolve_public_model_id(options_, load.model_id);
    service_                       = &service;
    request_jsonl_.write_server_start(service.options(), service.sampling_defaults(),
