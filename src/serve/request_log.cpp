...
const char* finish_reason_name(ginfer::FinishReason reason) {
    switch (reason) {
...
const char* kv_cache_name(ginfer::KvCacheStorage storage) {
    return storage == ginfer::KvCacheStorage::BFloat16 ? "bf16" : "int8-group64";
}

const char* kv_capacity_mode_name(ginfer::KvCapacityMode mode) {
    return mode == ginfer::KvCapacityMode::Automatic ? "auto" : "explicit";
}

const char* proposal_head_name(ginfer::ProposalHead proposal) {
    return proposal == ginfer::ProposalHead::Optimized ? "optimized" : "full";
}

const char* prefix_reuse_path_name(ginfer::PrefixReusePath path) {
    switch (path) {
    case ginfer::PrefixReusePath::FullReset:
        return "full_reset";
    case ginfer::PrefixReusePath::AppendAtFrontier:
        return "append_frontier";
    case ginfer::PrefixReusePath::RestoreTurnCheckpoint:
        return "restore_turn_checkpoint";
    case ginfer::PrefixReusePath::RestoreResponseCheckpoint:
...
Json sampler_json(const ginfer::ResolvedSamplingParameters& sampling) {
...
Json preset_json(const ginfer::SamplingPreset& preset) {
...
Json overrides_json(const ginfer::SamplingOverrides& overrides) {
...
Json arena_json(const ginfer::ArenaMemorySummary& arena) {
...
Json speculative_json(const GenerationMetrics& metrics) {
    return Json{{"backend", product::speculative_backend_name(metrics.speculative_backend)},
                ...
                {"expanded_rounds", metrics.speculative_expanded_rounds},
...
std::string sampler_str(const ginfer::ResolvedSamplingParameters& sampling) {
...
std::string format_server_start_json(
    ...
    const ginfer::ModelSamplingDefaults& sampling_defaults, const std::string& public_model_id,
    const ginfer::LoadSummary& load, const ginfer::MemorySummary& memory,
