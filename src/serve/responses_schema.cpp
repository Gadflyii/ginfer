...
ginfer::product::media_acquire::Source parse_image_source(const Json& part) {
    ...
    ginfer::product::media_acquire::Source source;
    source.value = part.at("image_url").get<std::string>();
    if (source.value.starts_with("data:")) {
        source.kind = ginfer::product::media_acquire::SourceKind::Data;
    } else if (source.value.starts_with("http://") || source.value.starts_with("https://")) {
        source.kind = ginfer::product::media_acquire::SourceKind::Url;
    } else {
...
ginfer::product::media_acquire::Source parse_video_source(const Json& part) {
...
std::string response_status(ginfer::FinishReason reason) {
    switch (reason) {
