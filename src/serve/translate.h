...
using MediaAcquirer = std::function<ginfer::OwnedMedia(const ContentPart&)>;

struct ResolvedPromptSemantics {
    bool enable_thinking = true;
    std::optional<ginfer::ReasoningEffort> reasoning_effort;
...
ginfer::PromptInput to_prompt_input(const GenerationRequest& req,
...
ginfer::RequestOptions to_request_options(const GenerationRequest& req, const ServeOptions& server);
