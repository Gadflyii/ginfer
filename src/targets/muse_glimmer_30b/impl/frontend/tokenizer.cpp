...
namespace uni = ginfer::text::unicode_internal;
...
void merge_added_tokens_decoder(const Json& root, std::string_view label,
                                ...
                                const std::unordered_map<std::string, int>& occupied_vocab_tokens,
                                std::vector<AddedToken>& tokens) {
    if (!root.is_object() || !root.contains("added_tokens_decoder")) { return; }
...
void append_bpe_ids(std::vector<int>& ids, std::string_view text, bool has_bpe_merges,
    ...
    for (const std::string_view word : qwen_split_words(normalized)) {
        const std::string encoded = byte_level_encode(word);
        if (const auto whole = token_to_id.find(encoded); whole != token_to_id.end()) {
            ids.push_back(whole->second);
            continue;
        }
        std::vector<std::string> symbols = byte_level_symbols(encoded);
