...
struct RoundStateSpec {
    ...
    std::uint32_t dflash_target_window = 0;
    std::uint32_t dflash_selector_top_k = 0;
...
struct DFlashDecodeIngress {
    ...
    std::array<std::int32_t, kMaximumConcurrency> target_extents{};
    std::array<std::int32_t, kMaximumConcurrency> target_valid_columns{};
    std::array<std::int32_t, kMaximumConcurrency> context_match_lengths{};
    std::array<std::int32_t, kMaximumConcurrency> context_copy_extents{};
    std::array<TokenId, kMaximumConcurrency * kDFlashDecodeMaximumDrafts> context_copy_tokens{};
    std::array<std::int32_t, kMaximumConcurrency> text_kv_table_rows{};
...
struct DFlashDecodeStateLayout {
    ...
    TensorRegion learned_draft_tokens;
    TensorRegion draft_tokens;
    TensorRegion learned_candidates;
    TensorRegion learned_probabilities;
    TensorRegion proposal_candidates;
    TensorRegion proposal_probabilities;
    TensorRegion verify_ids;
    TensorRegion target_argmax;
    TensorRegion target_logits;
    TensorRegion target_hidden;
...
struct DFlashDecodeState {
    ...
    Tensor target_extents;
    Tensor target_valid_columns;
    Tensor context_match_lengths;
    Tensor context_copy_extents;
    Tensor context_copy_tokens;
    Tensor text_kv_table_rows;
    ...
    Tensor learned_draft_tokens;
    Tensor draft_tokens;
    Tensor learned_candidates;
    Tensor learned_probabilities;
    Tensor proposal_candidates;
    Tensor proposal_probabilities;
    Tensor verify_ids;
    Tensor target_argmax;
    Tensor target_logits;
    Tensor target_hidden;
    ...
    DFlashDecodeState(DeviceSpan backing, const DFlashDecodeStateLayout& layout,
                      std::uint32_t batch_capacity, std::uint32_t draft_window,
                      std::uint32_t target_window, std::uint32_t selector_top_k);
};
