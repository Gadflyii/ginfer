...
struct PersistentLayout {
    ...
    std::optional<GdnReplayRecordLayout> wide_replay_records;
...
template <>
struct SequencePlanImpl<GINFER_QWEN36_VARIANT> {
    typename GINFER_QWEN36_VARIANT::WeightsProfile weights_profile;
    std::uint32_t capacity                 = 0;
    ...
    bool use_cuda_graph = true;
    int device          = 0;
    GINFER_QWEN36_RUNTIME_NS::PersistentLayout persistent;
    GINFER_QWEN36_RUNTIME_NS::WorkspacePlan workspace;
...
template <>
struct SequencePlannerImpl<GINFER_QWEN36_VARIANT> {
    GINFER_QWEN36_RUNTIME_NS::SequencePlanningInputs inputs;
    runtime::SequenceCapacityCurve curve;
    std::unique_ptr<SequencePlanImpl<GINFER_QWEN36_VARIANT>> minimum;
};
