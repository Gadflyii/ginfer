...
#include "ginfer/ops/gated_delta_net.h"
#include "ginfer/ops/gdn_gating_proj.h"
#include "ginfer/ops/gdn_input_proj.h"
...
std::uint32_t dflash_target_drafts(const SequencePlanImpl& plan) {
    if (!plan.features.dflash()) { return 0; }
    if constexpr (DFlashConfig::sampled_dflash2) {
        return qwen3_6::kDFlashDecodeMaximumDrafts;
    }
    return plan.draft_window;
}
...
PersistentLayout persistent_layout(const SequencePlanImpl& plan) {
    ...
    if (plan.speculative_backend != SpeculativeBackend::None) {
        ...
        const std::uint32_t target_drafts = dflash_target_drafts(plan);
        if (plan.features.dflash() && target_drafts > plan.draft_window) {
            out.wide_replay_records = plan_gdn_replay_records(
                builder, GdnReplayRecordSpec{
                             .layers          = TextConfig::gdn_layers(),
                             .record_capacity = static_cast<std::int32_t>(plan.max_concurrency),
                             .width           = static_cast<std::int32_t>(target_drafts + 1U),
                             .conv_channels   = TextConfig::convolution_dim,
                             .qk_heads        = TextConfig::gdn_key_heads,
                             .value_heads     = TextConfig::gdn_value_heads,
                             .key_dim         = TextConfig::gdn_key_head_dim,
    ...
    out.round = qwen3_6::begin_round_state_layout(
        builder, qwen3_6::RoundStateSpec{.hidden         = TextConfig::hidden,
                                         ...
                                         .dflash_target_window = dflash_target_drafts(plan),
                                         .dflash_selector_top_k =
                                             static_cast<std::uint32_t>(
                                                 plan.features.dflash() &&
                                                         DFlashConfig::sampled_dflash2
                                                     ? DFlashConfig::selector_top_k
                                                     : 0),
                                         .enable_mtp     = plan.features.mtp(),
...
WorkspacePlan build_workspace_plan(const SequencePlanImpl& plan) {
    ...
    const auto dflash_drafts = static_cast<std::int32_t>(dflash_target_drafts(plan));
    const auto dflash_verify = dflash_drafts + 1;
        ...
        if constexpr (!Variant::supports_dflash) {
            ...
            const auto dflash_proposal_capacity = [&](std::int32_t width, std::int32_t batch) {
                ...
                if constexpr (DFlashConfig::sampled_dflash2) {
                    {
                        auto layer = layout.scope();
                        (void)workspace_recipe::dflash2_layer<DFlashConfig>(layout, tokens);
                    ...
                    scratch(layout, ops::dflash2_select_path_workspace_capacity_bytes(
                                        TextConfig::token_domain, DFlashConfig::selector_rank,
                                        DFlashConfig::selector_top_k, drafts, drafts, batch,
                                        batch));
                    return finish(layout);
                }
                {
                    ...
                    scratch(layout,
                            std::max(ops::swa_workspace_capacity_bytes({0, plan.capacity}, width,
                                     ...
                                     ops::bidirectional_gqa_attention_workspace_capacity_bytes(
                                         {0, plan.capacity}, width, width, batch, plan.occupancy)));
            ...
            for (std::int32_t batch = 1; batch <= static_cast<std::int32_t>(plan.max_concurrency);
                ...
                target_body(target, aggregate, aggregate, qwen3_6::TextPhase::Verify,
                            ...
                            text_envelope);
                const std::size_t accept = DFlashConfig::sampled_dflash2
                                               ? ops::dflash2_accept_workspace_capacity_bytes(
                                                     TextConfig::token_domain, dflash_drafts,
                                                     dflash_drafts,
                                                     batch, batch)
                                               : ops::speculative_accept_greedy_drafts_workspace_capacity_bytes(
...
void validate_target_options(DeviceContext& device, const EngineOptions& options) {
    ...
    case SpeculativeBackend::Automatic:
        throw std::logic_error("automatic speculative selection was not resolved by the target");
    ...
    case SpeculativeBackend::DFlash:
        ...
        if constexpr (DFlashConfig::sampled_dflash2) {
            if (options.speculative.proposal_head != ProposalHead::Full) {
                throw std::invalid_argument(
                    "DFlash2 uses the registered full output head; optimized proposal heads are unsupported");
            }
        }
        break;
    }
...
std::unique_ptr<SequencePlanImpl> build_sequence_candidate(const SequencePlanningInputs& inputs,
    ...
    impl->occupancy           = inputs.occupancy;
    ...
    if (impl->use_cuda_graph) {
        ...
        if (impl->speculative_backend == SpeculativeBackend::None) {
            ...
            const auto class_allowance = [&](std::uint32_t batch_size,
                ...
                const auto profiles =
                    dflash_graph_profiles(impl->capacity, target_window, batch_size);
                return graph_topology_allowance(
                    profiles,
                    [&](GraphExecutionProfile profile) {
                        const std::uint64_t final_visible = std::min<std::uint64_t>(
                            impl->capacity,
                            static_cast<std::uint64_t>(profile.max) + target_window + 1ULL);
            ...
            for (std::uint32_t batch_size = 1; batch_size <= impl->max_concurrency; ++batch_size) {
                impl->graph_allowance_bytes =
                    checked_add(impl->graph_allowance_bytes,
                                class_allowance(batch_size, impl->draft_window),
                                "DFlash exact-b graph allowance");
            }
            const std::uint32_t target_window = dflash_target_drafts(*impl);
            if (target_window > impl->draft_window) {
                impl->graph_allowance_bytes =
                    checked_add(impl->graph_allowance_bytes,
                                class_allowance(1, target_window),
                                "wide DFlash batch-one graph allowance");
            }
        }
    }
