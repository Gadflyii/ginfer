...
#include "ginfer/ops/gdn_replay.h"
...
bool dflash_storage_ready(const SequenceState& sequence) noexcept {
    if (!sequence.kv || sequence.kv->text.bound_row() < 0) { return false; }
    if constexpr (DFlashConfig::full_layers > 0) {
        return sequence.kv->backend && sequence.kv->backend->bound_row() >= 0;
    }
    return !sequence.kv->backend;
}

std::int32_t dflash_table_row(const SequenceState& sequence) {
    if (!dflash_storage_ready(sequence)) {
        throw std::logic_error("DFlash sequence storage is not bound");
    }
    if constexpr (DFlashConfig::full_layers > 0) {
        return sequence.kv->backend->bound_row();
    }
    return static_cast<std::int32_t>(sequence.lane);
}
...
ProgramImplCore::ProgramImplCore(const LoadedModelData& model_in, const SequencePlanImpl& plan,
    ...
    : model(model_in), device(device_in), capacity(plan.capacity), kv_capacity(plan.kv_capacity),
      ...
      dflash_target_window(plan.persistent.round.spec.dflash_target_window),
    ...
    if (plan.persistent.wide_replay_records) {
        wide_replay_records.emplace(backing, *plan.persistent.wide_replay_records);
    }
    ...
    if (wide_replay_records.has_value() !=
        (speculative_backend == SpeculativeBackend::DFlash &&
         dflash_target_window > draft_window)) {
        throw std::logic_error("wide ReplaySSM records do not match the DFlash plan");
    }
    ...
    try {
        if (request_plan.reuse == ReusePath::FullReset) {
            ...
            if (speculative_backend == SpeculativeBackend::Mtp) {
                ...
                if (!dflash || !dflash_storage_ready(sequence) ||
                    sequence.dflash_context_frontier < base) {
        ...
        sequence.context_copy_confirmations = 0;
        sequence.prefix_identity.assign(prompt);

        if (speculative_backend == SpeculativeBackend::DFlash) {
            if (!dflash || !io.dflash_decode || !dflash_storage_ready(sequence)) {
            ...
            *dflash_host_ingress                         = {};
            ...
            dflash_host_ingress->dflash_kv_table_rows[0] = dflash_table_row(sequence);
...
void ProgramImplCore::resolve_pending_batch(std::span<const std::uint32_t> lanes,
    ...
    std::uint32_t pending_target_window = 0;
    for (std::size_t row = 0; row < lanes.size(); ++row) {
        const std::uint32_t lane = lanes[row];
        if (lane >= max_concurrency || requests[lane].lifecycle != Lifecycle::Pending ||
        ...
        if (speculative_backend == SpeculativeBackend::DFlash) {
            if (pending.row_stride == 0 || pending.target_window + 1U != pending.row_stride ||
                pending.target_window > dflash_target_window ||
                (row != 0 && pending.target_window != pending_target_window)) {
                throw std::logic_error(
                    "speculative pending rows have inconsistent physical widths");
            }
            pending_target_window = pending.target_window;
        } else {
            pending_target_window = draft_window;
        }
        const SequenceState& sequence   = sequences[lane];
    ...
    try {
        const GdnReplayRecords* active_replay = &*replay_records;
        if (speculative_backend == SpeculativeBackend::DFlash &&
            pending_target_window > draft_window) {
            if (!wide_replay_records) {
                throw std::logic_error("wide DFlash pending round has no ReplaySSM storage");
            }
            active_replay = &*wide_replay_records;
        }
        ops::gdn_replay_fold(*active_replay, decoder->linear_attention.all_layers_view(),
        ...
        if (needs_hidden_correction) {
            ...
            if (speculative_backend == SpeculativeBackend::Mtp && io.mtp_decode) {
                ...
                selector_tensor                   = frame.target_extents.slice(0, 0, batch);
                hidden = Tensor(frame.target_hidden.data, DType::BF16,
                                {TextConfig::hidden,
                                 static_cast<std::int32_t>(pending_target_window + 1U), batch});
                selected     = frame.target_continuation_hidden.slice(1, 0, batch);
                destinations = frame.lanes.slice(0, 0, batch);
            } else {
    ...
    try {
        for (std::size_t row = 0; row < lanes.size(); ++row) {
            ...
            const TokenId* token_base =
                speculative_backend == SpeculativeBackend::Mtp
                    ? mtp_host_egress->licensed_tokens.data() +
                          row * static_cast<std::size_t>(draft_window + 1U)
                    : dflash_host_egress->licensed_tokens.data() +
                          row * static_cast<std::size_t>(pending.row_stride);
            sequence.ledger.insert(sequence.ledger.end(), token_base, token_base + committed);
            sequence.context_copy.append(sequence.ledger);
            ...
            if (speculative_backend == SpeculativeBackend::Mtp) {
                ...
                const bool confirmed_copy = !terminal[row] &&
                    committed == pending.produced &&
                    pending.context_copy_extent >= draft_window &&
                    pending.accepted_drafts >= draft_window;
                sequence.context_copy_confirmations =
                    confirmed_copy ? std::min(sequence.context_copy_confirmations + 1U, 2U) : 0U;
            }
...
void ProgramImplCore::prepare_graphs() {
        ...
        if (io.dflash_decode) {
            ...
            *dflash_host_egress        = {};
            ...
            for (std::uint32_t row = 0; row < batch_size; ++row) {
                ...
                dflash_host_ingress->target_extents[row] = static_cast<std::int32_t>(extent);
    ...
    if (speculative_backend == SpeculativeBackend::DFlash) {
        ...
        prepare_representative(code_warm.min, 1);
        device.synchronize();
        schedule::dflash_decode_batch(dflash_state, 1, draft_window, draft_window,
        ...
        if (dflash_target_window > draft_window) {
            if (!wide_replay_records) {
                throw std::logic_error("wide DFlash graph plan has no ReplaySSM storage");
            }
            schedule::DFlashBatchContext wide_state = dflash_state;
            wide_state.execution.replay_records = &*wide_replay_records;
            const auto wide_batch_one =
                dflash_graph_profiles(capacity, dflash_target_window, 1);
            validate_graph_profiles(wide_batch_one, capacity - 1, "wide DFlash");
            dflash_wide_graphs.profiles.reserve(wide_batch_one.size());
            for (const GraphExecutionProfile planned : wide_batch_one) {
                dflash_wide_graphs.profiles.emplace_back();
                DecodeGraphProfile& profile    = dflash_wide_graphs.profiles.back();
                ...
                const ops::GqaExecutionEnvelope target_envelope{
                    ...
                    static_cast<std::uint32_t>(std::min<std::uint64_t>(
                        capacity, static_cast<std::uint64_t>(planned.max) +
                                      dflash_target_window + 1ULL))};
                schedule::capture_dflash_decode_batch(
                    wide_state, 1, draft_window, dflash_target_window,
                    dflash_envelopes(planned.min, planned.max, dflash_target_window),
    ...
    if (speculative_backend == SpeculativeBackend::DFlash) {
        ...
        if (!dflash_wide_graphs.profiles.empty()) {
            instantiate_graph_family(dflash_wide_graphs, "wide DFlash", device,
...
void ProgramImplCore::install_sampling(SequenceState& sequence, RequestControl& request,
    ...
    request.speculative_stats = SpeculativeStats{
        ...
        .verification_window   = speculative_backend == SpeculativeBackend::DFlash
                                     ? dflash_target_window
                                     : draft_window,
        .accepted_per_position = std::vector<std::uint64_t>(
            speculative_backend == SpeculativeBackend::DFlash ? dflash_target_window
                                                               : draft_window,
            0),
    };
...
void ProgramImplCore::enqueue_dflash_context_append(std::span<const std::uint32_t> lanes,
    ...
    std::uint32_t minimum_count = dflash_target_window + 1U;
    std::uint32_t maximum_count = 0;
    *dflash_host_ingress        = {};
    for (std::size_t row = 0; row < lanes.size(); ++row) {
        const std::uint32_t lane = lanes[row];
        if (lane >= max_concurrency || counts[row] == 0 ||
            counts[row] > dflash_target_window + 1U ||
        ...
        if (!dflash_storage_ready(sequence) || end64 > capacity) {
        ...
        dflash_host_ingress->dflash_kv_table_rows[row] = dflash_table_row(sequence);
        dflash_host_ingress->lanes[row]                = static_cast<std::int32_t>(lane);
        materialize_sequence_kv(
            sequence, std::max(sequence.text_kv_valid, end),
            DFlashConfig::full_layers > 0 ? end : 0U);
    ...
    Tensor positions(frame.append_positions.data, DType::I32,
                     {static_cast<std::int32_t>(maximum_count), batch});
    Tensor device_counts = frame.append_counts.slice(0, 0, batch);

    work.reset();
    Tensor features = work.alloc(
        DType::BF16,
        {DFlashConfig::feature_rows, static_cast<std::int32_t>(maximum_count), batch});
    Tensor pending_features =
        dflash->pending_features.slice(1, 0, static_cast<std::int32_t>(maximum_count));
    ops::prepare_ragged_prefix(pending_features, lane_tensor, device_starts, device_ends, features,
...
runtime::PrefillStepResult ProgramImplCore::advance_prefill(SequenceState& sequence,
    ...
    try {
        ...
        sequence.context_copy.rebuild(sequence.ledger);
        ...
        if (rewrite_checkpoint_capture) {
            ...
            if (speculative_backend == SpeculativeBackend::DFlash &&
                (!dflash || !dflash_storage_ready(sequence) ||
...
ProgramImplCore::decode_mtp_batch(std::span<const std::uint32_t> lanes,
    ...
    for (std::size_t row = 0; row < lanes.size(); ++row) {
        ...
        if (request.lifecycle != Lifecycle::Active ||
            budgets[row].generated_tokens_remaining == 0 || !sequence.kv ||
            sequence.kv->text.bound_row() < 0 || !sequence.kv->backend ||
    ...
    try {
        ...
        for (std::size_t row = 0; row < lanes.size(); ++row) {
            ...
            if (pcur == 0) {
                ...
                const std::int32_t reported_positions = std::min<std::int32_t>(
                    accepted_i,
                    static_cast<std::int32_t>(request.speculative_stats.accepted_per_position.size()));
                for (std::int32_t i = 0; i < reported_positions; ++i) {
...
ProgramImplCore::decode_dflash_batch(std::span<const std::uint32_t> lanes,
    ...
    std::array<qwen3_6::detail::ContextCopyMatch, kMaximumConcurrency> context_matches{};
    std::array<std::uint32_t, kMaximumConcurrency> context_confirmations{};
    bool requires_wide_append = false;
    std::uint32_t maximum_frontier = 0;
    for (std::size_t row = 0; row < lanes.size(); ++row) {
        const std::uint32_t lane = lanes[row];
        if (lane >= max_concurrency ||
        ...
        const RequestControl& request = requests[lane];
        if (request.lifecycle != Lifecycle::Active ||
            budgets[row].generated_tokens_remaining == 0 || !dflash_storage_ready(sequence) ||
            sequence.execution_frontier >= capacity ||
            sequence.text_kv_valid != sequence.execution_frontier ||
            sequence.dflash_context_frontier > sequence.execution_frontier ||
            sequence.execution_frontier - sequence.dflash_context_frontier >
                dflash_target_window + 1U ||
            sequence.ledger_frontier != sequence.execution_frontier + 1 ||
            sequence.ledger.size() != sequence.ledger_frontier ||
            sequence.prefix_identity.size() != sequence.ledger_frontier) {
            throw std::logic_error("DFlash batch row is not decode-ready");
        }
        requires_wide_append = requires_wide_append ||
            sequence.execution_frontier - sequence.dflash_context_frontier > draft_window + 1U;
        if constexpr (DFlashConfig::sampled_dflash2) {
            const std::uint32_t max_by_budget = budgets[row].generated_tokens_remaining > 1
                                                    ? budgets[row].generated_tokens_remaining - 1U
                                                    : 0U;
            const std::uint32_t max_by_capacity =
                capacity - sequence.execution_frontier - 1U;
            context_matches[row] = sequence.context_copy.find(
                sequence.ledger,
                std::min({dflash_target_window, max_by_budget, max_by_capacity}));
            context_confirmations[row] = sequence.context_copy_confirmations;
        }
        maximum_frontier = std::max(maximum_frontier, sequence.execution_frontier);
    }

    const bool use_wide_target = qwen3_6::detail::use_wide_context_copy(
        std::span<const qwen3_6::detail::ContextCopyMatch>(context_matches.data(), lanes.size()),
        std::span<const std::uint32_t>(context_confirmations.data(), lanes.size()), draft_window,
        dflash_target_window, requires_wide_append);
    const std::uint32_t target_window = use_wide_target ? dflash_target_window : draft_window;
    const std::uint32_t width = target_window + 1U;
    ...
    for (std::size_t row = 0; row < lanes.size(); ++row) {
        ...
        const std::uint32_t max_by_budget = budgets[row].generated_tokens_remaining > 1
                                                ? budgets[row].generated_tokens_remaining - 1U
                                                : 0U;
        const std::uint32_t max_by_capacity = capacity - sequence.execution_frontier - 1U;
        const std::uint32_t proposal_extent =
            std::min({draft_window, max_by_budget, max_by_capacity});
        const std::uint32_t target_extent =
            context_matches[row].available()
                ? std::min({target_window, context_matches[row].valid_tokens, max_by_budget,
                            max_by_capacity})
                : proposal_extent;
    ...
    try {
        ...
        schedule::DFlashEnvelopes envelopes =
            dflash_envelopes(0, maximum_frontier, target_window);
        ops::GqaExecutionEnvelope target_envelope{1, maximum_target_tokens};
        if (use_cuda_graph && (!use_wide_target || lanes.size() == 1U)) {
            DecodeGraphFamily& graph_family =
                use_wide_target ? dflash_wide_graphs : dflash_graphs;
            DecodeGraphProfile& profile =
                select_graph_profile(graph_family, static_cast<std::uint32_t>(lanes.size()),
                                     maximum_frontier, "DFlash batch");
            executable      = &install_graph_profile(graph_family, profile, "DFlash batch");
            envelopes       = dflash_envelopes(profile.min_execution_frontier,
                                               profile.max_execution_frontier, target_window);
            target_envelope = {
                1, static_cast<std::uint32_t>(std::min<std::uint64_t>(
        ...
        *dflash_host_ingress = {};
        for (std::size_t row = 0; row < lanes.size(); ++row) {
            ...
            const std::uint32_t max_by_budget = budgets[row].generated_tokens_remaining > 1
                                                    ? budgets[row].generated_tokens_remaining - 1U
                                                    : 0U;
            const std::uint32_t max_by_capacity = capacity - frontier - 1U;
            const std::uint32_t proposal_extent =
                std::min({draft_window, max_by_budget, max_by_capacity});
            const std::uint32_t context_extent =
                context_matches[row].available()
                    ? std::min({target_window, context_matches[row].valid_tokens, max_by_budget,
                                max_by_capacity})
                    : 0U;
            const std::uint32_t target_extent =
                context_extent != 0 ? context_extent : proposal_extent;
            ...
            dflash_host_ingress->proposal_extents[row] =
                static_cast<std::int32_t>(proposal_extent);
            dflash_host_ingress->proposal_valid_columns[row] =
                static_cast<std::int32_t>(proposal_extent + 1U);
            dflash_host_ingress->target_extents[row] =
                static_cast<std::int32_t>(target_extent);
            dflash_host_ingress->target_valid_columns[row] =
                static_cast<std::int32_t>(target_extent + 1U);
            dflash_host_ingress->context_match_lengths[row] =
                static_cast<std::int32_t>(context_matches[row].match_length);
            dflash_host_ingress->context_copy_extents[row] =
                static_cast<std::int32_t>(context_extent);
            for (std::uint32_t column = 0; column < context_extent; ++column) {
                dflash_host_ingress->context_copy_tokens[row * target_window + column] =
                    context_matches[row].tokens[column];
            }
            ...
            materialize_sequence_kv(sequence, frontier + target_extent + 1U,
                                    DFlashConfig::full_layers > 0 ? frontier : 0U);
        }

        const GdnReplayRecords* round_replay = replay_records ? &*replay_records : nullptr;
        if (use_wide_target) {
            if (!wide_replay_records) {
                throw std::logic_error("wide DFlash round has no ReplaySSM storage");
            }
            round_replay = &*wide_replay_records;
        }
        ...
        schedule::dflash_decode_batch(schedule_state, static_cast<std::int32_t>(lanes.size()),
                                      draft_window, target_window, envelopes, target_envelope,
                                      executable);
        device.synchronize();
        ...
        for (std::size_t row = 0; row < lanes.size(); ++row) {
            ...
            const std::uint32_t extent =
                static_cast<std::uint32_t>(dflash_host_ingress->target_extents[row]);
            if (count_i <= 0 || count_i > static_cast<std::int32_t>(width) || accepted_i < 0 ||
            ...
            if (extent == 0) {
                ...
                request.speculative_stats.rounds += 1;
                if (target_window > draft_window) {
                    request.speculative_stats.expanded_rounds += 1;
                }
            ...
            request.pending                  = PendingCandidate{
                                 ...
                                 .context_copy_extent = static_cast<std::uint32_t>(
                                     dflash_host_ingress->context_copy_extents[row]),
