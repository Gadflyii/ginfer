...
void validate_spec(const RoundStateSpec& spec) {
    ...
    if (spec.enable_dflash &&
        (spec.dflash_target_window < spec.draft_window ||
         spec.dflash_target_window > kDFlashDecodeMaximumDrafts)) {
        throw std::invalid_argument(
            "RoundState DFlash target window is outside the decode frame domain");
    }
    if (!spec.enable_dflash && spec.dflash_target_window != 0) {
        throw std::invalid_argument("RoundState target window requires DFlash");
    }
    if (!spec.enable_dflash && spec.dflash_selector_top_k != 0) {
        throw std::invalid_argument("RoundState DFlash selector requires DFlash");
    }
    if (spec.dflash_selector_top_k > 64) {
        throw std::invalid_argument("RoundState DFlash selector top-k exceeds 64");
    }
    ...
    (void)checked_i32(static_cast<std::uint64_t>(spec.dflash_target_window) + 1ULL,
                      "RoundState DFlash target window exceeds int32");
}
...
void complete_round_state_layout(LayoutBuilder& builder, RoundStateLayout& layout) {
    ...
    if (layout.spec.enable_dflash) {
        const std::int32_t target_columns = checked_i32(
            static_cast<std::uint64_t>(layout.spec.dflash_target_window) + 1ULL,
            "RoundState DFlash target columns exceed int32");
        const std::int32_t target_drafts = target_columns - 1;
        ...
        decode.target_positions =
            add_tensor(builder, DType::I32, {target_columns, batch}, "DFlash target positions");
        decode.append_positions =
            add_tensor(builder, DType::I32, {target_columns, batch}, "DFlash append positions");
        decode.append_counts = add_tensor(builder, DType::I32, {batch}, "DFlash append counts");
        decode.learned_draft_tokens =
            add_tensor(builder, DType::I32, {columns - 1, batch},
                       "DFlash learned proposal draft tokens");
        decode.draft_tokens =
            add_tensor(builder, DType::I32, {target_drafts, batch},
                       "DFlash fused proposal draft tokens");
        if (layout.spec.dflash_selector_top_k != 0) {
            const auto top_k = checked_i32(layout.spec.dflash_selector_top_k,
                                           "DFlash selector top-k exceeds int32");
            decode.learned_candidates = add_tensor(
                builder, DType::I32, {top_k, columns - 1, batch},
                "DFlash learned proposal candidates");
            decode.learned_probabilities = add_tensor(
                builder, DType::FP32, {top_k, columns - 1, batch},
                "DFlash learned proposal probabilities");
            decode.proposal_candidates = add_tensor(
                builder, DType::I32, {top_k, target_drafts, batch},
                "DFlash fused proposal candidates");
            decode.proposal_probabilities = add_tensor(
                builder, DType::FP32, {top_k, target_drafts, batch},
                "DFlash fused proposal probabilities");
        }
        decode.verify_ids =
            add_tensor(builder, DType::I32, {target_columns, batch}, "DFlash target verify ids");
        decode.target_argmax =
            add_tensor(builder, DType::I32, {target_columns, batch}, "DFlash target argmax");
        decode.target_logits =
            add_tensor(builder, DType::BF16, {layout.spec.output_rows, target_columns, batch},
                       "DFlash target logits");
        decode.target_hidden = add_tensor(
            builder, DType::BF16, {layout.spec.hidden, target_columns, batch},
...
DFlashDecodeState::DFlashDecodeState(DeviceSpan backing, const DFlashDecodeStateLayout& layout,
                                     std::uint32_t batch_capacity, std::uint32_t draft_window,
                                     std::uint32_t target_window,
                                     std::uint32_t selector_top_k) {
    if (batch_capacity == 0 || batch_capacity > kMaximumConcurrency || draft_window == 0 ||
        draft_window > kDFlashDecodeMaximumDrafts || target_window < draft_window ||
        target_window > kDFlashDecodeMaximumDrafts) {
        throw std::invalid_argument(
    ...
    const auto batch          = static_cast<std::int32_t>(batch_capacity);
    const auto target_drafts  = static_cast<std::int32_t>(target_window);
    const auto target_width   = target_drafts + 1;
    ingress                   = layout.ingress.bind(backing);
    egress                    = layout.egress.bind(backing);
    const auto ingress_tensor = [&](std::size_t offset, DType dtype,
                                    std::initializer_list<std::int32_t> shape) {
    ...
    proposal_valid_columns = ingress_tensor(
        offsetof(DFlashDecodeIngress, proposal_valid_columns), DType::I32, {batch});
    target_extents =
        ingress_tensor(offsetof(DFlashDecodeIngress, target_extents), DType::I32, {batch});
    target_valid_columns =
        ingress_tensor(offsetof(DFlashDecodeIngress, target_valid_columns), DType::I32, {batch});
    context_match_lengths = ingress_tensor(
        offsetof(DFlashDecodeIngress, context_match_lengths), DType::I32, {batch});
    context_copy_extents = ingress_tensor(
        offsetof(DFlashDecodeIngress, context_copy_extents), DType::I32, {batch});
    context_copy_tokens = ingress_tensor(
        offsetof(DFlashDecodeIngress, context_copy_tokens), DType::I32,
        {target_drafts, batch});
    text_kv_table_rows =
    ...
    licensed_tokens =
        egress_tensor(offsetof(DFlashDecodeEgress, licensed_tokens), DType::I32,
                      {target_width, batch});
    licensed_counts =
    ...
    learned_draft_tokens       = layout.learned_draft_tokens.bind(backing);
    draft_tokens               = layout.draft_tokens.bind(backing);
    if (selector_top_k != 0) {
        learned_candidates      = layout.learned_candidates.bind(backing);
        learned_probabilities   = layout.learned_probabilities.bind(backing);
        proposal_candidates    = layout.proposal_candidates.bind(backing);
        proposal_probabilities = layout.proposal_probabilities.bind(backing);
    }
    verify_ids                 = layout.verify_ids.bind(backing);
    target_argmax              = layout.target_argmax.bind(backing);
    target_logits              = layout.target_logits.bind(backing);
...
RoundState::RoundState(DeviceSpan backing, const RoundStateLayout& layout) {
    ...
    if (layout.dflash_decode) {
        dflash_decode.emplace(backing, *layout.dflash_decode, layout.spec.batch_capacity,
                              layout.spec.draft_window, layout.spec.dflash_target_window,
                              layout.spec.dflash_selector_top_k);
    }
}
