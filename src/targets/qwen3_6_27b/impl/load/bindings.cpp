...
namespace ginfer::targets::qwen3_6_27b::detail {
namespace {
...
NumericFormat endpoint_format(WeightsProfile weights_profile) {
    ...
    case WeightsProfile::Qwen38GroupwiseIntDFlash2Q4:
    case WeightsProfile::Qwen38GroupwiseIntDFlash2W8:
    case WeightsProfile::Qwen36Nvfp4:
        return NumericFormat::W8G32_F16S;
    case WeightsProfile::Qwen38Nvfp4:
    case WeightsProfile::Qwen38Nvfp4DFlash2Q4:
    case WeightsProfile::Qwen38Nvfp4DFlash2W8:
        return NumericFormat::FP8_E4M3FN_ROW_BF16S;
    }
    throw std::invalid_argument("qwen3_6_27b: invalid weights profile");
}

bool has_dflash2(WeightsProfile weights_profile) noexcept {
    return weights_profile == WeightsProfile::Qwen38GroupwiseIntDFlash2Q4 ||
           weights_profile == WeightsProfile::Qwen38GroupwiseIntDFlash2W8 ||
           weights_profile == WeightsProfile::Qwen38Nvfp4DFlash2Q4 ||
           weights_profile == WeightsProfile::Qwen38Nvfp4DFlash2W8;
}
...
NumericFormat dflash2_matrix_format(WeightsProfile weights_profile) {
    ...
    case WeightsProfile::Qwen38Nvfp4DFlash2W8:
        return NumericFormat::W8G32_F16S;
    default:
        throw std::invalid_argument("Qwen3.8 DFlash2 profile has no companion matrix format");
    }
}

std::string dflash2_name(std::size_t layer, std::string_view suffix) {
...
DFlash2Plan bind_dflash2(artifact::Binder& binder, WeightsProfile weights_profile,
                         artifact::TensorPlacement placement) {
    constexpr std::uint64_t kHidden       = 5120;
    constexpr std::uint64_t kCaptured     = 25600;
    constexpr std::uint64_t kQuery        = 4096;
    constexpr std::uint64_t kKv           = 1024;
    constexpr std::uint64_t kIntermediate = 17408;
    constexpr std::uint64_t kDynamic      = 1280;
    constexpr std::uint64_t kRank         = 256;
    constexpr std::uint64_t kVocab        = 248320;
    const NumericFormat matrix            = dflash2_matrix_format(weights_profile);
    const auto bind = [&](std::string_view name, NumericFormat format,
                          std::initializer_list<std::uint64_t> shape) {
        return bind_weight(binder, name, format, shape, placement);
    };

    DFlash2Plan out;
    out.feature_projection = bind("dflash2/fc", matrix, {kHidden, kCaptured});
    out.hidden_norm        = bind("dflash2/hidden_norm", NumericFormat::BF16, {kHidden});
    for (std::size_t layer = 0; layer < kDFlash2Layers; ++layer) {
        DFlash2LayerPlan& target = out.layers[layer];
        target.input_layernorm =
            bind(dflash2_name(layer, "input_layernorm"), NumericFormat::BF16, {kHidden});
        target.q_proj = bind(dflash2_name(layer, "self_attn/q_proj"), matrix,
                             {kQuery, kHidden});
        target.k_proj = bind(dflash2_name(layer, "self_attn/k_proj"), matrix, {kKv, kHidden});
        target.v_proj = bind(dflash2_name(layer, "self_attn/v_proj"), matrix, {kKv, kHidden});
        target.o_proj = bind(dflash2_name(layer, "self_attn/o_proj"), matrix, {kHidden, kQuery});
        target.q_norm =
            bind(dflash2_name(layer, "self_attn/q_norm"), NumericFormat::BF16, {128});
        target.k_norm =
            bind(dflash2_name(layer, "self_attn/k_norm"), NumericFormat::BF16, {128});
        target.attention_conv_base = bind(dflash2_name(layer, "attention_conv/base_kernel"),
                                          NumericFormat::BF16, {2, 2, kHidden});
        target.attention_conv_proj =
            bind(dflash2_name(layer, "attention_conv/kernel_projection"), matrix,
                 {kDynamic, kHidden});
        target.post_attention_layernorm =
            bind(dflash2_name(layer, "post_attention_layernorm"), NumericFormat::BF16, {kHidden});
        target.mlp_gate =
            bind(dflash2_name(layer, "mlp/gate_proj"), matrix, {kIntermediate, kHidden});
        target.mlp_up =
            bind(dflash2_name(layer, "mlp/up_proj"), matrix, {kIntermediate, kHidden});
        target.mlp_down =
            bind(dflash2_name(layer, "mlp/down_proj"), matrix, {kHidden, kIntermediate});
        target.mlp_conv_base = bind(dflash2_name(layer, "mlp_conv/base_kernel"),
                                    NumericFormat::BF16, {2, 2, kHidden});
        target.mlp_conv_proj = bind(dflash2_name(layer, "mlp_conv/kernel_projection"), matrix,
                                    {kDynamic, kHidden});
    }
    out.final_norm = bind("dflash2/norm", NumericFormat::BF16, {kHidden});
    out.selector_hidden = bind("dflash2/candidate_selector/hidden_projection", matrix,
                               {kRank, kHidden});
    out.predecessor_codebook = bind("dflash2/candidate_selector/predecessor_codebook", matrix,
                                    {kVocab, kRank});
    out.successor_codebook = bind("dflash2/candidate_selector/successor_codebook", matrix,
                                  {kVocab, kRank});
    return out;
}
...
ArtifactLoadPlan bind_artifact(artifact::Binder& binder, WeightsProfile weights_profile,
    ...
    const bool dflash2_profile = has_dflash2(weights_profile);
    if (features.dflash() && !dflash2_profile) {
        throw std::invalid_argument(
            "Qwen3.8 DFlash2 requires a registered DFlash2 companion artifact profile");
    }
    ...
    if (dflash2_profile) {
        DFlash2Plan companion = bind_dflash2(
            binder, weights_profile,
            features.dflash() ? artifact::TensorPlacement::Device
                              : artifact::TensorPlacement::ValidateOnly);
        if (features.dflash()) { out.dflash2.emplace(std::move(companion)); }
    }
...
LoadedModelData::LoadedModelData(BindingPlan plan, artifact::MaterializedArtifact materialized)
    ...
    if (plan.dflash2) {
        ...
        target.hidden_norm = artifact::materialized_tensor(
            backing, source.hidden_norm.object, NumericFormat::BF16, {5120});
        for (std::size_t layer = 0; layer < kDFlash2Layers; ++layer) {
            const DFlash2LayerPlan& src = source.layers[layer];
            DFlash2LayerWeights& dst    = target.layers[layer];
            dst.input_layernorm = artifact::materialized_tensor(
                backing, src.input_layernorm.object, NumericFormat::BF16, {5120});
            dst.q_proj = materialized_weight(backing, src.q_proj, 4096, 5120);
            dst.k_proj = materialized_weight(backing, src.k_proj, 1024, 5120);
            dst.v_proj = materialized_weight(backing, src.v_proj, 1024, 5120);
            dst.o_proj = materialized_weight(backing, src.o_proj, 5120, 4096);
            dst.q_norm = artifact::materialized_tensor(
                backing, src.q_norm.object, NumericFormat::BF16, {128});
            dst.k_norm = artifact::materialized_tensor(
                backing, src.k_norm.object, NumericFormat::BF16, {128});
            dst.attention_conv_base = artifact::materialized_tensor(
                backing, src.attention_conv_base.object, NumericFormat::BF16, {5120, 2, 2});
            dst.attention_conv_proj =
                materialized_weight(backing, src.attention_conv_proj, 1280, 5120);
            dst.post_attention_layernorm = artifact::materialized_tensor(
                backing, src.post_attention_layernorm.object, NumericFormat::BF16, {5120});
            dst.mlp_gate = materialized_weight(backing, src.mlp_gate, 17408, 5120);
            dst.mlp_up   = materialized_weight(backing, src.mlp_up, 17408, 5120);
            dst.mlp_down = materialized_weight(backing, src.mlp_down, 5120, 17408);
            dst.mlp_conv_base = artifact::materialized_tensor(
                backing, src.mlp_conv_base.object, NumericFormat::BF16, {5120, 2, 2});
            dst.mlp_conv_proj = materialized_weight(backing, src.mlp_conv_proj, 1280, 5120);
        }
        target.final_norm = artifact::materialized_tensor(
            backing, source.final_norm.object, NumericFormat::BF16, {5120});
        target.selector_hidden =
            materialized_weight(backing, source.selector_hidden, 256, 5120);
        target.predecessor_codebook =
            materialized_weight(backing, source.predecessor_codebook, 248320, 256);
        target.successor_codebook =
            materialized_weight(backing, source.successor_codebook, 248320, 256);
    }
