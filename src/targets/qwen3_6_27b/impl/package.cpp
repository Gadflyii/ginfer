...
namespace ginfer::targets::qwen3_6_27b {
namespace {
...
Package::WeightsProfile Package::resolve_weights(const artifact::ArtifactIdentity& identity) {
    ...
    if (identity.model_id == qwen3_8_model_id &&
        identity.weights_id == "groupwise-int-dflash2-q4") {
        return WeightsProfile::Qwen38GroupwiseIntDFlash2Q4;
    }
    if (identity.model_id == qwen3_8_model_id &&
        identity.weights_id == "groupwise-int-dflash2-w8") {
        return WeightsProfile::Qwen38GroupwiseIntDFlash2W8;
    }
    ...
    if (identity.model_id == qwen3_8_model_id &&
        identity.weights_id == "nvfp4-dflash2-q4") {
        return WeightsProfile::Qwen38Nvfp4DFlash2Q4;
    }
    if (identity.model_id == qwen3_8_model_id &&
        identity.weights_id == "nvfp4-dflash2-w8") {
        return WeightsProfile::Qwen38Nvfp4DFlash2W8;
    }
    throw std::runtime_error("artifact identity '" + identity.model_id + "/" + identity.weights_id +
                             "' is not supported by target '" + std::string(target_key) + "'");
}

void Package::resolve_startup_options(WeightsProfile weights_profile, EngineOptions& options) {
    if (options.speculative.backend != SpeculativeBackend::Automatic) { return; }
    const bool dflash2 = weights_profile == WeightsProfile::Qwen38GroupwiseIntDFlash2Q4 ||
    ...
    options.speculative.backend      = dflash2 ? SpeculativeBackend::DFlash
                                              : SpeculativeBackend::None;
    options.speculative.draft_tokens = dflash2 ? 4U : 0U;
...
GpuRequirements Package::gpu_requirements(WeightsProfile weights_profile,
    ...
    throw std::logic_error("invalid Qwen3.6-27B weight profile");
}
