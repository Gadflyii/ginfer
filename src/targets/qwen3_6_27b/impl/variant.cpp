...
#define GINFER_QWEN36_VARIANT    ::ginfer::targets::qwen3_6_27b::detail::Variant
#define GINFER_QWEN36_RUNTIME_NS qwen3_6_27b_runtime
...
std::vector<GraphExecutionProfile> dflash_base_profiles(std::uint32_t capacity,
    ...
    const std::uint32_t block        = draft_window + 1U;
    ...
    // The target verification pass uses the same Text attention leaves as ordinary decode. Split
    // at every envelope boundary that changes their captured launch topology.
    for (const std::uint32_t visible_end : {128U, 512U, 2048U, 4096U, 8198U, 16390U, 32768U}) {
        add_target_boundary(visible_end);
    }
    if (draft_window >= 6U && draft_window <= kMaximumDFlashDraftTokens) {
        add_target_boundary(draft_window <= 11U ? 512U : 1024U);
    }
...
bool dflash_target_uses_chunked_small_t(std::uint32_t draft_window, std::uint32_t batch_size,
    ...
    const std::uint32_t tokens = draft_window + 1U;
    if (tokens <= 6U) { return false; }
    if (batch_size > 1U) { return true; }
    const std::uint32_t prompt_visible_limit = tokens <= 12U ? 512U : 1024U;
...
std::vector<GraphExecutionProfile> Variant::dflash_graph_profiles(std::uint32_t capacity,
    ...
    std::vector<GraphExecutionProfile> profiles =
        dflash_base_profiles(capacity, draft_window);
...
void Variant::gdn_norm_control_projection(const Tensor& residual, const Tensor& norm_weight,
    ...
    if (const auto* split =
        ...
        ops::gdn_norm_gating_proj(residual, norm_weight, eps, split->a_projection,
                                  ...
                                  hidden, g, beta, occupancy, stream);
        return;
    }
    const Weight& fused =
    ...
    ops::gdn_norm_gating_proj(residual, norm_weight, eps, fused, weights.a_log, weights.dt_bias,
                              workspace, hidden, g, beta, occupancy, stream);
}

void Variant::post_mixer(const Tensor& hidden, const PostMixerWeights& weights, Tensor& residual,
                         qwen3_6::TextPhase, WorkspaceArena& workspace, const OccupancyPolicy&,
                         cudaStream_t stream) {
    auto scope        = workspace.scope();
...
void Variant::mtp_post_mixer(const Tensor& hidden, const MtpPostMixerWeights& weights,
                             Tensor& residual, WorkspaceArena& workspace, const OccupancyPolicy&,
                             cudaStream_t stream) {
    auto scope     = workspace.scope();
    const int cols = hidden.ne[1];
...
std::size_t Variant::gdn_norm_control_projection_workspace_capacity_bytes(
    std::int32_t first, std::int32_t last, const OccupancyPolicy& occupancy) {
    return ops::gdn_norm_gating_proj_workspace_capacity_bytes(
        TextConfig::gdn_value_heads, TextConfig::hidden, first, last, occupancy);
}
