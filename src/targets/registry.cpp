...
void validate_options(const EngineOptions& options) {
    ...
    if (options.artifact_path.extension() != ".ginfer") {
        throw std::invalid_argument("GInfer accepts only .ginfer artifacts");
    }
    if (options.max_context == 0) {
        throw std::invalid_argument("Engine max_context must be nonzero");
    }
    if (options.prefill_chunk == 0 || options.prefill_chunk % 128U != 0) {
...
ConstructedTarget construct_registered(EngineOptions& options, DeviceContext& device,
                                       artifact::Reader& reader, Clock::time_point load_start,
                                       std::string_view target_key) {
    if (device.sm() != GINFER_COMPILED_CC) {
        throw std::invalid_argument("this GInfer image requires compute capability " +
                                    std::to_string(GINFER_COMPILED_CC) + ", device is " +
                                    std::to_string(device.sm()));
    }
    const auto& identity                          = reader.identity();
    const auto weights_profile                    = Target::resolve_weights(identity);
    Target::resolve_startup_options(weights_profile, options);
    const GpuRequirements gpu_requirements =
        Target::gpu_requirements(weights_profile, options);
    if (!gpu_requirements_satisfied(device.capability(), gpu_requirements)) {
        throw std::runtime_error(
            "artifact identity '" + identity.model_id + "/" + identity.weights_id +
            "' is not qualified for sm_" +
            std::to_string(device.capability().compute_capability) + " by target '" +
...
MuseGlimmer30BInstance::MuseGlimmer30BInstance(
    std::unique_ptr<LoadedMuseGlimmer30B> stable_loaded, runtime::KvCapacityResolution resolution,
...
ConstructedTarget construct_target(EngineOptions& options, DeviceContext& device) {
