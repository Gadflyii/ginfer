...
The retained tests protect current `.ginfer`, numerical operator, target, runtime-transaction,
...
- `test_ginfer_artifact_reader.cpp` — C++ framing, directory, encoded-size, payload-span, and
...
- `test_ginfer_bench_support.cpp` — product benchmark CLI, timing boundary, and schema-v12 reports;
...
```bash
cmake --build build --parallel --target ginfer_sampling_test
ctest --test-dir build -R ginfer_sampling_test --output-on-failure
```

Enable uniform floating-point error records when establishing or reviewing an Op criterion:

```bash
GINFER_OP_REPORT_STATS=1 \
  ctest --test-dir build -V -R '^ginfer_(rmsnorm|gqa_attention)_test$'
```
...
cmake --build build --parallel --target \
  ginfer_linear_q4_a16_test ginfer_linear_q5_a16_test \
  ginfer_linear_q6_a16_test ginfer_linear_w8_a16_test
ctest --test-dir build -R '^ginfer_linear_(q4|q5|q6|w8)_a16_test$' --output-on-failure
```
...
```bash
python3 -m pytest \
  tests/artifact tests/targets/qwen3_6_27b tests/targets/qwen3_8_27b \
  tests/targets/qwen3_6_35b_a3b \
  tests/test_bench_matrix.py tests/test_serve_corpus.py
```

The Python binding tests use `GINFER_QWEN3_6_27B_ARTIFACT` when set, otherwise they look for
`out/qwen3_6_27b.ginfer`. They report a pytest skip when neither path provides the real
artifact. The 35B-A3B reference binding test follows the same rule with
`GINFER_QWEN3_6_35B_A3B_ARTIFACT` and `out/qwen3_6_35b_a3b.ginfer`. The remaining Python
...
```bash
GINFER_QWEN3_6_27B_WEIGHTS=$PWD/out/qwen3_6_27b.ginfer \
  ctest --test-dir build -R ginfer_qwen3_6_27b_prefix_real_test --output-on-failure
```

Run the peer 35B-A3B route independently:

```bash
GINFER_QWEN3_6_35B_A3B_WEIGHTS=$PWD/out/qwen3_6_35b_a3b.ginfer \
  ctest --test-dir build -R ginfer_qwen3_6_35b_a3b_real_test --output-on-failure
```
...
```bash
./build/apps/ginfer-serve out/qwen3_6_27b.ginfer \
...
python3 tools/smoke/serve_thinking_preservation.py \
  --artifact out/qwen3_6_35b_a3b.ginfer --backend dflash
```
