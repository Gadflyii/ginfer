...
using ginfer::test::linear_add::ShapeCase;
using ginfer::test::linear_add::WeightFormat;
...
int bf16_a16_rejections() {
    ...
    failures += expect_invalid(
        [] {
            (void)ginfer::ops::linear_add_workspace_capacity_bytes(ginfer::QType::BF16_CTRL, 5120,
                                                                   6143, 1, 32);
        },
        "BF16_A16 LinearAdd workspace shape");

    ginfer::DeviceBuffer input(static_cast<std::size_t>(6144) * sizeof(std::uint16_t));
    ginfer::DeviceBuffer residual(static_cast<std::size_t>(5120) * sizeof(std::uint16_t));
    ginfer::DeviceBuffer weight_storage(256);
    ginfer::Weight weight{};
    weight.qtype  = ginfer::QType::BF16_CTRL;
    weight.layout = ginfer::QuantLayout::RowSplit;
    weight.qdata  = weight_storage.p;
    weight.n      = 5120;
    weight.k      = 6144;
    ginfer::Tensor x(input.p, ginfer::DType::BF16, {6144, 1});
    ginfer::Tensor out(residual.p, ginfer::DType::BF16, {5120, 1});
    ginfer::WorkspaceArena workspace(1);
    failures += expect_invalid([&] { ginfer::ops::linear_add(x, weight, out, workspace, nullptr); },
...
int bf16_a16_conformance() {
    ...
    return ginfer::test::linear_add::run_shape(
...
int main() {
    if (!ginfer::test::linear_add::cuda_available()) {
