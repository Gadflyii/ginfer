...
using namespace ginfer::test::direct_bf16_weight;
using namespace ginfer::test::input_projection;
...
int run_q4_q5() {
    ...
    int failures = 0;
    for (const std::int32_t tokens : {1, 2, 5, 8, 16, 17, 21, 48}) {
...
int main(int argc, char** argv) {
    ...
    const bool groupwise_only = argc == 2 && std::string_view(argv[1]) == "--groupwise-only";
    if (argc > 2 || (argc == 2 && !groupwise_only)) {
        std::cerr << "usage: ginfer_attn_input_proj_test [--groupwise-only]\n";
        return 2;
    }
    ...
    failures += run_q4_q5();
    if (groupwise_only) {
        std::cout << (failures == 0 ? "OK" : "FAIL") << " attn_input_proj groupwise\n";
