...
int run_case(int tokens, int context_length, InputProfile profile = InputProfile::Random,
    ...
    const std::size_t workspace_bytes =
        ops::bidirectional_gqa_attention_workspace_capacity_bytes(envelope, tokens, tokens, 1, test_occupancy());
    ...
    ops::bidirectional_gqa_attention(q_tensor, query_k_tensor, query_v_tensor, length_tensor,
                                     ...
                                     workspace, out_tensor, test_occupancy(), nullptr);
    cuda_synchronize();
...
int graph_mapping_replay_case() {
    ...
    ops::bidirectional_gqa_attention(q_tensor, query_k_tensor, query_v_tensor, length_tensor,
                                     valid_tensor, table_row_tensor, kScale, context, envelope,
                                     workspace, out_tensor, test_occupancy(), stream);
...
int batch_table_case() {
    ...
    for (int b = 0; b < batch; ++b) {
        ...
        ops::bidirectional_gqa_attention(q_row, query_k_row, query_v_row, length_row, valid_row,
                                         ...
                                         single_out_tensor, test_occupancy(), nullptr);
        cuda_synchronize();
    ...
    DeviceArena workspace(
        ops::bidirectional_gqa_attention_workspace_capacity_bytes(envelope, tokens, tokens, batch, test_occupancy()));
    ops::bidirectional_gqa_attention(q_tensor, query_k_tensor, query_v_tensor, length_tensor,
                                     valid_tensor, table_row_tensor, kScale, context, envelope,
                                     workspace, out_tensor, test_occupancy(), nullptr);
    cuda_synchronize();
...
int main() {
    ...
    const std::size_t interval =
        ops::bidirectional_gqa_attention_workspace_capacity_bytes(capacity_envelope, 1, 16, 1, test_occupancy());
    const std::size_t witness = std::max(
        ops::bidirectional_gqa_attention_workspace_capacity_bytes(capacity_envelope, 8, 8, 1, test_occupancy()),
        ops::bidirectional_gqa_attention_workspace_capacity_bytes(capacity_envelope, 16, 16, 1, test_occupancy()));
    if (interval != witness) {
        std::cerr << "bidirectional GQA interval capacity missed a token-band endpoint\n";
        ++failures;
    }
    try {
        (void)ops::bidirectional_gqa_attention_workspace_capacity_bytes(capacity_envelope, 9, 8, 1, test_occupancy());
