...
int run_projection_case(const Geometry& geometry, std::int32_t tokens, std::uint32_t seed) {
    ...
    const std::size_t workspace_bytes = ops::gdn_gating_proj_workspace_capacity_bytes(
        geometry.heads, geometry.hidden, tokens, tokens, test_occupancy());
    ...
    if (geometry.parent_weight) {
        ...
        ops::gdn_gating_proj(tensor_x, parent, tensor_a_log, tensor_dt_bias, workspace, tensor_g,
                             tensor_beta, test_occupancy(), nullptr);
    } else {
        ...
        ops::gdn_gating_proj(tensor_x, weight_a, weight_b, tensor_a_log, tensor_dt_bias, workspace,
                             tensor_g, tensor_beta, test_occupancy(), nullptr);
    }
    cuda_synchronize();
...
int verify_workspace_capacity_contract(const Geometry& geometry, std::int32_t last) {
    const std::size_t interval = ops::gdn_gating_proj_workspace_capacity_bytes(
        geometry.heads, geometry.hidden, 1, last, test_occupancy());
    ...
    int failures = 0;
    if (interval != witness) {
        std::cerr << geometry.label << ": GDN control interval missed an exact-T plan\n";
        ++failures;
    }
    const std::size_t norm_interval = ops::gdn_norm_gating_proj_workspace_capacity_bytes(
        geometry.heads, geometry.hidden, 1, 64, test_occupancy());
    std::size_t norm_witness = 0;
    for (std::int32_t tokens = 1; tokens <= 64; ++tokens) {
        norm_witness =
            std::max(norm_witness, ops::gdn_norm_gating_proj_workspace_capacity_bytes(
                                       geometry.heads, geometry.hidden, tokens, tokens,
                                       test_occupancy()));
    }
    if (norm_interval != norm_witness) {
        std::cerr << geometry.label << ": GDN norm/control interval missed an exact-T plan\n";
        ++failures;
    }
    return failures;
}
...
int main() {
    ...
    // Execute both sides of the architecture-specific cooperative-residency frontier on the
    // hardware image that owns it. The host plan test covers every later frontier without making
    // the numerical suite repeat several long-context oracle cases on every GPU.
    if (test_occupancy().sm_arch == SmArch::Ampere86) {
        for (const std::int32_t tokens : {768, 769}) {
            failures += run_projection_case(kQwen27, tokens,
                                            0x5000u + static_cast<std::uint32_t>(tokens));
        }
        for (const std::int32_t tokens : {960, 961}) {
            failures += run_projection_case(kQwen35, tokens,
                                            0x6000u + static_cast<std::uint32_t>(tokens));
        }
    } else if (test_occupancy().sm_arch == SmArch::Ada89) {
        for (const std::int32_t tokens : {1280, 1281}) {
            failures += run_projection_case(kQwen27, tokens,
