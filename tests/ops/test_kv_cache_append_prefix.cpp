...
int cyclic_2048_wrap_case() {
    constexpr int capacity       = 2048;
    constexpr int tokens         = 8;
    constexpr int commit_count   = 7;
    constexpr int lane_capacity  = 3;
    constexpr int selected_lane  = 1;
    constexpr int first_position = capacity - 3;
    const std::size_t input_count = static_cast<std::size_t>(kHeadDim) * kKVHeads * tokens;
    const std::size_t lane_cache_count =
        static_cast<std::size_t>(kHeadDim) * capacity * kKVHeads;
    const std::size_t cache_count = lane_cache_count * lane_capacity;
    const auto host_k             = patterned_bits(input_count, 0x2048a001u);
    const auto host_v             = patterned_bits(input_count, 0x2048b002u);
    const auto initial_k          = patterned_bits(cache_count, 0x2048c003u);
    const auto initial_v          = patterned_bits(cache_count, 0x2048d004u);
    ...
    for (int token = 0; token < commit_count; ++token) {
        const int slot = positions[static_cast<std::size_t>(token)] % capacity;
        for (int head = 0; head < kKVHeads; ++head) {
            for (int d = 0; d < kHeadDim; ++d) {
                const std::size_t source = input_index(d, head, token);
                    ...
                    static_cast<std::size_t>(kHeadDim) *
                        (static_cast<std::size_t>(slot) +
                         static_cast<std::size_t>(capacity) *
                             (static_cast<std::size_t>(head) +
                              static_cast<std::size_t>(kKVHeads) * selected_lane));
                expected_k[destination] = host_k[source];
                expected_v[destination] = host_v[source];
            }
        }
    }

    GuardedDeviceBuffer d_k(input_count * sizeof(std::uint16_t));
    GuardedDeviceBuffer d_v(input_count * sizeof(std::uint16_t));
    GuardedDeviceBuffer d_positions(positions.size() * sizeof(std::int32_t));
    GuardedDeviceBuffer d_count(sizeof(std::int32_t));
    GuardedDeviceBuffer d_lane(sizeof(std::int32_t));
    GuardedDeviceBuffer cache_k(cache_count * sizeof(std::uint16_t));
    GuardedDeviceBuffer cache_v(cache_count * sizeof(std::uint16_t));
    d_k.copy_from_host(host_k.data(), d_k.bytes());
    d_v.copy_from_host(host_v.data(), d_v.bytes());
    d_positions.copy_from_host(positions.data(), d_positions.bytes());
    d_count.copy_from_host(&commit_count, sizeof(commit_count));
    d_lane.copy_from_host(&selected_lane, sizeof(selected_lane));
    cache_k.copy_from_host(initial_k.data(), cache_k.bytes());
    cache_v.copy_from_host(initial_v.data(), cache_v.bytes());

    Tensor k(d_k.data(), DType::BF16, {kHeadDim, kKVHeads, tokens, 1});
    Tensor v(d_v.data(), DType::BF16, {kHeadDim, kKVHeads, tokens, 1});
    Tensor position_tensor(d_positions.data(), DType::I32, {tokens, 1});
    Tensor count_tensor(d_count.data(), DType::I32, {1});
    Tensor lane_tensor(d_lane.data(), DType::I32, {1});
    CyclicKVCacheLayerView cache{
        .k = Tensor(cache_k.data(), DType::BF16,
                    {kHeadDim, capacity, kKVHeads, lane_capacity}),
        .v = Tensor(cache_v.data(), DType::BF16,
                    {kHeadDim, capacity, kKVHeads, lane_capacity}),
        .capacity        = capacity,
    ...
    constexpr const char* label = "kv_cache_append_prefix cyclic capacity=2048 wrap";
    int failures =
        verify_exact((std::string(label) + " cache k").c_str(),
    ...
    failures += verify_exact((std::string(label) + " input k unchanged").c_str(),
                             from_device<std::uint16_t>(d_k.data(), input_count), host_k);
    failures += verify_exact((std::string(label) + " input v unchanged").c_str(),
                             from_device<std::uint16_t>(d_v.data(), input_count), host_v);
    failures += verify_exact((std::string(label) + " positions unchanged").c_str(),
                             from_device<std::int32_t>(d_positions.data(), positions.size()),
                             positions);
    failures += verify_exact((std::string(label) + " count unchanged").c_str(),
                             from_device<std::int32_t>(d_count.data(), 1), {commit_count});
    failures += verify_exact((std::string(label) + " lane unchanged").c_str(),
                             from_device<std::int32_t>(d_lane.data(), 1), {selected_lane});
    failures += d_k.verify_guards(std::string(label) + " input k guards");
    failures += d_v.verify_guards(std::string(label) + " input v guards");
    failures += d_positions.verify_guards(std::string(label) + " positions guards");
    failures += d_count.verify_guards(std::string(label) + " count guards");
    failures += d_lane.verify_guards(std::string(label) + " lane guards");
    failures += cache_k.verify_guards(std::string(label) + " cache k guards");
    failures += cache_v.verify_guards(std::string(label) + " cache v guards");
    return failures;
}
...
int main() {
    ...
    failures += cyclic_2048_wrap_case();
