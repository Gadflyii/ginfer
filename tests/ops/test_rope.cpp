...
int run_pair_case(const Geometry& geometry, int q_heads, int k_heads, int first_position,
    ...
    ops::rope(position_tensor, geometry.rotary_dim, geometry.theta, q_tensor, k_tensor, test_occupancy(), nullptr);
    cuda_synchronize();
...
int run_single_case(const Geometry& geometry, int heads, int first_position, int padding = 0) {
    ...
    ops::rope(position_tensor, geometry.rotary_dim, geometry.theta, tensor, test_occupancy(), nullptr);
    cuda_synchronize();
...
int run_vision_packed_case() {
    ...
    ops::rope(position_tensor, kHeadDim, kVisionTheta, q_tensor, k_tensor, test_occupancy(), nullptr);
    cuda_synchronize();
