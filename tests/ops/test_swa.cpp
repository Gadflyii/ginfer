...
int run_case(int tokens, int context_length, InputProfile profile = InputProfile::Random,
    ...
    const std::size_t workspace_bytes =
        ops::swa_workspace_capacity_bytes(envelope, tokens, tokens, 1, window);
...
int run_batch_case() {
    ...
    DeviceArena workspace(
        ops::swa_workspace_capacity_bytes(envelope, tokens, tokens, batch, kWindow));
    ...
    DeviceArena single_workspace(
        ops::swa_workspace_capacity_bytes(envelope, tokens, tokens, 1, kWindow));
...
int main() {
    ...
    const std::size_t interval =
        ops::swa_workspace_capacity_bytes(capacity_envelope, 1, 16, 1, kWindow);
    const std::size_t endpoint =
        ops::swa_workspace_capacity_bytes(capacity_envelope, 16, 16, 1, kWindow);
    if (interval != endpoint) {
        std::cerr << "swa interval capacity did not resolve to its monotonic endpoint\n";
        ++failures;
    }
    try {
        (void)ops::swa_workspace_capacity_bytes(capacity_envelope, 0, 16, 1, kWindow);
    ...
    failures += run_case(1, 2048, InputProfile::Random, 2048, 2048);
    failures += run_case(2, 2048, InputProfile::WindowBoundary, 2048, 2048);
    failures += run_case(2, 4098, InputProfile::Random, 4098, 2048);
