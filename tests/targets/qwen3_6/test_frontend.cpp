...
using Frontend          = ginfer::targets::qwen3_6::Frontend;
using FrontendFactory   = ginfer::targets::qwen3_6::FrontendTestAccess;
using FrontendResources = ginfer::targets::qwen3_6::FrontendResources;
using PublishedOutput   = ginfer::targets::qwen3_6::PublishedOutput;
namespace fi            = ginfer::targets::qwen3_6::frontend_internal;
...
const std::string& thinking_toggle_template_source() {
    static const std::string source = read_template_fixture(
        GINFER_SOURCE_DIR "/tests/fixtures/frontend/thinking_toggle_chat_template.jinja");
    return source;
}

const std::string& reasoning_effort_template_source() {
    static const std::string source = read_template_fixture(
        GINFER_SOURCE_DIR "/tests/fixtures/frontend/reasoning_effort_chat_template.jinja");
    return source;
}
...
const std::string& official_model_root() {
    static const std::string root = [] {
        const char* value = std::getenv("GINFER_QWEN3_6_27B_SOURCE");
        if (value == nullptr || *value == '\0') {
            throw std::runtime_error(
                "GINFER_QWEN3_6_27B_SOURCE is required for the official-resource test");
        }
...
const fi::Tokenizer& official_tokenizer() {
    static const std::string tokenizer_json =
        read_file((official_model_root() + "/tokenizer.json").c_str());
    static const std::string tokenizer_config_json =
        read_file((official_model_root() + "/tokenizer_config.json").c_str());
    static const std::string generation_config_json =
        read_file((official_model_root() + "/generation_config.json").c_str());
...
ginfer::PromptInput image_text_input(std::vector<std::uint8_t> bytes, std::string text,
                                     std::string source_name) {
    ginfer::MessagePart image;
    image.kind              = ginfer::MessagePartKind::Media;
    image.media.kind        = ginfer::MediaKind::Image;
    image.media.bytes       = std::move(bytes);
    image.media.media_type  = "image/x-portable-pixmap";
    image.media.source_name = std::move(source_name);

    ginfer::ChatMessage message;
    message.role = ginfer::ChatRole::User;
    message.parts.push_back(std::move(image));
    if (!text.empty()) {
        message.parts.push_back(ginfer::MessagePart{
            .kind = ginfer::MessagePartKind::Text, .text = std::move(text), .media = {}});
    }
...
ginfer::PromptInput image_input() {
    ginfer::MessagePart image;
    image.kind              = ginfer::MessagePartKind::Media;
    image.media.kind        = ginfer::MediaKind::Image;
...
std::string channel_text(const PublishedOutput& output, ginfer::OutputChannel channel) {
    std::string result;
...
fi::ChatMessage chat_message(ginfer::ChatRole role, std::string content) {
...
int test_official_chat_template() {
    int failures = 0;
    failures += check(render_chat_text({chat_message(ginfer::ChatRole::User, "hello")}) ==
                          "<|im_start|>user\nhello<|im_end|>\n<|im_start|>assistant\n<think>\n",
    ...
    failures +=
        check(render_chat_text({chat_message(ginfer::ChatRole::System, "  be concise  "),
                                chat_message(ginfer::ChatRole::User, "hello")},
                               no_generation) == "<|im_start|>system\nbe concise<|im_end|>\n"
                                                 "<|im_start|>user\nhello<|im_end|>\n",
              "leading system prompt differs from the official template");
    failures += check(render_chat_text({chat_message(ginfer::ChatRole::System, ""),
                                        chat_message(ginfer::ChatRole::User, "hello")},
                                       no_generation) ==
                          "<|im_start|>system\n<|im_end|>\n<|im_start|>user\nhello<|im_end|>\n",
                      "empty leading system prompt differs from the official template");

    fi::ChatMessage tool_assistant = chat_message(ginfer::ChatRole::Assistant, "");
    tool_assistant.tool_calls.push_back(
        {.id = "", .name = "f", .arguments_json = R"({"flag":true,"nested":{"x":[1,2]}})"});
    failures += check(render_chat_text({chat_message(ginfer::ChatRole::User, "hi"), tool_assistant},
                                       no_generation) ==
                          "<|im_start|>user\nhi<|im_end|>\n"
                          "<|im_start|>assistant\n<think>\n\n</think>\n\n"
                          "<tool_call>\n<function=f>\n<parameter=flag>\ntrue\n</parameter>\n"
    ...
    failures +=
        check(render_chat_text({chat_message(ginfer::ChatRole::User, "q1"),
                                chat_message(ginfer::ChatRole::Assistant,
                                             "<think>\nold thought\n</think>\n\nold answer"),
                                chat_message(ginfer::ChatRole::User, "q2")},
    ...
    fi::ChatMessage lookup = chat_message(ginfer::ChatRole::Assistant, "");
    lookup.tool_calls.push_back(
        {.id = "", .name = "lookup", .arguments_json = R"({"city":"Paris"})"});
    failures +=
        check(render_chat_text({chat_message(ginfer::ChatRole::User, "weather?"), lookup,
                                chat_message(ginfer::ChatRole::Tool, "sunny"),
                                chat_message(ginfer::ChatRole::Tool, "20C"),
                                chat_message(ginfer::ChatRole::User, "thanks")},
                               no_generation) ==
                  "<|im_start|>user\nweather?<|im_end|>\n"
                  "<|im_start|>assistant\n<tool_call>\n<function=lookup>\n"
                  "<parameter=city>\nParis\n</parameter>\n</function>\n</tool_call><|im_end|>\n"
    ...
    const std::string tools_rendered =
        render_chat_text({chat_message(ginfer::ChatRole::System, "be exact"),
                          chat_message(ginfer::ChatRole::User, "hi")},
                         tools);
    failures += check(
    ...
    failures += check(throws_invalid_argument([&] {
                          (void)render_chat({chat_message(ginfer::ChatRole::System, "only")},
                                            no_generation);
                      }),
...
int test_ordered_instruction_turns() {
    ...
    const std::string leading_developer =
        render_chat_text({chat_message(ginfer::ChatRole::Developer, "policy"),
                          chat_message(ginfer::ChatRole::User, "hi")},
                         no_generation);
    ...
    const std::string late_system =
        render_chat_text({chat_message(ginfer::ChatRole::User, "hi"),
                          chat_message(ginfer::ChatRole::System, "  current diagnostics  ")},
                         no_generation);
    ...
    failures += check(
        render_chat_text({chat_message(ginfer::ChatRole::User, "hi"),
                          chat_message(ginfer::ChatRole::Developer, "  current diagnostics  ")},
    ...
    const std::string stable_history =
        render_chat_text({chat_message(ginfer::ChatRole::System, "stable policy"),
                          chat_message(ginfer::ChatRole::User, "hi")},
                         no_generation);
    ...
    const fi::RenderedChat generated =
        render_chat({chat_message(ginfer::ChatRole::User, "hi"),
                     chat_message(ginfer::ChatRole::System, "current diagnostics")});
    ...
    failures +=
        check(header != std::string::npos && generated.rewrite_checkpoint &&
                  generated.rewrite_checkpoint->kind ==
                      ginfer::targets::qwen3_6::RewriteCheckpointKind::TurnClosure &&
    ...
    fi::ChatMessage invalid = chat_message(ginfer::ChatRole::System, "diagnostics");
    invalid.tool_calls.push_back({.id = "call", .name = "f", .arguments_json = "{}"});
    failures += check(throws_invalid_argument([&] {
                          (void)render_chat({chat_message(ginfer::ChatRole::User, "hi"), invalid},
                                            no_generation);
                      }),
                      "system turn carrying assistant tool metadata was accepted");

    fi::ChatMessage media_instruction = chat_message(ginfer::ChatRole::Developer, "diagnostics");
    media_instruction.parts.push_back(fi::ChatPart::image({}));
    failures +=
        check(throws_invalid_argument([&] {
                  (void)render_chat({chat_message(ginfer::ChatRole::User, "hi"), media_instruction},
                                    no_generation);
              }),
              "developer turn carrying media was accepted");

    fi::ChatMessage invalid_role = chat_message(ginfer::ChatRole::User, "bad");
    invalid_role.role            = static_cast<ginfer::ChatRole>(255);
    failures +=
        check(throws_invalid_argument([&] {
                  (void)render_chat({chat_message(ginfer::ChatRole::User, "hi"), invalid_role},
                                    no_generation);
              }),
...
int test_reasoning_effort_chat_template() {
    ...
    const ginfer::PromptCapabilities toggle_capabilities =
        thinking_toggle_template().capabilities();
    const ginfer::PromptCapabilities effort_capabilities =
    ...
    failures += check(
        effort_capabilities.enable_thinking && effort_capabilities.reasoning_effort.low &&
            ...
            effort_capabilities.reasoning_effort.default_effort == ginfer::ReasoningEffort::XHigh,
        "reasoning-effort template did not advertise its complete capability set");

    const auto render_effort = [](ginfer::ReasoningEffort effort) {
        fi::ChatRenderOptions options;
        options.reasoning_effort = effort;
        return reasoning_effort_template()
            .render({chat_message(ginfer::ChatRole::User, "hello")}, options)
            .text;
    };
    const std::string tail = "<|im_start|>user\nhello<|im_end|>\n<|im_start|>assistant\n<think>\n";
    failures += check(
        reasoning_effort_template().render({chat_message(ginfer::ChatRole::User, "hello")}).text ==
            "<|im_start|>system\n" + std::string(xhigh_instructions) + "<|im_end|>\n" + tail,
        "reasoning-effort template did not apply its xhigh default");
    failures +=
        check(render_effort(ginfer::ReasoningEffort::Low) ==
                  "<|im_start|>system\n" + std::string(low_instructions) + "<|im_end|>\n" + tail,
              "low reasoning effort did not render the official instruction");
    failures += check(render_effort(ginfer::ReasoningEffort::Medium) == tail,
    ...
    failures += check(reasoning_effort_template()
                              .render({chat_message(ginfer::ChatRole::System, ""),
                                       chat_message(ginfer::ChatRole::User, "hello")},
                                      disabled)
                              .text == "<|im_start|>user\nhello<|im_end|>\n"
                                       "<|im_start|>assistant\n<think>\n\n</think>\n\n",
                      "disabled thinking did not suppress effort and an empty system turn");
    disabled.reasoning_effort = ginfer::ReasoningEffort::Low;
    failures += check(throws_invalid_argument([&] {
                          (void)reasoning_effort_template().render(
                              {chat_message(ginfer::ChatRole::User, "hello")}, disabled);
                      }),
                      "reasoning effort and disabled thinking were accepted together");

    fi::ChatRenderOptions unsupported;
    unsupported.reasoning_effort = ginfer::ReasoningEffort::Low;
    failures += check(throws_invalid_argument([&] {
                          (void)thinking_toggle_template().render(
                              {chat_message(ginfer::ChatRole::User, "hello")}, unsupported);
                      }),
                      "thinking-toggle template accepted reasoning effort");

    fi::ChatMessage previous   = chat_message(ginfer::ChatRole::Assistant, "old answer");
    previous.reasoning_content = "old thought";
    fi::ChatRenderOptions no_generation;
    no_generation.add_generation_prompt = false;
    no_generation.reasoning_effort      = ginfer::ReasoningEffort::Medium;
    const std::string preserved         = reasoning_effort_template()
                                      .render({chat_message(ginfer::ChatRole::User, "q1"), previous,
                                               chat_message(ginfer::ChatRole::User, "q2")},
                                              no_generation)
                                      .text;
    failures += check(
    ...
    fi::ChatMessage empty_arguments = chat_message(ginfer::ChatRole::Assistant, "");
    empty_arguments.tool_calls.push_back({.id = "", .name = "f", .arguments_json = ""});
    failures += check(
        reasoning_effort_template()
            .render({chat_message(ginfer::ChatRole::User, "call"), empty_arguments}, no_generation)
...
int test_rewrite_checkpoint_trace() {
    const std::string assistant_header = "<|im_start|>assistant\n";
    fi::ChatMessage first              = chat_message(ginfer::ChatRole::Assistant, "");
    first.reasoning_content            = "first thought";
    first.parts.front().text           = "first answer";
    fi::ChatMessage second             = chat_message(ginfer::ChatRole::Assistant, "");
    second.reasoning_content           = "second thought";
    second.parts.front().text          = "second answer";

    const std::vector<fi::ChatMessage> tool_loop{
        chat_message(ginfer::ChatRole::User, "question"), first,
        chat_message(ginfer::ChatRole::Tool, "result one"), second,
        chat_message(ginfer::ChatRole::Tool, "result two")};
    ...
    failures += check(preserved_header != std::string::npos && preserved.rewrite_checkpoint &&
                          preserved.rewrite_checkpoint->kind ==
                              ginfer::targets::qwen3_6::RewriteCheckpointKind::ResponseReplay &&
    ...
    next_turn.push_back(chat_message(ginfer::ChatRole::User, "next question"));
    ...
    const fi::RenderedChat no_assistant =
        render_chat({chat_message(ginfer::ChatRole::User, "question")}, no_generation);
    ...
    const fi::RenderedChat wrapped = render_chat(
        {chat_message(ginfer::ChatRole::User, "question"), first,
         chat_message(ginfer::ChatRole::User, "<tool_response>compat result</tool_response>"),
         second},
        no_generation);
...
int test_official_resource_guards() {
    ...
    const ginfer::PromptCapabilities capabilities = effort_frontend.prompt_capabilities();
    failures +=
        check(capabilities.reasoning_effort.low && capabilities.reasoning_effort.medium &&
                  capabilities.reasoning_effort.xhigh &&
                  capabilities.reasoning_effort.default_effort == ginfer::ReasoningEffort::XHigh,
...
int test_text_and_image_prepare(const Frontend& frontend) {
    ginfer::ChatMessage text_message;
    text_message.role = ginfer::ChatRole::User;
    text_message.parts.push_back(
        ginfer::MessagePart{.kind = ginfer::MessagePartKind::Text, .text = "x", .media = {}});
    ginfer::PromptInput text_input;
    ...
    const std::vector<ginfer::TokenId> expected{248045, 30, 0, 248046, 32, 248045, 31, 248068, 32};
    int failures =
    ...
    ginfer::ChatMessage preserved_message;
    preserved_message.role = ginfer::ChatRole::User;
    preserved_message.parts.push_back(
        ginfer::MessagePart{.kind = ginfer::MessagePartKind::Text, .text = "x", .media = {}});
    ginfer::PromptInput preserved_input;
    ...
    ginfer::ChatMessage nonthinking_message;
    nonthinking_message.role = ginfer::ChatRole::User;
    nonthinking_message.parts.push_back(
        ginfer::MessagePart{.kind = ginfer::MessagePartKind::Text, .text = "x", .media = {}});
    ginfer::PromptInput nonthinking_input;
    ...
    image.media.media_type  = "image/x-portable-pixmap";
    image.media.source_name = "inline.ppm";
    ginfer::ChatMessage image_message;
    image_message.role = ginfer::ChatRole::User;
...
int test_media_admission_uses_aggregate_resources(const Frontend& frontend) {
    ...
    for (std::size_t index = 0; index < kMediaItems; ++index) {
        ginfer::OwnedMedia media;
        media.kind        = ginfer::MediaKind::Image;
        media.bytes       = bytes;
        media.media_type  = "image/x-portable-pixmap";
        media.source_name = "aggregate-" + std::to_string(index) + ".ppm";
        message.parts.push_back(ginfer::MessagePart{
            .kind = ginfer::MessagePartKind::Media, .text = {}, .media = std::move(media)});
    }
    ginfer::PromptInput input;
    input.messages.push_back(std::move(message));
    ...
    auto cache = std::make_shared<fi::MediaPreprocessCache>(ginfer::kDefaultMediaCacheBytes,
                                                            ginfer::kDefaultMediaLiveBytes);
    fi::Processor processor(official_tokenizer(), thinking_toggle_template(), options,
                            std::move(cache));
    fi::ChatMessage internal_message;
    internal_message.role = ginfer::ChatRole::User;
...
int test_video_prepare(const Frontend& frontend) {
    ginfer::MessagePart video;
    video.kind              = ginfer::MessagePartKind::Media;
    video.media.kind        = ginfer::MediaKind::Video;
    ...
    if (!prepared_data.vision_items.empty()) {
        const auto& item = prepared_data.vision_items.front();
        failures +=
            check(item.modality == ginfer::targets::qwen3_6::PromptModality::Video &&
...
int test_cross_round_stop(const Frontend& frontend) {
    ...
    stop.strings.push_back(ginfer::StopString{.text = "STOP"});
    auto session = frontend.make_output_session(prompt, stop);

    const auto first_decision =
        session.preview(std::array<ginfer::TokenId, 1>{1}, 2, ginfer::FinishReason::OutputLimit);
    ...
    failures += check(channel_text(first, ginfer::OutputChannel::Content) == "hello",
                      "cross-round stop did not retain the ambiguous suffix");

    const auto second_decision =
        session.preview(std::array<ginfer::TokenId, 1>{2}, 1, ginfer::FinishReason::OutputLimit);
    failures += check(second_decision.accepted_tokens == 1 &&
                          second_decision.finish_reason == ginfer::FinishReason::StopString,
...
int test_same_token_stop_priority(const Frontend& frontend) {
    ...
    ginfer::StopPolicy stop;
    stop.strings = {
        ginfer::StopString{.text = "tail", .include_in_output = true},
        ginfer::StopString{.text = "OPtail"},
        ginfer::StopString{.text = "OP", .include_in_output = true},
    };
    auto session = frontend.make_output_session(prompt, stop);
    const auto decision =
        session.preview(std::array<ginfer::TokenId, 1>{2}, 2, ginfer::FinishReason::OutputLimit);
    int failures      = check(decision.accepted_tokens == 1 &&
                                  decision.finish_reason == ginfer::FinishReason::StopString,
...
int test_terminal_flush(const Frontend& frontend) {
    ...
    const auto terminal = session.preview_terminal(ginfer::FinishReason::Cancelled);
    failures += check(terminal.accepted_tokens == 0 &&
                          terminal.finish_reason == ginfer::FinishReason::Cancelled,
                      "between-round terminal preview returned the wrong decision");
    const auto flushed = session.commit_preview();
    failures += check(channel_text(flushed, ginfer::OutputChannel::Content) == "ST",
...
int test_reasoning_split(const Frontend& frontend) {
    ...
    const std::array<ginfer::TokenId, 2> tokens{3, 4};
    const auto decision = session.preview(tokens, 2, ginfer::FinishReason::OutputLimit);
    int failures        = check(decision.accepted_tokens == 2 &&
                                    decision.finish_reason == ginfer::FinishReason::OutputLimit,
                                "reasoning output did not finish at the requested token limit");
    const auto output   = session.commit_preview();
    failures += check(channel_text(output, ginfer::OutputChannel::Reasoning) == "thought",
                      "reasoning channel did not remove the close marker");
    failures += check(channel_text(output, ginfer::OutputChannel::Content) == "answer",
...
int test_utf8_and_hidden_eos(const Frontend& frontend) {
    ...
    for (const ginfer::TokenId token : {10, 11}) {
        const auto decision = session.preview(std::array<ginfer::TokenId, 1>{token}, remaining,
    ...
    const auto complete_decision = session.preview(std::array<ginfer::TokenId, 1>{12}, remaining,
                                                   ginfer::FinishReason::OutputLimit);
    ...
    failures += check(channel_text(complete, ginfer::OutputChannel::Content) == "中",
    ...
    const auto eos_decision = eos_session.preview(std::array<ginfer::TokenId, 1>{6}, 2,
                                                  ginfer::FinishReason::OutputLimit);
    failures += check(eos_decision.accepted_tokens == 1 &&
                          eos_decision.finish_reason == ginfer::FinishReason::StopToken,
    ...
    const auto raw_eos_decision = raw_session.preview(std::array<ginfer::TokenId, 1>{6}, 2,
                                                      ginfer::FinishReason::OutputLimit);
    failures += check(raw_eos_decision.accepted_tokens == 1 &&
                          raw_eos_decision.finish_reason == ginfer::FinishReason::StopToken,
                      "raw EOS token did not end generation");
    const auto raw_eos = raw_session.commit_preview();
    failures += check(channel_text(raw_eos, ginfer::OutputChannel::Content) == "<eos>",
...
int test_media_cache_reuses_immutable_payload() {
    ...
    const ginfer::MediaCacheSummary cache = frontend.media_cache_summary();
...
int test_media_payload_outlives_frontend_cache() {
    ginfer::targets::qwen3_6::PreparedPrompt survivor;
    {
        const Frontend frontend = FrontendFactory::create_component(resources());
...
int test_media_preparation_cancellation() {
    const Frontend frontend = FrontendFactory::create_component(resources());
    ginfer::PreparationControl control{
        .deadline     = {},
        .cancellation = ginfer::CancellationView([] { return true; }),
    };
    try {
        (void)frontend.prepare(image_input(), control);
    } catch (const ginfer::RequestError& error) {
        return check(error.kind() == ginfer::RequestErrorKind::Cancelled &&
