...
#include <ginfer/targets/qwen3_6/decoder_state.h>
#include <ginfer/targets/qwen3_6/hybrid_topology.h>
#include <ginfer/targets/qwen3_6/mtp_alignment.h>
#include <ginfer/targets/qwen3_6/round_state.h>
...
namespace q36 = ginfer::targets::qwen3_6;
...
q36::DecoderStateSpec decoder_spec(ginfer::DType dtype, bool mtp) {
    return q36::DecoderStateSpec{
        ...
        .kv_dtype                  = dtype,
        .kv_quant_group            = dtype == ginfer::DType::I8 ? q36::kKvQuantGroup : 0,
        .enable_mtp                = mtp,
        ...
        .linear_attention =
            {
                ...
                .key_head_dim   = 6,
                .slot_count     = 4,
                .conv_dtype     = ginfer::DType::BF16,
            },
    };
}

void test_decoder_layout() {
    ginfer::LayoutBuilder bf16_builder;
    const q36::DecoderStateLayout bf16 =
        q36::plan_decoder_state(bf16_builder, decoder_spec(ginfer::DType::BF16, false));
    ...
    expect(std::all_of(bf16.text_kv.pool.planes.begin(), bf16.text_kv.pool.planes.end(),
                       [](const ginfer::PagedKVPlaneLayout& plane) {
                           return plane.spec.dtype == ginfer::DType::BF16;
                       }),
    ...
    ginfer::LayoutBuilder int8_builder;
    const q36::DecoderStateLayout int8 =
        q36::plan_decoder_state(int8_builder, decoder_spec(ginfer::DType::I8, true));
    (void)int8_builder.finish(256);
    expect(int8.text_kv.pool.planes.size() == 8 &&
               int8.text_kv.pool.planes[2].spec.dtype == ginfer::DType::FP16 &&
               int8.text_kv.pool.planes[3].spec.dtype == ginfer::DType::FP16,
    ...
    expect(int8.mtp_kv && int8.mtp_kv->pool.planes[2].spec.dtype == ginfer::DType::FP16 &&
               int8.mtp_kv->pool.planes[3].spec.dtype == ginfer::DType::FP16,
...
void test_round_layout() {
    ...
    const ginfer::TensorRegion exact_prefill =
        builder.add_tensor(ginfer::DType::BF16, {32, 16}, 256, "exact prefill hidden");
    ...
    ginfer::LayoutBuilder speculative_builder;
    ...
    expect(dflash.logits.shape[1] == 1 && dflash.dflash_prefill.has_value() &&
               ...
               dflash.dflash_decode->learned_draft_tokens.shape[0] == 4 &&
               dflash.dflash_decode->draft_tokens.shape[0] == 15 &&
               dflash.dflash_decode->target_positions.shape[0] == 16,
           "K=4 learned/K=15 target DFlash storage is backend-owned");
...
void append_text_token(q36::PreparedPromptData& prompt, ginfer::TokenId token,
...
void test_prefix_identity() {
    ...
    std::vector<ginfer::TokenId> ledger = original.token_ids;
...
void test_context_copy() {
    using q36::detail::ContextCopyIndex;

    ContextCopyIndex index;
    std::vector<ginfer::TokenId> unique;
    for (ginfer::TokenId token = 1; token <= 32; ++token) { unique.push_back(token); }
    index.rebuild(unique);
    expect(!index.find(unique, 15).available(), "context copy rejects a unique suffix");

    const std::vector<ginfer::TokenId> body{
        10, 11, 12, 13, 14, 15, 21, 22, 23, 24, 25, 26, 27, 28, 29,
    };
    std::vector<ginfer::TokenId> quote = body;
    quote.insert(quote.end(), body.begin(), body.begin() + 6);
    index.rebuild(quote);
    const auto copied = index.find(quote, 9);
    expect(copied.match_length == 6 && copied.valid_tokens == 9,
           "context copy reports exact match and available continuation");
    expect(std::equal(copied.tokens.begin(), copied.tokens.begin() + copied.valid_tokens,
                      body.begin() + 6),
           "context copy returns the represented continuation");

    // The older occurrence has a longer backward match than the newer one and therefore wins.
    std::vector<ginfer::TokenId> longest{
        1, 2, 3, 4, 5, 6, 7, 8, 100, 101, 102, 103,
        9, 9, 3, 4, 5, 6, 7, 8, 200, 201,
        1, 2, 3, 4, 5, 6, 7, 8,
    };
    index.rebuild(longest);
    const auto longest_match = index.find(longest, 4);
    expect(longest_match.match_length == 8 && longest_match.valid_tokens == 4 &&
               longest_match.tokens[0] == 100,
           "context copy chooses longest match before recency");

    std::vector<ginfer::TokenId> periodic{5, 6, 7, 8, 5, 6, 7, 8, 5, 6, 7, 8};
    index.rebuild(periodic);
    const auto overlap = index.find(periodic, 7);
    expect(overlap.available() && overlap.tokens[0] == 5 && overlap.tokens[1] == 6 &&
               overlap.tokens[2] == 7 && overlap.tokens[3] == 8,
           "context copy admits an overlapping periodic continuation");

    periodic.insert(periodic.end(), {5, 6, 7, 8});
    index.append(periodic);
    expect(index.find(periodic, 7).available(), "context copy incrementally indexes commits");
    periodic.resize(12);
    index.append(periodic);
    expect(index.find(periodic, 7).available(), "context copy rebuilds after truncation");

    std::array<q36::detail::ContextCopyMatch, 2> matches{};
    matches[0].valid_tokens = 15;
    matches[1].valid_tokens = 8;
    std::array<std::uint32_t, 2> confirmations{2, 2};
    expect(q36::detail::use_wide_context_copy(matches, confirmations, 4, 15, false),
           "context copy widens only after unanimous confirmation");
    confirmations[1] = 1;
    expect(!q36::detail::use_wide_context_copy(matches, confirmations, 4, 15, false),
           "context copy retains the short batch while one row is unconfirmed");
    matches[1].valid_tokens = 4;
    confirmations[1] = 2;
    expect(!q36::detail::use_wide_context_copy(matches, confirmations, 4, 15, false),
           "context copy retains the short batch without a useful tail");
    matches = {};
    confirmations = {};
    expect(q36::detail::use_wide_context_copy(matches, confirmations, 4, 15, true),
           "context copy widens one recovery round for a retained wide append");
}
