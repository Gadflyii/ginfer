...
ginfer::EngineOptions engine_options(const char* artifact) {
    ...
    options.kv_capacity               = ginfer::KvCapacityPolicy::explicit_capacity(4096);
    options.prefill_chunk             = 1024;
    options.speculative.backend       = ginfer::SpeculativeBackend::Mtp;
    options.speculative.draft_tokens  = 3;
    options.speculative.proposal_head = ginfer::ProposalHead::Optimized;
...
ginfer::PromptInput chinese_chat(bool enable_thinking) {
    ginfer::ChatMessage message;
    message.role = ginfer::ChatRole::User;
    message.parts.push_back(ginfer::MessagePart{
        .kind = ginfer::MessagePartKind::Text, .text = "你好，简单介绍一下你自己。", .media = {}});
...
int exercise_registered_frontend(const ginfer::Engine& engine) {
...
int exercise_full_prefill_chunk(ginfer::Engine& engine) {
    constexpr std::size_t kChunkTokens = 1024;
    std::vector<ginfer::TokenId> prompt(kChunkTokens, 198);
...
int exercise_zero_suffix_reuse(ginfer::Engine& engine, const std::vector<ginfer::TokenId>& prompt) {
    ginfer::RequestOptions baseline_options;
    ...
    const ginfer::GenerationResult baseline =
    ...
    const ginfer::GenerationResult reused =
...
int exercise_prefix(ginfer::Engine& engine) {
    ginfer::RequestOptions first_options;
    ...
    const std::vector<ginfer::TokenId> prompt{248045, 846, 198, 5834, 248046, 198};
    ...
    std::vector<ginfer::TokenId> continuation = prompt;
...
int exercise_rewrite_checkpoints(ginfer::Engine& engine) {
    auto text_message = [](ginfer::ChatRole role, std::string text) {
        ginfer::ChatMessage message;
        message.role = role;
        message.parts.push_back(ginfer::MessagePart{
    ...
    auto assistant_call = [&](std::string reasoning, std::string id, std::string key) {
        ginfer::ChatMessage message = text_message(ginfer::ChatRole::Assistant, "");
        message.reasoning_content   = std::move(reasoning);
        message.tool_calls.push_back(ginfer::ToolCall{
    ...
    auto input_with_history = [&](int completed_responses, bool preserve_thinking) {
        ...
        if (completed_responses >= 1) {
            ...
            ginfer::ChatMessage tool =
                text_message(ginfer::ChatRole::Tool, "{\"value\":17,\"next\":\"beta\"}");
        ...
        if (completed_responses >= 2) {
            ...
            ginfer::ChatMessage tool = text_message(ginfer::ChatRole::Tool, "{\"value\":25}");
    ...
    const ginfer::GenerationResult second_replay =
        engine.generate(engine.prepare(input_with_history(2, true)), options(true));
    if (second_replay.generated_token_ids.size() != 4 ||
        second_replay.prefix_reuse_path != ginfer::PrefixReusePath::RestoreResponseCheckpoint ||
    ...
    const ginfer::GenerationResult mode_change =
        engine.generate(engine.prepare(input_with_history(2, false)), options(true));
    if (mode_change.generated_token_ids.size() != 4 ||
        mode_change.prefix_reuse_path != ginfer::PrefixReusePath::RestoreResponseCheckpoint ||
...
int exercise_vision(ginfer::Engine& engine) {
    ...
    auto assistant_message = [](const ginfer::GenerationResult& result) {
        ginfer::ChatMessage message;
        message.role              = ginfer::ChatRole::Assistant;
        message.reasoning_content = result.reasoning;
        message.parts.push_back(ginfer::MessagePart{
            .kind = ginfer::MessagePartKind::Text, .text = result.content, .media = {}});
        return message;
    };
    auto first_input = [&](const std::vector<std::uint8_t>& bytes) {
        ...
        message.parts.push_back(ginfer::MessagePart{
            .kind = ginfer::MessagePartKind::Text, .text = "What is visible?", .media = {}});
        ginfer::PromptInput input;
        input.messages.push_back(std::move(message));
    ...
    auto followup_input = [&](const std::vector<std::uint8_t>& bytes,
                              const ginfer::GenerationResult& first) {
        ginfer::PromptInput input = first_input(bytes);
        input.messages.push_back(assistant_message(first));
        ginfer::ChatMessage followup;
        followup.role = ginfer::ChatRole::User;
        followup.parts.push_back(ginfer::MessagePart{
            .kind = ginfer::MessagePartKind::Text, .text = "Give one more detail.", .media = {}});
        input.messages.push_back(std::move(followup));
        return input;
    };
    auto appended_media_input =
        [&](const std::vector<std::uint8_t>& old_bytes, const ginfer::GenerationResult& first,
            const ginfer::GenerationResult& second, const std::vector<std::uint8_t>& new_bytes) {
            ginfer::PromptInput input = followup_input(old_bytes, first);
            input.messages.push_back(assistant_message(second));
            ginfer::ChatMessage followup;
            followup.role = ginfer::ChatRole::User;
            followup.parts.push_back(image_part(new_bytes, "second.ppm"));
            followup.parts.push_back(ginfer::MessagePart{
                .kind = ginfer::MessagePartKind::Text, .text = "Compare the images.", .media = {}});
            input.messages.push_back(std::move(followup));
            return input;
        };
    ...
    ginfer::RequestOptions cross_chunk_options            = options(false);
    cross_chunk_options.execution.requested_output_tokens = 1;
    const ginfer::GenerationResult cross_chunk =
    ...
    if (!first.prompt.has_media || first.generated_token_ids.size() != 2 ||
        first.finish_reason != ginfer::FinishReason::OutputLimit) {
    ...
    const ginfer::GenerationResult appended = engine.generate(
    ...
    const ginfer::GenerationResult baseline = engine.generate(
        engine.prepare(appended_media_input(image_bytes, first, reused, second_image)),
        options(false));
    ...
    const ginfer::GenerationResult miss = engine.generate(
    ...
    ginfer::RequestOptions mtp_options            = options(false);
    mtp_options.execution.requested_output_tokens = 5;
    const ginfer::GenerationResult mtp_baseline =
    ...
    const ginfer::GenerationResult stopped_reuse =
    ...
    const std::vector<ginfer::TokenId> visual_prefix{248045, 846, 198, 248053};
    ginfer::RequestOptions source_options            = options(false);
    source_options.execution.requested_output_tokens = 1;
    const ginfer::GenerationResult bridge_source =
        engine.generate(engine.prepare_tokens(visual_prefix), source_options);
    ginfer::RequestOptions bridge_options            = options(true);
    bridge_options.execution.requested_output_tokens = 5;
    const ginfer::GenerationResult visual_bridge =
    ...
    ginfer::RequestOptions bridge_baseline_options       = bridge_options;
    bridge_baseline_options.execution.allow_prefix_reuse = false;
    const ginfer::GenerationResult visual_bridge_baseline =
...
int verify_loaded_product(const ginfer::Engine& engine) {
    const ginfer::LoadSummary load = engine.load_summary();
...
int main() {
    const char* groupwise = std::getenv("GINFER_QWEN3_6_27B_WEIGHTS");
    const char* nvfp4     = std::getenv("GINFER_QWEN3_6_27B_NVFP4_WEIGHTS");
    if ((groupwise == nullptr || *groupwise == '\0') && (nvfp4 == nullptr || *nvfp4 == '\0')) {
        std::cout << "skip: neither GINFER_QWEN3_6_27B_WEIGHTS nor "
                     "GINFER_QWEN3_6_27B_NVFP4_WEIGHTS is set\n";
        return 77;
    }
