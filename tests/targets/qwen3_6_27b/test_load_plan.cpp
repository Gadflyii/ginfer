...
ginfer::targets::qwen3_6::StartupFeatures all_features() {
    return {
        .vision        = true,
        .speculative   = ginfer::SpeculativeBackend::Mtp,
...
int verify_profile_mismatch_rejection() {
    ...
    try {
        (void)ginfer::targets::qwen3_6::create_program<Variant>(
...
int main() {
    const std::filesystem::path groupwise =
        artifact_path("GINFER_QWEN3_6_27B_WEIGHTS", "qwen3_6_27b.ginfer");
    const std::filesystem::path nvfp4 =
        artifact_path("GINFER_QWEN3_6_27B_NVFP4_WEIGHTS", "qwen3_6_27b_nvfp4.ginfer");
