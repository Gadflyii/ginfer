...
ginfer::EngineOptions ordinary_engine_options(const char* artifact) {
    ginfer::EngineOptions options;
    options.artifact_path  = artifact;
    options.max_context    = 128;
    options.kv_capacity    = ginfer::KvCapacityPolicy::explicit_capacity(128);
...
ginfer::EngineOptions dflash_engine_options(const char* artifact, ginfer::ProposalHead proposal,
                                            std::uint32_t max_context) {
    ginfer::EngineOptions options     = ordinary_engine_options(artifact);
    options.max_context               = max_context;
    options.kv_capacity               = ginfer::KvCapacityPolicy::explicit_capacity(max_context);
...
ginfer::RequestOptions greedy_options(std::uint32_t outputs, bool reuse) {
...
ginfer::PromptInput initial_conversation() {
    ...
    user.parts.push_back(ginfer::MessagePart{
        .kind = ginfer::MessagePartKind::Text, .text = "Name one prime number.", .media = {}});
    input.messages.push_back(std::move(user));
    return input;
}

ginfer::PromptInput followup_conversation(const ginfer::GenerationResult& first,
                                          std::string followup) {
    ginfer::PromptInput input = initial_conversation();
    ginfer::ChatMessage assistant;
    assistant.role              = ginfer::ChatRole::Assistant;
    assistant.reasoning_content = first.reasoning;
    assistant.parts.push_back(ginfer::MessagePart{
        .kind = ginfer::MessagePartKind::Text, .text = first.content, .media = {}});
    input.messages.push_back(std::move(assistant));

    ginfer::ChatMessage next;
    next.role = ginfer::ChatRole::User;
    next.parts.push_back(ginfer::MessagePart{
        .kind = ginfer::MessagePartKind::Text, .text = std::move(followup), .media = {}});
    input.messages.push_back(std::move(next));
    return input;
}

ginfer::PromptInput altered_history_after_boundary() {
    ginfer::PromptInput input = initial_conversation();
    ginfer::ChatMessage assistant;
    assistant.role = ginfer::ChatRole::Assistant;
    assistant.parts.push_back(ginfer::MessagePart{
        .kind = ginfer::MessagePartKind::Text, .text = "This history was changed.", .media = {}});
    input.messages.push_back(std::move(assistant));

    ginfer::ChatMessage next;
    next.role = ginfer::ChatRole::User;
    next.parts.push_back(ginfer::MessagePart{
        .kind = ginfer::MessagePartKind::Text, .text = "Continue briefly.", .media = {}});
    input.messages.push_back(std::move(next));
    return input;
}

int verify_dflash_load(const ginfer::Engine& engine) {
    ...
    if (memory.max_context != 4352 || memory.kv_cache != ginfer::KvCacheStorage::BFloat16 ||
...
int exercise_partial_terminal(ginfer::Engine& engine, const std::vector<ginfer::TokenId>& prompt,
                              const std::vector<ginfer::TokenId>& baseline) {
    for (std::size_t stop_index = 1; stop_index < baseline.size(); ++stop_index) {
        const ginfer::TokenId stop = baseline[stop_index];
        ...
        ginfer::RequestOptions options = greedy_options(24, false);
        ...
        if (stopped.finish_reason != ginfer::FinishReason::StopToken) { continue; }
        ...
        const ginfer::GenerationResult reused = engine.generate(
...
int exercise_boundary_restore(ginfer::Engine& engine) {
    ...
    const ginfer::GenerationResult restored =
...
int exercise_long_boundary_restore(ginfer::Engine& engine) {
    constexpr std::uint32_t generated_tokens = 4100;
    const ginfer::GenerationResult long_run  = engine.generate(
...
int main() {
    const char* artifact = std::getenv("GINFER_QWEN3_6_35B_A3B_WEIGHTS");
    if (artifact == nullptr || *artifact == '\0') {
        std::cout << "skip: GINFER_QWEN3_6_35B_A3B_WEIGHTS is not set\n";
        return 77;
    }

    const std::vector<ginfer::TokenId> prompt{
        248045, 846,    198, 109266, 3709,  96220, 117443, 97913,
        1710,   248046, 198, 248045, 74455, 198,   248068, 198,
    };
    std::vector<ginfer::TokenId> target_output;
    {
        ginfer::Engine ordinary(ordinary_engine_options(artifact));
        target_output =
    ...
    {
        ginfer::EngineOptions options =
            dflash_engine_options(artifact, ginfer::ProposalHead::Full, 128);
        options.max_concurrency = 2;
        ginfer::Engine full(std::move(options));
        ...
        const ginfer::GenerationResult first_result  = first.wait();
        const ginfer::GenerationResult second_result = second.wait();
        const auto valid = [&](const ginfer::GenerationResult& result, std::size_t count) {
    ...
    ginfer::Engine engine(dflash_engine_options(artifact, ginfer::ProposalHead::Optimized, 4352));
    if (const int result = verify_dflash_load(engine); result != 0) { return result; }
    engine.reset_memory_peaks();
    const ginfer::GenerationResult dflash =
    ...
    if (dflash.speculative.backend != ginfer::SpeculativeBackend::DFlash ||
