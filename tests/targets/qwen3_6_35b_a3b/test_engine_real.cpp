...
ginfer::EngineOptions maximum_engine_options(const char* artifact) {
    ginfer::EngineOptions options     = engine_options(artifact);
    options.max_context               = 262144;
    options.kv_capacity               = ginfer::KvCapacityPolicy::explicit_capacity(262144);
    options.speculative.backend       = ginfer::SpeculativeBackend::Mtp;
...
int exercise_text_mtp_and_prefix(ginfer::Engine& engine) {
    ...
    if (first.generated_token_ids.size() != 5 ||
        first.speculative.backend != ginfer::SpeculativeBackend::Mtp ||
    ...
    ginfer::RequestOptions stop_options = greedy_options(5, false);
    ...
    std::vector<ginfer::TokenId> stopped_continuation = prompt;
    ...
    const ginfer::GenerationResult stopped_reuse = engine.generate(
...
int exercise_vision(ginfer::Engine& engine) {
    ...
    const ginfer::MemorySummary before = engine.memory_summary();
    ...
    const ginfer::MemorySummary after = engine.memory_summary();
...
int exercise_maximum_configuration(const char* artifact) {
    ginfer::Engine engine(maximum_engine_options(artifact));
    const ginfer::MemorySummary memory = engine.memory_summary();
    if (memory.max_context != 262144 || memory.kv_cache != ginfer::KvCacheStorage::Int8Group64 ||
    ...
    std::vector<ginfer::TokenId> oversized(262145, 198);
    bool rejected = false;
    try {
    ...
    const std::vector<ginfer::TokenId> prompt{248045, 846, 198, 5834, 248046, 198};
    const ginfer::GenerationResult probe =
