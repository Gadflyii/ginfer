...
int main() {
    using ginfer::runtime::ActiveAdmissionSnapshot;
    using ginfer::runtime::AdmissionResources;
    using ginfer::runtime::BackfillClass;
    ...
    const auto protection = ginfer::runtime::make_admission_protection(
    ...
    failures += check(ginfer::runtime::protection_frontier_distance(protection, incumbents) == 20,
                      "frontier distance did not follow the frozen donor");

    const AdmissionResources persistent_candidate{1, 24, 40};
    failures += check(ginfer::runtime::persistent_backfill_is_safe(protection, incumbents,
                                                                   persistent_candidate, capacity),
                      "future resource surplus rejected a persistent-safe backfill");
    failures += check(!ginfer::runtime::persistent_backfill_is_safe(
    ...
    failures += check(ginfer::runtime::protection_frontier_distance(protection, after_donor) == 0,
                      "later temporal work changed the frozen frontier");
    failures += check(
        ginfer::runtime::protected_head_safe_without_temporal(protection, after_donor, capacity),
        "released frontier did not mature behind a temporal borrower");

    failures += check(
        !ginfer::runtime::admission_resources_fit(AdmissionResources{1, 161, 1}, capacity) &&
            !ginfer::runtime::admission_resources_fit(AdmissionResources{1, 1, 129}, capacity),
