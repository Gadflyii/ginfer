...
ginfer::PromptCapabilities effort_capabilities() {
    ginfer::PromptCapabilities capabilities;
...
ginfer::OwnedMedia fake_media(const ContentPart& part) {
    ginfer::OwnedMedia media;
    media.kind =
        part.kind == ContentKind::Image ? ginfer::MediaKind::Image : ginfer::MediaKind::Video;
    media.bytes.push_back(0);
    media.media_type = part.source.media_type;
    return media;
}

ginfer::PromptInput translate(const GenerationRequest& req) {
...
std::string joined_text(const ginfer::ChatMessage& message) {
    std::string text;
    for (const ginfer::MessagePart& part : message.parts) {
        if (part.kind == ginfer::MessagePartKind::Text) { text += part.text; }
    }
    return text;
}
...
int test_parse_basic_and_system() {
    ...
    failures += check(req.messages[0].role == ginfer::ChatRole::System, "system turn is first");
    failures += check(req.messages[0].content[0].text == "be terse", "system text carried");
    failures += check(req.messages[1].role == ginfer::ChatRole::User, "user turn follows system");
...
int test_parse_system_array_and_blocks() {
    ...
    const GenerationRequest req = parse_messages_request(body, default_limits());
    failures += check(req.messages[0].role == ginfer::ChatRole::System, "system first");
    failures += check(req.messages[0].content[0].text == "a\nb", "system blocks joined");
    const ginfer::PromptInput prompt = translate(req);
...
int test_ordered_system_messages() {
    ...
    failures += check(req.messages[0].role == ginfer::ChatRole::System &&
                          req.messages[0].content[0].text == "top-level system",
                      "top-level system was not preserved independently");
    failures += check(req.messages[1].role == ginfer::ChatRole::User &&
                          req.messages[1].content[0].text == "hello",
                      "user turn moved around dynamic system");
    failures += check(req.messages[2].role == ginfer::ChatRole::System &&
                          req.messages[2].content[0].text == "reminder from messages",
                      "dynamic system was not preserved at its message-array position");
    const ginfer::PromptInput prompt = translate(req);
    failures +=
        check(prompt.messages.size() == 3 && prompt.messages[0].role == ginfer::ChatRole::System &&
                  prompt.messages[1].role == ginfer::ChatRole::User &&
                  prompt.messages[2].role == ginfer::ChatRole::System,
    ...
    failures +=
        check(breq.messages.size() == 2 && breq.messages[0].role == ginfer::ChatRole::User &&
                  breq.messages[1].role == ginfer::ChatRole::System &&
    ...
    failures +=
        check(creq.messages.size() == 4 && creq.messages[1].role == ginfer::ChatRole::System &&
                  creq.messages[1].content[0].text == "first" &&
                  creq.messages[2].role == ginfer::ChatRole::System &&
    ...
    failures += check(tool_req.messages.size() == 4 &&
                          tool_req.messages[0].role == ginfer::ChatRole::User &&
                          tool_req.messages[1].role == ginfer::ChatRole::Assistant &&
                          tool_req.messages[2].role == ginfer::ChatRole::Tool &&
                          tool_req.messages[3].role == ginfer::ChatRole::System,
...
int test_user_content_block_order() {
    ...
    failures += check(req.messages[0].role == ginfer::ChatRole::User &&
                          req.messages[0].content[0].text == "before" &&
                          req.messages[1].role == ginfer::ChatRole::Tool &&
                          req.messages[1].content[0].text == "result" &&
                          req.messages[2].role == ginfer::ChatRole::User &&
...
int test_parse_image() {
    ...
    failures += check(prompt.messages[0].parts[0].kind == ginfer::MessagePartKind::Media &&
                          prompt.messages[0].parts[0].media.kind == ginfer::MediaKind::Image,
...
int test_tool_use_result_roundtrip() {
    ...
    failures += check(req.messages[1].role == ginfer::ChatRole::Assistant, "assistant turn");
    ...
    failures += check(req.messages[2].role == ginfer::ChatRole::Tool, "tool_result -> tool turn");
    ...
    failures += check(prompt.messages[2].parts.size() == 2 &&
                          prompt.messages[2].parts[1].kind == ginfer::MessagePartKind::Media &&
                          prompt.messages[2].parts[1].media.kind == ginfer::MediaKind::Image,
...
int test_thinking_and_sampling() {
    ...
    const ginfer::RequestOptions options = to_request_options(req, default_server());
    failures +=
    ...
    failures += check(dreq.enable_thinking.has_value() && !*dreq.enable_thinking &&
                          dreq.reasoning_effort == RequestedReasoningEffort::None,
                      "thinking disabled did not normalize to none");
    ginfer::PromptCapabilities fixed_thinking;
    const auto resolve_disabled = [&] {
        (void)resolve_prompt_semantics(dreq, default_server(), fixed_thinking);
    };
    failures += check(api_code(resolve_disabled) == "reasoning_effort_not_supported" &&
                          api_param(resolve_disabled) == "thinking.type",
                      "unsupported Anthropic thinking disable bypassed capabilities");
    ...
    Json unknown         = body;
    unknown["thinking"] = Json{{"type", "future"}};
    failures += check(throws_api([&] { (void)parse_messages_request(unknown, default_limits()); }),
                      "unknown Anthropic thinking mode was accepted");
...
int test_reasoning_effort() {
    ...
    const ginfer::PromptInput low_prompt = translate(low_request);
    failures += check(low_prompt.options.enable_thinking &&
                          low_prompt.options.reasoning_effort == ginfer::ReasoningEffort::Low,
    ...
    const ResolvedPromptSemantics mapped_high =
        resolve_prompt_semantics(high_request, default_server(), effort_capabilities());
    failures += check(mapped_high.reasoning_effort == ginfer::ReasoningEffort::XHigh,
                      "Anthropic high effort did not map to Qwen xhigh");

    Json maximum                 = base;
    maximum["output_config"]     = Json{{"effort", "max"}};
    failures += check(resolve_prompt_semantics(
                          parse_messages_request(maximum, default_limits()), default_server(),
                          effort_capabilities())
                              .reasoning_effort == ginfer::ReasoningEffort::XHigh,
                      "Anthropic max effort did not map to Qwen xhigh");

    ginfer::PromptCapabilities high_capabilities = effort_capabilities();
    high_capabilities.reasoning_effort.high       = true;
    const ResolvedPromptSemantics high_semantics =
        resolve_prompt_semantics(high_request, default_server(), high_capabilities);
    failures += check(high_semantics.reasoning_effort == ginfer::ReasoningEffort::High,
                      "template-supported Anthropic high effort was not resolved");
    ...
    const ResolvedPromptSemantics disabled_semantics =
        resolve_prompt_semantics(conflicting_request, default_server(), effort_capabilities());
    failures += check(conflicting_request.reasoning_effort == RequestedReasoningEffort::None &&
                          !disabled_semantics.enable_thinking &&
                          !disabled_semantics.reasoning_effort,
                      "Anthropic disabled thinking did not override independent effort");
...
int test_stop_reason_mapping() {
    int failures = 0;
    failures += check(std::string(messages_stop_reason(ginfer::FinishReason::OutputLimit, false)) ==
                          "max_tokens",
                      "output limit -> max_tokens");
    failures += check(std::string(messages_stop_reason(ginfer::FinishReason::StopToken, false)) ==
                          "end_turn",
                      "stop token -> end_turn");
    failures += check(std::string(messages_stop_reason(ginfer::FinishReason::Cancelled, false)) ==
                          "end_turn",
                      "cancelled -> end_turn");
    failures += check(std::string(messages_stop_reason(ginfer::FinishReason::StopString, true)) ==
                          "tool_use",
                      "tool calls -> tool_use");
    return failures;
}
