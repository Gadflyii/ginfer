...
ginfer::test::artifact_fixture::TemporaryArtifact write_fixture() {
    using Json = ginfer::test::artifact_fixture::Json;
    return ginfer::test::artifact_fixture::write_fixture(
        {
...
int main() {
    try {
        ...
        ginfer::artifact::Reader reader(fixture.path);
        ginfer::artifact::Binder validation_binder(reader);
        const auto validated_resource = validation_binder.require_resource(
            "frontend/test.json", ginfer::artifact::ResourceEncoding::RawBytesV1);
        ...
        const auto validated_only                              = validation_binder.require_tensor(
            "weights/test", ginfer::artifact::NumericFormat::BF16,
            ginfer::artifact::StorageLayout::ContiguousLeV1, validated_shape);
        ...
        const auto retained_tensor                            = validation_binder.require_tensor(
            "weights/second", ginfer::artifact::NumericFormat::BF16,
            ginfer::artifact::StorageLayout::ContiguousLeV1, retained_shape);
        ...
        const auto validated_fp8                         = validation_binder.require_tensor(
            "weights/fp8", ginfer::artifact::NumericFormat::FP8_E4M3FN_ROW_BF16S,
            ginfer::artifact::StorageLayout::RowScaleV1, fp8_shape);
        ...
        const auto second =
            binder.require_tensor("weights/second", ginfer::artifact::NumericFormat::BF16,
                                  ginfer::artifact::StorageLayout::ContiguousLeV1, second_shape);
        ...
        const auto tensor =
            binder.require_tensor("weights/test", ginfer::artifact::NumericFormat::BF16,
                                  ginfer::artifact::StorageLayout::ContiguousLeV1, tensor_shape);
        ...
        const ginfer::artifact::MaterializationPlan plan = binder.finish();
        ...
        ginfer::DeviceContext device(0);
        auto materialized = ginfer::artifact::materialize(reader, plan, device);
        ...
        const ginfer::Weight fp8_weight = ginfer::artifact::materialized_weight(
            materialized, fp8, ginfer::artifact::NumericFormat::FP8_E4M3FN_ROW_BF16S, 2, 4);
        require(fp8_weight.qtype == ginfer::QType::FP8_E4M3FN_ROW_BF16S &&
                    fp8_weight.layout == ginfer::QuantLayout::RowScale &&
                    fp8_weight.scale_dtype == ginfer::DType::BF16 && fp8_weight.n == 2 &&
        ...
        require(stats.tensor_count == 3 && stats.resource_count == 1 &&
                    ...
                    stats.file_bytes == kResource.size() +
                                            ginfer::artifact::Reader::direct_io_alignment +
                                            kTailReadBytes,
