...
from tools.bench.run_ginfer_bench_matrix import BenchCase, report_rows
...
def test_schema_v12_report_is_flattened_for_matrix_summary(tmp_path) -> None:
    ...
    report_path.write_text(
        json.dumps(
            {
                "schema_version": 12,
                "artifact_type": "ginfer_bench_report",
                "tool": "ginfer_bench",
                "artifact": {"path": "model.ginfer"},
    ...
    assert (row["spec_draft_window"], row["spec_verification_window"]) == (3, 15)
    assert row["spec_expanded_rounds"] == 1
