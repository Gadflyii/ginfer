...
int expect_throws_device(int device_id) {
    try {
        ginfer::DeviceContext invalid(device_id);
    } catch (const std::runtime_error&) { return 0; }
    std::cerr << "DeviceContext(" << device_id << ") did not throw\n";
    return 1;
}

int check_context(const ginfer::DeviceContext& ctx, const char* label) {
    int failures = 0;
    ...
    if (ctx.occupancy().sm_count != ctx.props.multiProcessorCount) {
        std::cerr << label << " occupancy.sm_count does not match multiProcessorCount\n";
        ++failures;
    }
    if (ctx.capability().compute_capability != ctx.sm()) {
        std::cerr << label << " capability.compute_capability does not match sm()\n";
        ++failures;
    }
    if (ctx.occupancy().gqa_decode_splits_base != ctx.occupancy().sm_count / 2) {
        std::cerr << label << " occupancy.gqa_decode_splits_base is not sm_count / 2\n";
        ++failures;
    }
    return failures;
}
...
int main() {
    ...
    ginfer::DeviceContext moved(std::move(ctx));
    ...
    ginfer::CudaEventTimer timer(moved);
    timer.start();
    moved.synchronize();
