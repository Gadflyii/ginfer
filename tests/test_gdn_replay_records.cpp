...
int expect_shape(const ginfer::Tensor& tensor, std::int32_t d0, std::int32_t d1, std::int32_t d2,
...
std::size_t record_bytes(const ginfer::GdnReplayRecordSpec& spec) {
    ginfer::LayoutBuilder builder;
    (void)ginfer::plan_gdn_replay_records(builder, spec);
...
int main() {
    ...
    const ginfer::GdnReplayRecordSpec spec{
        .layers          = 3,
        .record_capacity = 5,
        .width           = 4,
        .conv_channels   = 256,
    ...
    ginfer::LayoutBuilder builder;
    const auto layout       = ginfer::plan_gdn_replay_records(builder, spec);
    const std::size_t bytes = builder.finish(256);
    auto backing            = make_backing(bytes);
    const ginfer::GdnReplayRecords records({backing.get(), bytes}, layout);
    ...
    failures += expect(records.conv.dtype == ginfer::DType::BF16, "conv dtype differs");
    failures += expect(records.key.dtype == ginfer::DType::BF16, "key dtype differs");
    failures += expect(records.value.dtype == ginfer::DType::BF16, "value dtype differs");
    failures += expect(records.gate.dtype == ginfer::DType::FP32, "gate dtype differs");
    ...
    failures += expect_throw(
        [&] {
            ginfer::LayoutBuilder invalid;
            (void)ginfer::plan_gdn_replay_records(invalid, {.layers          = 1,
                                                            .record_capacity = 9,
                                                            .width           = 2,
                                                            .conv_channels   = 1,
                                                            .qk_heads        = 1,
    ...
    ginfer::LayoutBuilder state_builder;
    const auto state_layout = ginfer::plan_linear_attention_state_pool(
    ...
    ginfer::LinearAttentionStatePool state({state_backing.get(), state_bytes}, state_layout);
    ...
    const ginfer::Tensor saved = state.conv[1];
