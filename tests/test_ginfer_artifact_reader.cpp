...
using ginfer::artifact::NumericFormat;
using ginfer::artifact::ObjectDescriptor;
using ginfer::artifact::Reader;
using ginfer::artifact::ResourceDescriptor;
using ginfer::artifact::StorageLayout;
using ginfer::artifact::TensorDescriptor;
using Json = nlohmann::json;
using ginfer::test::artifact_fixture::write_fixture;
...
void expect_artifact_error(Function&& function, std::string_view label) {
    try {
        function();
    } catch (const ginfer::artifact::ArtifactError&) { return; }
    throw std::runtime_error(std::string(label) + " was accepted");
}

void test_registered_sizes() {
    using ginfer::artifact::tensor_encoded_size;
...
void test_normative_fixture() {
    ...
    for (std::size_t i = 0; i < expected_names.size(); ++i) {
        ...
        if (ginfer::artifact::object_name(object) != expected_names[i] ||
        ...
        if (payload.absolute_offset !=
                reader.payload_offset() + ginfer::artifact::object_offset(object) ||
            payload.data.size() != ginfer::artifact::object_bytes(object) ||
...
void test_common_validation() {
    ...
    }
    {
        auto directory = normative_directory();
        auto fixture =
            write_fixture(directory, "legacy_v1", ginfer::test::artifact_fixture::kV1Magic);
        try {
            Reader reader(fixture.path);
        } catch (const ginfer::artifact::ArtifactError& error) {
            if (std::string_view(error.what())
                    .find("replace it with the published version-2 artifact") ==
                std::string_view::npos) {
                throw std::runtime_error("v1 rejection omitted replacement guidance");
            }
            return;
        }
...
void test_external_producer_fixture(const std::filesystem::path& path) {
    Reader reader(path);
    if (reader.identity().model_id != "ginfer-contract-fixture" ||
        reader.identity().weights_id != "v2" || reader.objects().size() != 3) {
        throw std::runtime_error("external producer fixture descriptor mismatch");
    }
    const std::array<std::string_view, 3> names = {"resource", "bf16", "q4"};
    for (const auto name : names) {
        const auto* object = reader.find(name);
        if (object == nullptr || reader.payload(*object).data.empty()) {
            throw std::runtime_error("external producer fixture payload mismatch");
        }
    }
}
...
int main(int argc, char** argv) {
    try {
        if (argc == 2) {
            test_external_producer_fixture(argv[1]);
            return 0;
        }
        if (argc != 1) {
            throw std::runtime_error("usage: ginfer_artifact_reader_test [producer-fixture]");
        }
