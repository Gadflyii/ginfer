...
int test_cli_contract() {
    ...
    failures += expect_string(parsed.artifact_path, "model.ginfer", "artifact path");
    ...
    failures += expect(parsed.kv_cache == ginfer::KvCacheStorage::Int8Group64, "INT8 KV");
    failures += expect(parsed.mtp_draft_tokens == 5, "MTP window");
    failures +=
        expect(parsed.proposal_head == ginfer::ProposalHead::Optimized, "optimized proposal head");
    ...
    failures += expect(qb::usage_text("ginfer_bench").find("artifact.ginfer") != std::string::npos,
                       "help names native artifact");
    failures += expect(parse_for_test({"ginfer_bench", "--help"}).help_requested, "help flag");

    failures += expect_throws<std::invalid_argument>([] { (void)parse_for_test({"ginfer_bench"}); },
                                                     "missing artifact");
    failures += expect_throws<std::invalid_argument>(
        [] {
            (void)parse_for_test({"ginfer_bench", "--weights", "model.ginfer", "--lm-head-draft"});
        },
        "optimized head without MTP");
    failures += expect_throws<std::invalid_argument>(
        [] {
            (void)parse_for_test(
                {"ginfer_bench", "--weights", "model.ginfer", "--mtp-draft-tokens", "6"});
        },
        "unsupported MTP window");
    failures += expect_throws<std::invalid_argument>(
        [] {
            (void)parse_for_test(
                {"ginfer_bench", "--weights", "model.ginfer", "--prefill-chunk", "129"});
        },
        "misaligned prefill chunk");
    failures += expect_throws<std::invalid_argument>(
        [] {
            (void)parse_for_test(
                {"ginfer_bench", "--weights", "model.ginfer", "--kv-dtype", "fp8"});
        },
...
ginfer::GenerationTimings timings(double prepare, double prefill, double decode, double total) {
...
ginfer::SpeculativeStats speculative(std::uint64_t rounds, std::uint64_t drafted,
...
qb::BenchEnvironment sample_environment() {
    ...
    env.device_id                         = 0;
    env.artifact_path                     = "model.ginfer";
    ...
    env.memory.kv_cache                   = ginfer::KvCacheStorage::Int8Group64;
    ...
    env.kv_cache                          = ginfer::KvCacheStorage::Int8Group64;
    env.mtp_draft_tokens                  = 5;
    env.proposal_head                     = ginfer::ProposalHead::Optimized;
...
int test_report_contract() {
    ...
    try {
        report = Json::parse(qb::format_json(
            env, "ginfer_bench --weights model.ginfer --mtp-draft-tokens 5", results));
    ...
    failures += expect(report.at("artifact_type") == "ginfer_bench_report", "report identity");
    failures += expect(report.at("artifact").at("path") == "model.ginfer", "artifact path");
    ...
    failures += expect(tg.at("speculative").at("verification_window") == 15,
                       "speculative verification window");
    failures += expect(tg.at("speculative").at("expanded_rounds") == 1,
                       "speculative expanded rounds");
...
int test_human_and_csv_reports() {
    ...
    failures += expect(table.find("model.ginfer") != std::string::npos, "table artifact");
    failures +=
    ...
    for (const std::string_view field :
         {"proposal_head", "kv_payload_bytes", "load_host_to_device_bytes",
          ...
          "workspace_allocator_peak_bytes", "spec_verification_window",
          "spec_expanded_rounds", "spec_acceptance_rate", "decode_output_tok_s_mean",
...
int main() {
    ...
    return failures == 0 ? 0 : fail("ginfer_bench support contract failed");
}
