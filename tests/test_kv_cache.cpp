...
struct PlannedPagedCache {
    ginfer::PagedKVPoolLayout layout;
    std::size_t bytes = 0;
};

PlannedPagedCache
plan_paged_cache(std::uint32_t pages, std::uint32_t logical_pages, std::int32_t rows,
                 std::vector<ginfer::PagedKVPlaneSpec> planes,
                 ginfer::PagedKVPlaneOrder order = ginfer::PagedKVPlaneOrder::PageMajor) {
    ginfer::LayoutBuilder builder;
    auto layout = ginfer::plan_paged_kv_pool(builder, {.page_group_count      = pages,
...
int check_shape(const ginfer::Tensor& tensor, const std::int32_t (&expected)[4],
                const char* label) {
    int failures = 0;
    for (int i = 0; i < 4; ++i) {
...
int expect_device_page_ids(const ginfer::Tensor& row, std::initializer_list<std::int32_t> expected,
                           const char* label) {
...
int expect_zeroed_pages(ginfer::PagedKVPool& pool, ginfer::PagedKVPlaneOrder order,
                        std::span<const std::int32_t> pages, cudaStream_t stream,
                        const char* label) {
    const ginfer::Tensor& plane = pool.plane(0);
    ...
    for (const std::int32_t page : pages) {
        if (order == ginfer::PagedKVPlaneOrder::PageMajor) {
...
int main() {
    ...
    int failures = 0;
    ginfer::DeviceContext ctx(0);

    auto paged_plan = plan_paged_cache(10, 10, 2,
                                       {{ginfer::DType::I8, 64, 2},
                                        {ginfer::DType::I8, 64, 2},
                                        {ginfer::DType::FP16, 1, 2},
                                        {ginfer::DType::FP16, 1, 2}});
    ginfer::DeviceArena paged_arena(paged_plan.bytes);
    ginfer::PagedKVPool paged_pool({paged_arena.base(), paged_arena.capacity()}, paged_plan.layout);
    ...
    const std::size_t expected_payload =
        2 * ginfer::Tensor(nullptr, ginfer::DType::I8, {64, 64, 2, 10}).bytes() +
        2 * ginfer::Tensor(nullptr, ginfer::DType::FP16, {1, 64, 2, 10}).bytes();
    failures +=
    ...
    failures += expect_zeroed_pages(paged_pool, ginfer::PagedKVPlaneOrder::PageMajor,
                                    selected_pages, ctx.stream, "page-major selective zero");

    auto head_major_plan = plan_paged_cache(10, 10, 1, {{ginfer::DType::BF16, 128, 8}},
                                            ginfer::PagedKVPlaneOrder::HeadMajor);
    ginfer::DeviceArena head_major_arena(head_major_plan.bytes);
    ginfer::PagedKVPool head_major_pool({head_major_arena.base(), head_major_arena.capacity()},
                                        head_major_plan.layout);
    failures += check_shape(head_major_pool.plane(0), {128, 64, 10, 8}, "head-major paged plane");
    failures += expect_zeroed_pages(head_major_pool, ginfer::PagedKVPlaneOrder::HeadMajor,
    ...
    const ginfer::PagedKVResize claim[]  = {
        {.allocation = &allocation_c, .mapped_pages = 2, .page_entitlement = 5}};
    ginfer::resize_paged_kv_bundle(claim);
    allocation_c.bind_row(0);
    ...
    auto main_plan    = plan_paged_cache(4, 4, 1, {{ginfer::DType::BF16, 16, 1}});
    auto backend_plan = plan_paged_cache(2, 2, 1, {{ginfer::DType::BF16, 16, 1}});
    ginfer::DeviceArena main_arena(main_plan.bytes);
    ginfer::DeviceArena backend_arena(backend_plan.bytes);
    ginfer::PagedKVPool main_pool({main_arena.base(), main_arena.capacity()}, main_plan.layout);
    ginfer::PagedKVPool backend_pool({backend_arena.base(), backend_arena.capacity()},
                                     backend_plan.layout);
    const ginfer::PagedKVReservation impossible_bundle[] = {
        {.pool = &main_pool, .page_entitlement = 3},
        {.pool = &backend_pool, .page_entitlement = 3},
    };
    try {
        auto unused = ginfer::reserve_paged_kv_bundle(impossible_bundle);
        (void)unused;
        ++failures;
