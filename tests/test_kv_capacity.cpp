...
int main() {
    int failures = 0;
    const ginfer::runtime::SequenceCapacityCurve curve{
        .main_page_tokens                     = 64,
    ...
    const auto automatic =
        ginfer::runtime::resolve_kv_capacity(ginfer::KvCapacityPolicy::automatic(50), curve, 1360);
    failures +=
    ...
    const auto capped =
        ginfer::runtime::resolve_kv_capacity(ginfer::KvCapacityPolicy::automatic(50), curve, 10000);
    ...
    const auto explicit_capacity = ginfer::runtime::resolve_kv_capacity(
        ginfer::KvCapacityPolicy::explicit_capacity(129), curve, 1200);
    failures +=
    ...
    try {
        (void)ginfer::runtime::resolve_kv_capacity(ginfer::KvCapacityPolicy::automatic(50), curve,
                                                   1049);
