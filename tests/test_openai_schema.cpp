...
int test_parse_string_content() {
    ...
    failures += check(req.messages[0].role == ginfer::ChatRole::User, "role parsed");
...
int test_reasoning_effort() {
    ...
    const ginfer::PromptInput none_prompt =
    ...
    failures += check(mapped_high.reasoning_effort == ginfer::ReasoningEffort::XHigh,
                      "Chat Completions high effort did not map to Qwen xhigh");

    Json minimal                = base;
    minimal["reasoning_effort"] = "minimal";
    failures += check(resolve_prompt_semantics(
                          parse_chat_completion_request(minimal, default_limits()),
                          default_server(), effort_capabilities())
                              .reasoning_effort == ginfer::ReasoningEffort::Low,
                      "Chat Completions minimal effort did not map to Qwen low");

    Json maximum                = base;
    maximum["reasoning_effort"] = "max";
    failures += check(resolve_prompt_semantics(
                          parse_chat_completion_request(maximum, default_limits()),
                          default_server(), effort_capabilities())
                              .reasoning_effort == ginfer::ReasoningEffort::XHigh,
                      "Chat Completions max effort did not map to Qwen xhigh");
    ...
    failures += check(high_semantics.reasoning_effort == ginfer::ReasoningEffort::High,
                      "template-supported Chat Completions high effort was not resolved");

    Json disabled                = base;
    disabled["enable_thinking"] = false;
    const GenerationRequest disabled_request =
        parse_chat_completion_request(disabled, default_limits());
    ginfer::PromptCapabilities fixed_thinking;
    const auto resolve_disabled = [&] {
        (void)resolve_prompt_semantics(disabled_request, default_server(), fixed_thinking);
    };
    failures += check(api_code(resolve_disabled) == "reasoning_effort_not_supported" &&
                          api_param(resolve_disabled) == "enable_thinking",
                      "unsupported Chat Completions thinking disable bypassed capabilities");

    ginfer::PromptCapabilities toggle_capabilities;
    toggle_capabilities.enable_thinking = true;
    const ResolvedPromptSemantics binary_low =
        resolve_prompt_semantics(low_request, default_server(), toggle_capabilities);
    failures += check(binary_low.enable_thinking && !binary_low.reasoning_effort,
                      "reasoning effort did not map to binary thinking-on mode");
...
int test_parse_media_in_translate() {
    ...
    failures += check(req.messages[0].content[0].source.kind ==
                          ginfer::product::media_acquire::SourceKind::Data,
    ...
    failures += check(req.messages[0].content[1].source.kind ==
                          ginfer::product::media_acquire::SourceKind::Url,
                      "video URL source preserved");
    failures += check(prompt.messages[0].parts[1].kind == ginfer::MessagePartKind::Media &&
                          prompt.messages[0].parts[1].media.kind == ginfer::MediaKind::Video,
...
int test_instruction_roles_preserved() {
    ...
    int failures                     = 0;
    failures +=
        check(req.messages.size() == 3 && req.messages[0].role == ginfer::ChatRole::Developer &&
                  req.messages[1].role == ginfer::ChatRole::User &&
                  req.messages[2].role == ginfer::ChatRole::System,
              "schema did not preserve ordered developer/system roles");
    failures += check(prompt.messages.size() == 3 &&
                          prompt.messages[0].role == ginfer::ChatRole::Developer &&
...
int test_parse_tool_history_messages() {
    ...
    failures += check(req.messages[2].role == ginfer::ChatRole::Tool, "tool role parsed");
...
int test_models_and_error() {
    ...
    failures += check(list.at("data").at(0).at("owned_by") == "ginfer", "models list owner");
    ...
    failures += check(one.at("owned_by") == "ginfer", "model owner");
...
int test_finish_reason_wire() {
    int failures = 0;
    failures += check(std::string(finish_reason_wire(ginfer::FinishReason::StopToken)) == "stop",
                      "stop token wire");
    failures +=
        check(std::string(finish_reason_wire(ginfer::FinishReason::OutputLimit)) == "length",
              "output limit wire");
    failures += check(std::string(finish_reason_wire(ginfer::FinishReason::Cancelled)) == "stop",
