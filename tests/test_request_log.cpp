...
int main() {
    ...
    try {
        JsonlRequestLog unsafe("same-path.ginfer", "same-path.ginfer");
    ...
    options.artifact_path                  = "/models/qwen3_6_27b.ginfer";
    ...
    options.kv_capacity                    = ginfer::KvCapacityPolicy::explicit_capacity(524288);
    options.prefill_chunk                  = 1024;
    options.log_stats_interval_ms          = 2500;
    options.kv_cache                       = ginfer::KvCacheStorage::Int8Group64;
    ...
    options.startup_argv = {"ginfer-serve", options.artifact_path, "--api-key", "<redacted>"};

    const ginfer::ModelSamplingDefaults sampling_defaults{
        .thinking     = {.temperature = 1.0F, .top_k = 20, .top_p = 0.95F},
        .non_thinking = {.temperature = 0.7F, .top_k = 20, .top_p = 0.8F, .presence_penalty = 1.5F},
    };

    ginfer::LoadSummary load;
    ...
    ginfer::MemorySummary memory;
    memory.max_context                       = 262144;
    memory.kv_capacity_mode                  = ginfer::KvCapacityMode::Explicit;
    memory.kv_capacity                       = 524288;
    memory.kv_capacity_page_groups           = 8192;
    memory.kv_capacity_max_page_groups       = 16384;
    memory.kv_cache                          = ginfer::KvCacheStorage::Int8Group64;
    ...
    failures += check(server.at("engine").at("speculative_draft_window") == 3,
                      "speculative draft window missing");
    failures +=
    ...
    // GenerationService promotes the Engine-resolved automatic policy into its effective options
    // before HttpServer writes server_start. Exercise the resulting DFlash fields independently of
    // the explicit-MTP record above.
    ServeOptions resolved_options             = options;
    resolved_options.speculative.backend      = ginfer::SpeculativeBackend::DFlash;
    resolved_options.speculative.draft_tokens = 4;
    resolved_options.speculative.proposal_head = ginfer::ProposalHead::Full;
    const Json resolved_server = Json::parse(format_server_start_json(
        "serve-test", 1000, resolved_options, sampling_defaults, "deployment-alias", load,
        memory, environment, std::uint64_t{123456}));
    failures += check(resolved_server.at("engine").at("speculative_backend") == "dflash" &&
                          resolved_server.at("engine").at("speculative_draft_window") == 4 &&
                          resolved_server.at("engine").at("proposal_head") == "full",
                      "resolved automatic speculative policy was not logged");
    ...
    outcome.finish_reason                       = ginfer::FinishReason::OutputLimit;
    ...
    outcome.metrics.prefix_reuse_path           = ginfer::PrefixReusePath::RestoreTurnCheckpoint;
    outcome.metrics.speculative_backend         = ginfer::SpeculativeBackend::DFlash;
    outcome.metrics.speculative_draft_window    = 3;
    outcome.metrics.speculative_verification_window = 15;
    outcome.metrics.speculative_rounds          = 300;
    outcome.metrics.speculative_expanded_rounds = 7;
    ...
    outcome.metrics.prefix_reuse_path = ginfer::PrefixReusePath::RestoreResponseCheckpoint;
    ...
    failures +=
        check(done.at("speculative").at("backend") == "dflash", "speculative backend missing");
    failures +=
        check(done.at("speculative").at("draft_window") == 3, "speculative draft window missing");
    failures += check(done.at("speculative").at("verification_window") == 15,
                      "speculative verification window missing");
    failures += check(done.at("speculative").at("expanded_rounds") == 7,
                      "speculative expanded round count missing");
    ...
    failures += check(console_prefix.starts_with('[') &&
                          console_prefix.ends_with("] [info] ginfer-serve: "),
                      "console log prefix mismatch");

    const std::filesystem::path log_path =
        std::filesystem::temp_directory_path() /
        ("ginfer-request-log-test-" + std::to_string(static_cast<long long>(::getpid())) +
         ".jsonl");
