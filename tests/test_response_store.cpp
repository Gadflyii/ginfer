...
ChatTurn text_turn(ginfer::ChatRole role, std::string text) {
    ChatTurn turn;
    turn.role = role;
    ContentPart part;
...
int test_context_dag() {
    const ResponseContext first =
        append_response_context({}, {text_turn(ginfer::ChatRole::User, "one"),
                                     text_turn(ginfer::ChatRole::Assistant, "a")});
    const ResponseContext second =
        append_response_context(first, {text_turn(ginfer::ChatRole::User, "two"),
                                        text_turn(ginfer::ChatRole::Assistant, "b")});
...
int test_lru_and_delete() {
    ...
    const ResponseContext root =
        append_response_context({}, {text_turn(ginfer::ChatRole::User, "root")});
    store.put(record("resp_1", root));
    const ResponseContext child =
        append_response_context(root, {text_turn(ginfer::ChatRole::Assistant, "child")});
    store.put(record("resp_2", child));
    (void)store.get("resp_1"); // resp_2 becomes the least-recently used entry.
    store.put(record("resp_3",
                     append_response_context(root, {text_turn(ginfer::ChatRole::User, "fork")})));
...
int test_oversized_record() {
    ...
    StoredResponse large = record(
        "resp_large",
        append_response_context({}, {text_turn(ginfer::ChatRole::User, std::string(1024, 'x'))}));
    std::string code;
    try {
