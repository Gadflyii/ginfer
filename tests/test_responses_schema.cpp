...
int test_basic_request() {
    ...
    failures += check(request.input_turns.size() == 1 &&
                          request.input_turns[0].role == ginfer::ChatRole::User &&
    ...
    ChatTurn previous;
    previous.role = ginfer::ChatRole::Assistant;
    ...
    failures += check(composed.generation.messages.size() == 3 &&
                          composed.generation.messages[0].role == ginfer::ChatRole::Developer &&
...
int test_instruction_message_order() {
    ...
    int failures = check(request.input_turns.size() == 3 &&
                             request.input_turns[0].role == ginfer::ChatRole::System &&
                             request.input_turns[1].role == ginfer::ChatRole::User &&
                             request.input_turns[2].role == ginfer::ChatRole::Developer,
    ...
    failures += check(request.generation.messages.size() == 3 &&
                          request.generation.messages[0].role == ginfer::ChatRole::System &&
                          request.generation.messages[1].role == ginfer::ChatRole::User &&
                          request.generation.messages[2].role == ginfer::ChatRole::Developer,
...
int test_reasoning_effort() {
    ...
    failures += check(low_semantics.enable_thinking &&
                          low_semantics.reasoning_effort == ginfer::ReasoningEffort::Low,
    ...
    failures += check(mapped_high.reasoning_effort == ginfer::ReasoningEffort::XHigh,
                      "Responses high effort did not map to Qwen xhigh");

    Json minimal         = base;
    minimal["reasoning"] = Json{{"effort", "minimal"}};
    failures += check(resolve_prompt_semantics(
                          parse_responses_request(minimal, limits()).generation, ServeOptions{},
                          effort_capabilities())
                              .reasoning_effort == ginfer::ReasoningEffort::Low,
                      "Responses minimal effort did not map to Qwen low");

    Json maximum         = base;
    maximum["reasoning"] = Json{{"effort", "max"}};
    failures += check(resolve_prompt_semantics(
                          parse_responses_request(maximum, limits()).generation, ServeOptions{},
                          effort_capabilities())
                              .reasoning_effort == ginfer::ReasoningEffort::XHigh,
                      "Responses max effort did not map to Qwen xhigh");
    ...
    const ResolvedPromptSemantics high_semantics =
        resolve_prompt_semantics(high_request, ServeOptions{}, high_capabilities);
    failures += check(high_semantics.reasoning_effort == ginfer::ReasoningEffort::High,
                      "template-supported Responses high effort was not resolved");
...
int test_typed_items_and_tools() {
    ...
    failures += check(request.input_turns[0].role == ginfer::ChatRole::Assistant &&
    ...
    failures += check(request.input_turns[1].role == ginfer::ChatRole::Tool &&
...
GenerationOutcome sample_outcome() {
    ...
    outcome.finish_reason                   = ginfer::FinishReason::StopToken;
...
int test_response_object() {
    ...
    incomplete.finish_reason     = ginfer::FinishReason::OutputLimit;
