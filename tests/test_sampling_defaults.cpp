...
bool same_preset(const ginfer::SamplingPreset& actual, const ginfer::SamplingPreset& expected) {
...
int main() {
    using Dense27 = ginfer::targets::qwen3_6_27b::Package;
    using Moe35   = ginfer::targets::qwen3_6_35b_a3b::Package;
    using Muse30  = ginfer::targets::muse_glimmer_30b::Package;

    int failures = 0;

    const ginfer::ModelSamplingDefaults qwen3_6 = Dense27::sampling_defaults(Dense27::model_id);
    const ginfer::ModelSamplingDefaults qwen3_8 =
        Dense27::sampling_defaults(Dense27::qwen3_8_model_id);
    const ginfer::ModelSamplingDefaults qwen3_6_35 = Moe35::sampling_defaults(Moe35::model_id);

    const ginfer::SamplingPreset dense_thinking{
        .temperature = 1.0F, .top_k = 20, .top_p = 0.95F, .min_p = 0.0F};
    const ginfer::SamplingPreset dense_non_thinking{
        .temperature      = 0.7F,
        .top_k            = 20,
        .top_p            = 0.8F,
        .min_p            = 0.0F,
    ...
    const ginfer::SamplingPreset moe_thinking{
        .temperature      = 1.0F,
        .top_k            = 20,
        .top_p            = 0.95F,
        .min_p            = 0.0F,
    ...
    const ginfer::ModelSamplingDefaults muse = Muse30::sampling_defaults(Muse30::model_id);
    const ginfer::SamplingPreset muse_preset{
        .temperature = 1.0F, .top_k = 64, .top_p = 0.95F, .min_p = 0.0F};
    failures += check(same_preset(muse.thinking, muse_preset) &&
                          same_preset(muse.non_thinking, muse_preset),
                      "Muse Glimmer 30B sampling defaults mismatch");
    failures += check(throws_runtime([] { (void)Muse30::sampling_defaults("unknown"); }),
                      "unknown model received Muse sampling defaults");

    const ginfer::ResolvedSamplingParameters thinking = ginfer::runtime::resolve_sampling(
        qwen3_8, ginfer::SamplingMode::Thinking, ginfer::SamplingOverrides{});
    const ginfer::ResolvedSamplingParameters non_thinking = ginfer::runtime::resolve_sampling(
        qwen3_8, ginfer::SamplingMode::NonThinking, ginfer::SamplingOverrides{});
    ...
    ginfer::SamplingOverrides overrides;
    ...
    overrides.seed              = 123;
    const ginfer::ResolvedSamplingParameters overridden =
        ginfer::runtime::resolve_sampling(qwen3_8, ginfer::SamplingMode::NonThinking, overrides);
    ...
    failures += check(throws_invalid([&] {
                          (void)ginfer::runtime::resolve_sampling(
                              qwen3_8, ginfer::SamplingMode::Thinking, overrides);
                      }),
