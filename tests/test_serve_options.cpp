...
int main() {
    ...
    const ServeOptions defaults = parse({"ginfer-serve", "model.ginfer"});
    ...
    failures += check(defaults.media_cache_bytes == ginfer::kDefaultMediaCacheBytes &&
                          defaults.media_live_bytes == ginfer::kDefaultMediaLiveBytes &&
                          defaults.media_preprocess_threads == 0,
                      "media preparation resource defaults mismatch");
    failures += check(defaults.kv_capacity.mode == ginfer::KvCapacityMode::Explicit &&
                          defaults.kv_capacity.explicit_tokens == defaults.max_context,
                      "default KV capacity does not follow max context");
    failures += check(defaults.speculative.backend == ginfer::SpeculativeBackend::Automatic,
                      "artifact-owned speculative selection is not the default");
    ...
    const ServeOptions model_alias =
        parse({"ginfer-serve", "model.ginfer", "--model-id", "deployment-alias"});
    failures +=
    ...
    try {
        (void)parse({"ginfer-serve", "model.ginfer", "--model-id", ""});
    ...
    const ServeOptions dflash = parse({"ginfer-serve", "model.ginfer", "--spec", "dflash",
                                       "--draft-tokens", "15", "--lm-head-draft"});
    failures += check(dflash.speculative.backend == ginfer::SpeculativeBackend::DFlash,
    ...
    failures += check(dflash.speculative.proposal_head == ginfer::ProposalHead::Optimized,
                      "--lm-head-draft did not select the optimized proposal head");

    const ServeOptions no_spec =
        parse({"ginfer-serve", "model.ginfer", "--spec", "none"});
    failures += check(no_spec.speculative.backend == ginfer::SpeculativeBackend::None,
                      "--spec none did not explicitly disable speculation");

    bool dflash_vision_rejected = false;
    try {
        (void)parse({"ginfer-serve", "model.ginfer", "--spec", "dflash", "--draft-tokens", "15",
                     "--vision"});
    ...
    try {
        (void)parse({"ginfer-serve", "model.ginfer", "--draft-tokens", "3"});
    ...
    const ServeOptions configured = parse({"ginfer-serve",
                                           "model.ginfer",
                                           "--no-prefix-reuse",
                                           "--vision",
                                           "--max-concurrency",
    ...
    failures += check(configured.max_context == 4096 &&
                          configured.kv_capacity.mode == ginfer::KvCapacityMode::Explicit &&
    ...
    const ServeOptions response_store =
        parse({"ginfer-serve", "model.ginfer", "--response-store-max-records", "42",
    ...
    const ServeOptions sampling =
        parse({"ginfer-serve", "model.ginfer", "--temperature", "0", "--top-p", "0.9", "--top-k",
    ...
    ginfer::PromptCapabilities prompt_capabilities;
    ...
    const ginfer::RequestOptions inherited_sampling = to_request_options(request, sampling);
    ...
    failures +=
        check(serve_usage_text("ginfer-serve").find("--no-prefix-reuse") != std::string::npos,
              "serve help omits --no-prefix-reuse");
    failures +=
        check(serve_usage_text("ginfer-serve").find("--preserve-thinking") != std::string::npos,
              "serve help omits --preserve-thinking");
    failures += check(serve_usage_text("ginfer-serve").find("--vision") != std::string::npos,
                      "serve help omits --vision");
    failures +=
        check(serve_usage_text("ginfer-serve").find("--log-stats-interval-ms") != std::string::npos,
              "serve help omits --log-stats-interval-ms");
    failures += check(serve_usage_text("ginfer-serve").find("--media-preprocess-threads") !=
                          std::string::npos,
                      "serve help omits media preparation controls");
    failures += check(serve_usage_text("ginfer-serve").find("--kv-capacity") != std::string::npos,
                      "serve help omits --kv-capacity");
    failures += check(serve_usage_text("ginfer-serve").find("--response-store-max-mib") !=
                          std::string::npos,
                      "serve help omits Responses store limits");
    failures +=
        check(serve_usage_text("ginfer-serve").find("identity.model_id") != std::string::npos,
              "serve help omits the artifact-derived model id default");

    const ServeOptions inherited =
        parse({"ginfer-serve", "model.ginfer", "--max-context", "16384"});
    failures += check(inherited.kv_capacity.mode == ginfer::KvCapacityMode::Explicit &&
                          inherited.kv_capacity.explicit_tokens == 16384,
                      "omitted --kv-capacity did not follow --max-context");

    const ServeOptions automatic = parse({"ginfer-serve", "model.ginfer", "--kv-capacity", "auto"});
    failures += check(automatic.kv_capacity.mode == ginfer::KvCapacityMode::Automatic &&
                          automatic.kv_capacity.explicit_tokens == 0 &&
                          automatic.kv_capacity.automatic_headroom_bytes ==
                              ginfer::kDefaultKvCapacityHeadroomBytes,
                      "--kv-capacity auto did not select automatic sizing");

    const ServeOptions logged = parse({"ginfer-serve", "model.ginfer", "--request-log-jsonl",
    ...
    failures +=
        check(serve_usage_text("ginfer-serve").find("--request-log-jsonl") != std::string::npos,
