...
struct PlannedState {
    ginfer::LinearAttentionStatePoolLayout layout;
    std::size_t bytes = 0;
};
...
PlannedState plan_state(std::uint32_t layers, std::int32_t conv_channels, std::int32_t conv_width,
                        ...
                        ginfer::DType conv_dtype = ginfer::DType::BF16) {
    ginfer::LayoutBuilder builder;
    auto layout = ginfer::plan_linear_attention_state_pool(
        builder, ginfer::LinearAttentionStatePoolSpec{.layers         = layers,
...
int expect_device_byte(const ginfer::Tensor& tensor, unsigned char expected, const char* label) {
...
int main() {
    ...
    ginfer::DeviceArena state_arena(state_plan.bytes);
    CUDA_CHECK(cudaMemset(state_arena.base(), 0x4a, state_arena.capacity()));
    ginfer::LinearAttentionStatePool state({state_arena.base(), state_arena.capacity()},
                                           state_plan.layout);
    ...
    for (std::size_t layer = 0; layer < state.layer_count(); ++layer) {
        ...
        if (state.conv[layer].dtype != ginfer::DType::BF16) {
            ++failures;
            std::cerr << "conv dtype is not BF16\n";
        }
        if (state.recurrent[layer].dtype != ginfer::DType::FP32) {
            ++failures;
    ...
    ginfer::DeviceArena slotted_arena(slotted_plan.bytes);
    CUDA_CHECK(cudaMemset(slotted_arena.base(), 0, slotted_arena.capacity()));
    ginfer::LinearAttentionStatePool slotted({slotted_arena.base(), slotted_arena.capacity()},
                                             slotted_plan.layout);
    ...
    ginfer::Tensor conv0             = slotted.conv_slot(0, 0);
    ginfer::Tensor conv1             = slotted.conv_slot(0, 1);
    ginfer::Tensor recurrent0        = slotted.recurrent_slot(0, 0);
    ginfer::Tensor recurrent1        = slotted.recurrent_slot(0, 1);
    ginfer::Tensor conv1_layer1      = slotted.conv_slot(1, 1);
    ginfer::Tensor recurrent1_layer1 = slotted.recurrent_slot(1, 1);
    ...
    auto fp32_conv_plan = plan_state(1, 7, 2, 2, 3, 4, 2, ginfer::DType::FP32);
    ginfer::DeviceArena fp32_conv_arena(fp32_conv_plan.bytes);
    ginfer::LinearAttentionStatePool fp32_conv({fp32_conv_arena.base(), fp32_conv_arena.capacity()},
                                               fp32_conv_plan.layout);
    if (fp32_conv.conv[0].dtype != ginfer::DType::FP32) {
        ++failures;
