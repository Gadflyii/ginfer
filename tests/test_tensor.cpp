...
int check_shape(const ginfer::Tensor& t, const std::int32_t (&expected)[4], const char* label) {
    int failures = 0;
    for (int i = 0; i < 4; ++i) {
...
int check_strides(const ginfer::Tensor& t, const std::int64_t (&expected)[4], const char* label) {
    int failures = 0;
    for (int i = 0; i < 4; ++i) {
...
int main() {
    ...
    ginfer::Tensor t(base, ginfer::DType::BF16, {2, 3, 4});
    ...
    ginfer::Tensor viewed = t.view({4, 6});
    if (viewed.data != base) {
        ++failures;
    ...
    ginfer::Tensor sliced = t.slice(1, 1, 2);
    ...
    ginfer::Tensor permuted = t.permute({2, 1, 0, 3});
    ...
    ginfer::Tensor reshaped = t.reshape({6, 4});
    ...
    failures += expect_invalid([&] { (void)ginfer::Tensor(base, ginfer::DType::BF16, {2, 0}); },
                               "zero dimension");
