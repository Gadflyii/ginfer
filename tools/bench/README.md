...
Offline helper for the `ginfer_bench` throughput tool. Correctness/parity tooling lives separately
under [`tools/parity`](../parity).

## Corpus baker

`ginfer_bench` benchmarks prefill at an exact length by slicing the first `P` token ids of a
...
`--tokenizer-path` or set `GINFER_TOKENIZER_PATH`.
...
## GInfer performance matrix

`run_ginfer_bench_matrix.py` runs the layered public-Engine `ginfer_bench` matrix against the native
`.ginfer` artifact and stores its local reports under `profiles/bench/`. Its defaults are:

```text
artifact: out/qwen3_6_27b.ginfer
binary:   build/bench/ginfer_bench
...
cmake -S . -B build -DGINFER_BUILD_BENCHMARKS=ON

# Inspect commands without running the model.
python3 tools/bench/run_ginfer_bench_matrix.py --preset core --dry-run

# Main run. Builds build/bench/ginfer_bench first, then writes JSON and summary.csv.
python3 tools/bench/run_ginfer_bench_matrix.py --preset core

# Longer run that adds 32k/64k prompt and context-decode points.
python3 tools/bench/run_ginfer_bench_matrix.py --preset full

# Run only the MTP draft-window sweep.
python3 tools/bench/run_ginfer_bench_matrix.py --preset full --suite mtp_sweep
```

Default outputs:

```text
profiles/bench/ginfer-<preset>-<timestamp>/
  commands.sh
  manifest.json
...
Each raw report must be `ginfer_bench_report` schema v12. The flattened summary and schema-v3 matrix
...
draft/verification windows, expanded rounds, drafts, acceptance, and fallbacks. The matrix manifest
is descriptive and records the commands and selected local inputs; it does not make repository
state part of report validity.
...
schema-v11 serving startup record. The stochastic route pins its complete
