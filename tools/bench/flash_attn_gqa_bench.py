...
def load_ginfer_csv(path: str) -> dict[tuple[int, int], dict[str, str]]:
    if not path:
        return {}
    ...
    with open(path, newline="") as f:
        for row in csv.DictReader(f):
            ...
            if key in out:
                raise SystemExit(
                    "--ginfer-csv must contain only one execution/cache result for each T/context"
                )
            out[key] = row
    return out


def attach_ginfer_comparison(rows: list[dict], ginfer_rows: dict[tuple[int, int], dict[str, str]]) -> None:
    for row in rows:
        ginfer = ginfer_rows.get((int(row["T"]), int(row["context"])))
        if ginfer is None:
            continue
        ginfer_us = float(ginfer["median_us"])
        ginfer_ms = ginfer_us * 1.0e-3
        ginfer_tflops = float(ginfer["useful_flops"]) / (ginfer_us * 1.0e-6) / 1.0e12
        row["ginfer_ms"] = ginfer_ms
        row["ginfer_tflops"] = ginfer_tflops
        row["flash_vs_ginfer_speedup"] = ginfer_ms / float(row["ms"]) if float(row["ms"]) > 0.0 else None
        row["flash_vs_ginfer_tflops_ratio"] = (
            float(row["tflops"]) / ginfer_tflops if ginfer_tflops > 0.0 else None
        )
...
def main() -> int:
    ...
    parser.add_argument(
        "--ginfer-csv",
        default="",
        help="optional ginfer_causal_softmax_attention_bench CSV to merge",
    )
    ...
    attach_ginfer_comparison(rows, load_ginfer_csv(args.ginfer_csv))
    metadata = {
        "torch_version": torch.__version__,
        "torch_cuda": torch.version.cuda,
        "gpu": torch.cuda.get_device_name(0),
