#!/usr/bin/env python3
"""Bake the ginfer_bench meaningful-token corpus.

ginfer_bench slices the first P token ids of this corpus to build a prefill of an exact
...
def resolve_tokenizer_path(cli_path: str | None) -> Path:
    raw = cli_path or os.environ.get("GINFER_TOKENIZER_PATH")
    if not raw:
        raise SystemExit(
            "tokenizer path required; pass --tokenizer-path or set GINFER_TOKENIZER_PATH"
        )
...
def build_manifest(
    ...
    return {
        "artifact_type": "ginfer_bench_corpus",
        "schema_version": 1,
...
def check_outputs(out_path: Path, manifest_path: Path) -> int:
    ...
    failures = 0
    if manifest.get("artifact_type") != "ginfer_bench_corpus":
        print(
            "artifact_type mismatch: "
            f"{manifest.get('artifact_type')!r} != 'ginfer_bench_corpus'",
            file=sys.stderr,
        )
        failures += 1
