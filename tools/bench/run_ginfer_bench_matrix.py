#!/usr/bin/env python3
"""Run the native GInfer product performance matrix.
...
Raw ginfer_bench reports stay under profiles/bench. This script writes a
...
DEFAULT_BENCH = REPO_ROOT / "build/bench/ginfer_bench"
DEFAULT_WEIGHTS = REPO_ROOT / "out/qwen3_6_27b.ginfer"
...
REPORT_ARTIFACT_TYPE = "ginfer_bench_report"
REPORT_TOOL = "ginfer_bench"
...
def report_rows(report_path: Path, case: BenchCase) -> list[dict[str, Any]]:
    ...
    for test in report.get("tests", []):
        ...
        row = {
            ...
            "spec_draft_window": speculative.get("draft_window"),
            "spec_verification_window": speculative.get("verification_window"),
            "spec_rounds": speculative.get("rounds"),
            "spec_expanded_rounds": speculative.get("expanded_rounds"),
...
def write_manifest(
    ...
    manifest = {
        "artifact_type": "ginfer_bench_matrix_run",
        "schema_version": 3,
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--weights", type=Path, default=DEFAULT_WEIGHTS, help=".ginfer artifact passed to the bench"
    )
    ...
    parser.add_argument(
        "--no-build", action="store_true", help="do not build build/bench/ginfer_bench"
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if out_dir is None:
        out_dir = REPO_ROOT / "profiles/bench" / f"ginfer-{args.preset}-{utc_stamp()}"
    ...
    if not args.no_build:
        ...
        rc = run_command(
            ["cmake", "--build", "build", "-j", "--target", "ginfer_bench"],
            build_stdout,
            build_stderr,
        )
        if rc != 0:
