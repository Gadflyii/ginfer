...
POINT_ARTIFACT_TYPE = "ginfer_serve_concurrency_bench_point"
SUMMARY_ARTIFACT_TYPE = "ginfer_serve_concurrency_bench_summary"
SCHEMA_VERSION = 2
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="262144",
        metavar="N|auto",
        help="shared Main KV capacity passed to ginfer-serve (default: 262144)",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not args.dry_run:
        if not serve.is_file():
            raise corpus.CampaignError(f"ginfer-serve executable not found: {serve}")
        if not os.access(serve, os.X_OK):
            raise corpus.CampaignError(f"ginfer-serve is not executable: {serve}")
