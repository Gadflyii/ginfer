...
RUN_ARTIFACT_TYPE = "ginfer_serve_corpus_result"
RUN_SCHEMA_VERSION = 5
SERVER_LOG_ARTIFACT_TYPE = "ginfer_serve_request_log"
...
class ServerLogTail:
    ...
    def _check_process(self) -> None:
        ...
        if returncode is not None:
            raise CampaignError(f"ginfer-serve exited unexpectedly with status {returncode}")
...
class RunningServer:
    ...
    def wait_until_ready(self) -> dict[str, Any]:
        ...
        while True:
            returncode = self.process.poll()
            if returncode is not None:
                raise CampaignError(f"ginfer-serve exited during startup with status {returncode}")
            ...
            if time.monotonic() >= deadline:
                raise CampaignError(
                    f"timed out waiting for ginfer-serve at http://{self.host}:{self.port}"
                )
            time.sleep(0.2)
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--serve",
        type=Path,
        default=REPO_ROOT / "build/apps/ginfer-serve",
        help="ginfer-serve executable",
    )
    parser.add_argument(
        "--artifact",
        action="append",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not serve.is_file():
        raise CampaignError(f"ginfer-serve executable not found: {serve}")
    if not os.access(serve, os.X_OK):
        raise CampaignError(f"ginfer-serve is not executable: {serve}")
