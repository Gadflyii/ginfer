...
def main() -> None:
    ...
    ginfer_captures: dict[str, torch.Tensor] = {}
    with RefModel(args.weights, device=device, compile_codec=True) as model, torch.inference_mode():
        ...
        ginfer_output = model.encode_vision(
            batch,
            compile_codec=True,
            tap=lambda name, value: ginfer_captures.__setitem__(
                name, value.detach().to(device="cpu", dtype=torch.bfloat16)
            ),
        )
        ginfer_captures["merger"] = torch.cat(
            [
                value
                for value in (ginfer_output.image_embeddings, ginfer_output.video_embeddings)
                if value is not None
            ]
        ).detach().to(device="cpu", dtype=torch.bfloat16)
        vision_stats = ginfer_output.stats
    if device.type == "cuda":
    ...
    comparisons = {
        name: metrics(ginfer_captures[name], hf_captures[name])
        for name in ("block_00", "block_13", "block_26", "merger")
    }
    report = {
        "format": "ginfer_vision_bf16_comparison_v1",
