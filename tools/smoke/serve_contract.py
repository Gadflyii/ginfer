"""Exercise the implemented GInfer HTTP product contract with the standard library."""
...
def exercise(base_url: str, model: str) -> dict[str, Any]:
    ...
    if entries[0].get("id") != model or entries[0].get("owned_by") != "ginfer":
        raise ContractError("model-list response does not identify the configured GInfer model")
    ...
    stops = ["__GINFER_SMOKE_UNLIKELY_STOP__"]
    ...
    return {
        "format": "ginfer_serve_contract_v2",
        "model": model,
