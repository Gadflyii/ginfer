"""Run the Qwen3.6 thinking-preservation fixture through a real ginfer-serve process."""
...
def wait_for_server(base_url: str, process: subprocess.Popen[str], timeout: float) -> None:
    ...
    while time.monotonic() < deadline:
        if process.poll() is not None:
            raise TestFailure(f"ginfer-serve exited during startup with code {process.returncode}")
        try:
            ...
            last_error = error
        time.sleep(0.25)
    raise TestFailure(f"ginfer-serve was not healthy after {timeout:g}s: {last_error}")
...
def main() -> None:
    ...
    parser.add_argument("--server-bin", type=Path, default=Path("build/apps/ginfer-serve"))
    parser.add_argument(
        "--fixture",
        type=Path,
    ...
    with tempfile.TemporaryDirectory(prefix=f"ginfer-{args.backend}-thinking-") as temporary:
